...
status_code_t http_server_handle(void)
{
    ...
    FreeRTOS_setsockopt (pxTCPServer->xServers[0].xSocket,
                         0,
    ...
    while (1)
    {
        ...
        if (RESET_VALUE == strncmp((char*) pxTCPServer->pcCommandBuffer, GET_SWITCH1_COMMAND, GET_SWITCH1_COMMAND_LENGTH))
        {
        ...
        if (RESET_VALUE == strncmp((char*) pxTCPServer->pcCommandBuffer, GET_SWITCH2_COMMAND, GET_SWITCH2_COMMAND_LENGTH))
        {
        ...
        if (APP_CHECK_KEY)
        {
            ...
            APP_ERR_RET(pdTRUE !=  xResult, (status_code_t)xResult, "Failed to close the TCP socket.\r\n");
...
static void toggleLED(bsp_io_port_pin_t ledNumber)
{
    ...
    R_IOPORT_PinWrite (&g_ioport_ctrl, ledNumber, (led_status == BSP_IO_LEVEL_LOW) ? BSP_IO_LEVEL_HIGH : BSP_IO_LEVEL_LOW);
}
...
static fsp_err_t freertos_fat_init(void)
{
    ...
    APP_ERR_RET(FSP_SUCCESS !=  err, err, "FREERTOS PLUS FAT OPEN API failed\r\n");
    ...
    APP_ERR_RET(FSP_SUCCESS !=  err, err, "FreeRTOS Plus FAT Media Init API failed\r\n");
    ...
    APP_ERR_RET(FSP_SUCCESS !=  err, err, "FreeRTOS Plus FAT Disk Init API failed\r\n");
    ...
    APP_ERR_RET(FF_ERR_NONE !=  ff_err, ff_err, "FF_Mount API failed\r\n");

    /* Add the disk to the file system. */
    ff_err = FF_FS_Add ("/", &g_disk);
    APP_ERR_RET(FF_ERR_NONE ==  ff_err, ff_err, "FF_FS_Add API failed\r\n");

    return FSP_SUCCESS;
}

/**********************************************************************************************************************
 * @brief       This function update the network information.
 * @param[IN]   None.
 * @retval      None.
 *********************************************************************************************************************/
static void update_network_info(void)
{
    ...
    snprintf (ip_content_body, sizeof(ip_content_body), INNER_HTML_NET_ID, g_mac_address[0],
                                      g_mac_address[1], g_mac_address[2], g_mac_address[3],
                                      g_mac_address[4], g_mac_address[5], g_ip_address[0],
                                      g_ip_address[1], g_ip_address[2], g_ip_address[3],
                                      g_netmask[0], g_netmask[1], g_netmask[2],
                                      g_netmask[3], g_gateway_address[0], g_gateway_address[1],
...
static status_code_t write_file(const char *fileName, const char *content)
{
    ...
    if (file_pointer != NULL)
    {
        ...
        snprintf ((char*) g_write_data, sizeof(g_write_data), content);
...
status_code_t tcp_client_handle(char *destination)
{
    ...
    remote_address.sin_address.ulIP_IPv4 = FreeRTOS_inet_addr_quick(ipv4_address[0], ipv4_address[1], ipv4_address[2], ipv4_address[3]);
    ...
    APP_ERR_RET(FREERTOS_INVALID_SOCKET ==  client_socket, (status_code_t)STATUS_ERROR, "Error: Failed to create socket.\r\n");
    ...
    if (PING_SUCCESS == ping_status)
    {
        ...
        {
            APP_PRINT("\r\nConnected to server at %d.%d.%d.%d:%d\r\n",ipv4_address[0], ipv4_address[1], ipv4_address[2], ipv4_address[3],port);
...
status_code_t udp_client_handle(char *destination)
{
    ...
    if (PING_SUCCESS == ping_status)
    {
        ...
        bytes_received = FreeRTOS_recvfrom (client_socket, /* The socket being received from. */
                                            recv_buffer, /* The buffer into which the received data will be written. */
                                            sizeof(recv_buffer), /* The size of the buffer provided to receive the data. */
                                            0, /* ulFlags with the FREERTOS_ZERO_COPY bit clear. */
                                            &remote_address, /* The address from where the data was sent (the source address). */
                                            &xAddressLength);
