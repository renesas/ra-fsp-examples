...
static WIFIReturnCode_t scan_wifi(void)
{
    WIFIScanResult_t scan_data[MAX_WIFI_SCAN_RESULTS] = {RESET_VALUE};
...
static void manual_connect(void)
{
    ...
    while ((ZERO_ASCII > input_buff[INDEX_ZERO]) || (TWO_ASCII < input_buff[INDEX_ZERO]) || (NULL_CHARACTER != input_buff[INDEX_ONE]));
