...
static void board_ui(void)
{
    ...
    switch (g_u1_motor_status)
    {
        case MOTOR_SENSORLESS_CTRL_STOP:
            ...
            /* Check SW1 */
            if ((MTR_FLG_SET == mtr_remove_sw_chattering(u1_temp_sw_signal, SW_ON)) && (MTR_FLG_SET != g_u1_stop_req))
            {
        ...
        case MOTOR_SENSORLESS_CTRL_RUN:
            ...
            /* Check SW1 */
            if ((MTR_FLG_SET == mtr_remove_sw_chattering(u1_temp_sw_signal, SW_OFF)) || (MTR_FLG_CLR != g_u1_stop_req))
            {
        ...
        case MOTOR_SENSORLESS_CTRL_ERROR:
            ...
            if ((SW_OFF == g_u1_reset_req) && (MTR_FLG_SET == mtr_remove_sw_chattering(u1_temp_sw_signal, SW_ON)))
            {
                g_u1_reset_req = SW_ON;
            }
            else if ((SW_ON == g_u1_reset_req) && (MTR_FLG_SET == mtr_remove_sw_chattering(u1_temp_sw_signal, SW_OFF)))
            {
