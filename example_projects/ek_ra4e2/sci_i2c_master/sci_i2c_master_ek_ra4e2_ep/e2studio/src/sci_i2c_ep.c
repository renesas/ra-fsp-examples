...
/*******************************************************************************************************************//**
 *  @brief       This function wait i2c event from SCI I2C master callback
...
static fsp_err_t sci_i2c_wait_event(const i2c_master_event_t event)
{
    ...
    /* Wait until i2c master event equal to need wait event */
...
static fsp_err_t sensor_init_hardware(void)
{
    ...
    APP_ERR_RETURN(err, "R_SCI_B_I2C_Open API failed\r\n");
#else
    err = R_SCI_I2C_Open(&g_sci_i2c_master_ctrl, &g_sci_i2c_master_cfg);
    APP_ERR_RETURN(err, "R_SCI_I2C_Open API failed\r\n");
#endif
...
#if (BSP_FEATURE_SCI_VERSION == 2U)
    err = R_SCI_B_2C_Write(&g_sci_i2c_master_ctrl, i2c_buf, I2C_TWO_BYTE, false);
    APP_ERR_RETURN(err, "R_SCI_B_2C_Write API failed\r\n");
#else
    err = R_SCI_I2C_Write(&g_sci_i2c_master_ctrl, i2c_buf, I2C_TWO_BYTE, false);
    APP_ERR_RETURN(err, "R_SCI_I2C_Write API failed\r\n");
#endif
...
static fsp_err_t sensor_read_reg(uint8_t addr, uint8_t * p_value)
{
    ...
    APP_ERR_RETURN(err, "R_SCI_B_I2C_Write API failed\r\n");
#else
    ...
    APP_ERR_RETURN(err, "R_SCI_B_I2C_Read API failed\r\n");
#else
    err = R_SCI_I2C_Read(&g_sci_i2c_master_ctrl, p_value, I2C_ONE_BYTE, false);
    APP_ERR_RETURN(err, "R_SCI_I2C_Read API failed\r\n");
#endif
...
static void sensor_deinit(void)
{
    ...
    if(g_sci_i2c_master_ctrl.open)
    {
        ...
        if(FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("R_SCI_B_I2C_Close API failed\r\n");
        }
#else
...
/*******************************************************************************************************************//**
 *  @brief       This function is used initialize related module and start the sci i2c example operation.
 *  @param[in]   None
 *  @retval      None
 **********************************************************************************************************************/
