...
#define SUB_OPTIONS4     "\r\n d) RA MCU 128-bit Unique ID (hex):   \x1b[32m%08x\x1b[37m-"

#define SUB_OPTIONS5                                               "\x1b[32m%08x\x1b[37m-"
#define SUB_OPTIONS6                                               "\x1b[32m%08x\x1b[37m-"
#define SUB_OPTIONS7                                               "\x1b[32m%08x\x1b[37m"

#define SUB_OPTIONS8     "\r\n e) RA MCU Die temperature (F/C):     \x1b[32m%d.%02d/%d.%02d\x1b[37m         "
#define SUB_OPTIONS9     "\r\n f) Blue LED blinking frequency (Hz): \x1b[32m%d\x1b[37m "
#define SUB_OPTIONS10    "\r\n g) Blue LED blinking intensity (%%%%):  \x1b[32m%d\x1b[37m "
...
void  update_console(void)
{
    if (0 == ( --s_console_update_speed))
    {
        ...
        /*p_uid cast to unsigned int*/
        sprintf(s_print_buffer, SUB_OPTIONS4, (uint_t)p_uid->unique_id_words[0]);
        print_to_console(s_print_buffer);
        R_BSP_SoftwareDelay(sDelay, BSP_DELAY_UNITS_MICROSECONDS);

        /*p_uid cast to unsigned int*/
        sprintf(s_print_buffer, SUB_OPTIONS5, (uint_t)p_uid->unique_id_words[1]);
        print_to_console(s_print_buffer);
        R_BSP_SoftwareDelay(sDelay, BSP_DELAY_UNITS_MICROSECONDS);

        /*p_uid cast to unsigned int*/
        sprintf(s_print_buffer, SUB_OPTIONS6, (uint_t)p_uid->unique_id_words[2]);
        print_to_console(s_print_buffer);
        R_BSP_SoftwareDelay(sDelay, BSP_DELAY_UNITS_MICROSECONDS);

        /*p_uid cast to unsigned int*/
        sprintf(s_print_buffer, SUB_OPTIONS7, (uint_t)p_uid->unique_id_words[3]);
        print_to_console(s_print_buffer);
        R_BSP_SoftwareDelay(sDelay, BSP_DELAY_UNITS_MICROSECONDS);

        sprintf(s_print_buffer, SUB_OPTIONS8,  wn_mcu_temp_f, fr_mcu_temp_f, wn_mcu_temp_c, fr_mcu_temp_c);
        print_to_console(s_print_buffer);
        R_BSP_SoftwareDelay(sDelay, BSP_DELAY_UNITS_MICROSECONDS);

        sprintf(s_print_buffer, SUB_OPTIONS9,  g_pwm_rates_data[g_board_status.led_frequency]);
        print_to_console(s_print_buffer);
        R_BSP_SoftwareDelay(sDelay, BSP_DELAY_UNITS_MICROSECONDS);

        sprintf(s_print_buffer, SUB_OPTIONS10, g_pwm_dcs_data[g_board_status.led_intensity]);
