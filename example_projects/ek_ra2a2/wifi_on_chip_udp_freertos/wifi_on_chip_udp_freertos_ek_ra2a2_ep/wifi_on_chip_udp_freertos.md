...
|configuration.xml > g_uart0 > module > general > channel | 0 | 9 | PMOD0 is available on UART channel 9. |
|configuration.xml > g_uart0 > module > Flow control > CTS/RTS Selection | - | Hardware RTS | To enable flow control. |
...
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
| --------------------------------------- | ----------------- | ---------------| ---------- |
|configuration.xml > BSP > common > main | 0x400 | 0x400 | Main Program thread stack is configured to store the local variables of different functions in the code. |
|configuration.xml > BSP > common > heap | 0 | 0x1000 |Heap size is required for standard library functions to be used.|
|configuration.xml > g_uart0 > module > general > channel | 0 | 3 | PMOD2 is available on UART channel 3. |
|configuration.xml > User App Thread > Properties > Settings > Property > Common > Memory Allocation > Total Heap Size |0 |0x4000|RAM is used to obtain memory allocation for secure libraries. |
