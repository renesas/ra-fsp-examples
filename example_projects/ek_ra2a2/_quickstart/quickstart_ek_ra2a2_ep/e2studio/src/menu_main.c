 ...
 * Description  : Contains macros, data structures and functions used in the Welcome screen page
 *********************************************************************************************************************/
...
int8_t main_display_menu(void)
{
    ...
    for (uint8_t test_active = 0; NULL != s_menu_items[test_active].p_func; test_active++ )
    {
    ...
    while (0 != c)
    {
        ...
        if (check_for_input_from_console())
        {
            ...
            if (0 != c)
            {
                ...
                /* Selecting which menu based on user input  */
                g_selected_menu = c;
