...
#define EP_INFO     "\r\nThe example project demonstrates the typical use of the CRC HAL module APIs.\r\n"\
                    ...
                    "interface. If the board does not support snoop mode (FPB-RA0E1), reception is performed in\r\n"\
                    "normal mode through SAU interface. On pressing any key through RTT Viewer, CRC value in normal\r\n"\
                    "mode is calculated for 4 bytes of data. The calculated CRC value and input data are transmitted\r\n"\
                    "and received through loopback on SCI_UART (except for FPB-RA0E1, which uses SAU_UART).\r\n"\
