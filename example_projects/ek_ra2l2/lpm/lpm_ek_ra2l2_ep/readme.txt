...
2. Hardware Requirements:
	...
	For RA boards do not support J-Link On Board VCOM functionality: EK-RA2A1, EK-RA2E1, EK-RA2E2, EK-RA2L1, EK-RA4M1, EK-RA4M2, EK-RA4M3, 
	EK-RA4W1, EK-RA6M1, EK-RA6M2, EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA6M5, FPB-RA4E1, FPB-RA6E1, MCK-RA6T2, RSSK-RA6T1
...
3.Hardware Connections:
	...
	For FPB-RA0E1:
	  	...
	  	2) Replace R10 with 150ohm resistor.

4. Configuration Settings:
	The pin configuration file for LPM mode in each example project is named with the suffix "LPM". The user can change these configurations 
	...
	Note: 	If using RTOS, the SysTick timer must be stopped before entering the Sleep mode because any interrupt will cancel the sleep mode. 
...
5. The hardware modifications required for measuring the MCU's current.
	...
	For EK-RA2L1, EK-RA2E1, EK-RA2E2, EK-RA4M2, EK-RA4M3, EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA4E2, EK-RA6E2, EK-RA6M5, EK-RA8M1, 
	    EK-RA8D1, EK-RA4L1:
	  	...
	  	2) Measure the voltage drops across R3 and calculate MCU's current (Icc); replace R3 with a larger resistor if needed.
	...
	For FPB-RA6E1, FPB-RA4E1:
	  	1) Measure the voltage drops across R3 and calculate MCU's current (Icc); replace R3 with a larger resistor if needed.
	...
	For RSSK-RA6T1, MCK-RA6T2, MCK-RA8T1, FPB-RA0E1:
	  	Not applicable.
	...
	For FPB-RA2E3:
        ...
        For FPB-RA8E1:
	  	1) Measure the voltage drops across R2 and calculate MCU's current (Icc); replace R2 with a larger resistor if needed.
        ...
        For EK-RA2L2:
	  	1) Measure the voltage drops across R17 and calculate MCU's current (Icc); replace R17 with a larger resistor if needed.
	  	2) To measure MCU's current (Icc) directly, remove R17 resistor and connect Multi-meter between the +3V3 and +3V3_MCU pins on the 
	           kit connectors.

6. Verifying Operation:
	1) Download the example project to the RA board using the USB Debug Port. J-Link may be disconnected when MCU enter LPM mode. It is 
	   recommended to stop debugging after downloading the EP.
	...
	5) Press the user push-button S1 or wait 10 seconds to enter and cancel LPM mode. It will turn off LED1 before entering each LPM mode 
	...
	5. Normal → Deep SW Standby → Normal.					(Supported by RA4E2, RA4M2, RA4M3, RA4E1, RA4T1, RA6 MCU lines, and RA8 MCU lines)
