...
static void master_write_slave_read_operation(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_SSI_Write API failed. Closing SSI and GPT \r\n");
        deinit_ssi();
        deinit_gpt();
        /* Trap here */
        APP_ERR_TRAP(err);
    ...
    while(WRITE_COUNTS > g_write_counter)
    {
        if ((true == g_send_data_in_main_loop))
        {
            ...
            {
                g_send_data_in_main_loop = false;
                err = R_SSI_Write(&g_i2s_master_ctrl, &g_src_buff[g_buf_row_index][INDEX_0],\
...
static void master_read_slave_write_operation(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_SSI_Read API failed. Closing SSI and GPT \r\n");
        deinit_ssi();
        deinit_gpt();
        /* Trap here */
        APP_ERR_TRAP(err);
    ...
    while(true)
    {
        ...
        if (true == g_read_data_in_main_loop)
        {
            ...
            if((I2S_EVENT_IDLE == g_i2s_event) || (I2S_EVENT_RX_FULL == g_i2s_event))
            {
                ...
                /* Read data from slave */
                err = R_SSI_Read(&g_i2s_master_ctrl, &g_dest_buff[g_buf_row_index][INDEX_0],\
                                 (BUF_FIFO_STAGE)* sizeof(int32_t));
                /* Handle error */
                if (FSP_SUCCESS != err)
                {
...
void ssi_example_callback(i2s_callback_args_t * p_args)
{
    ...
    if (((I2S_EVENT_TX_EMPTY == p_args->event) || (I2S_EVENT_IDLE == p_args->event))  && \
        (MASTER_WRITE_SLAVE_READ == g_selection_value) && (WRITE_COUNTS > g_write_counter))
    {
    ...
    if (((I2S_EVENT_RX_FULL == p_args->event) || (I2S_EVENT_IDLE == p_args->event)) && \
        (MASTER_READ_SLAVE_WRITE == g_selection_value)&& (READ_COUNTS > g_read_counter))
    {
        /* Reload FIFO and handle errors. */
