...
static fsp_err_t can_validate_result(void)
{
    ...
    else if (RESET_VALUE == strncmp ((char*) &rxMsg.data[ZERO], (char*) &can_ack_msg[ZERO],\
...
static void can_handle_error(fsp_err_t status,  const char * err_str)
{
    if(FSP_SUCCESS != status)
    {
        ...
        /* Close opened CGC module */
        if (RESET_VALUE != g_cgc_ctrl.open)
        {
            if (FSP_SUCCESS != R_CGC_Close(&g_cgc_ctrl))
            {
...
void can_entry (void)
{
...
#if defined (BOARD_RA2A1_EK) || defined (BOARD_RA2L1_EK) || defined (BOARD_RA2L2_EK)
        /* Initialize the CGC */
        status = R_CGC_Open(&g_cgc_ctrl, &g_cgc_cfg);
        can_handle_error(status,"\r\n **R_CGC_Open API failed**");

        /* Start the main-clock oscillator */
        status = R_CGC_ClockStart(&g_cgc_ctrl, CGC_CLOCK_MAIN_OSC, NULL);
    ...
    while (true)
    {
        ...
        if (APP_CHECK_DATA)
        {
            ...
            APP_PRINT("\r\n(1) This is the receiver Node."
                      " Sending a remote frame to request the data from Sender Node.\r\n");
        ...
        if (true == g_can_rx)
        {
            ...
            if (CAN_FRAME_TYPE_REMOTE == rxMsg.type)
            {
                ...
                APP_PRINT("\r\n(2) This is the sender Node. Received the remote frame: ID 0x%x, DLC %d,"
                          " responding with four 'TX__MESG'\r\n", rxMsg.id, rxMsg.data_length_code);
