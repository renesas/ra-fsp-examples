 ...
 * @param  resp_length: A pointer to a variable that will contain the PPS response length.
 * @param  pps1:        Pointer to a PPS1 value.
 * @param  etu:         Elementary time unit in us.
 * @retval The status of the PPS exchange. The returned value can be one of the following:
...
bool PPS_receive_character(uint8_t received_data)
{
    ...
    if(pps_received_count == 2)
    {
        ...
        pps_response_length = (uint8_t)(((received_data & 0x40)>>6) + \
                              ((received_data & 0x20)>>5) + ((received_data & 0x10)>>4)) + 3;
    }
 ...
 * @retval The status of the PPS comparison. The returned value can be one of the following:
 ...
 * @param  len_response:    A pointer to a variable that contains the length of the PPS response buffer.
 * @param  etu_usec:        ETU in us.
 * @retval None.
 **********************************************************************************************************************/
static fsp_err_t PPS_Receive(uint8_t * const response, uint8_t * len_response)
