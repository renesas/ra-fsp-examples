...
/******************************************************************************
 * Function Name   : usbx_cdc_dfu_device_status_change_callback
 * Description     : USB callback function for USBX Peripheral CDC Device status change
    ...
    while(true)
    {
        ...
        if (TX_SUCCESS != status && DETACH_EVENT_FLAG != actual_flags)
        {
            ...
            if (NULL != gp_peri_cdc_device)
            {
                ...
                if (FSP_SUCCESS == _ux_device_class_cdc_acm_read(gp_peri_cdc_device, g_receiveBuffer, CDC_DATA_LENGTH, &g_cdc_actual_length))
                {
                    ...
                    status =  _ux_device_class_cdc_acm_write(gp_peri_cdc_device, g_receiveBuffer, g_cdc_actual_length, &g_cdc_actual_length);
                    if (UX_SUCCESS != status)
                    {
                    ...
                    status =  _ux_device_class_cdc_acm_write(gp_peri_cdc_device, g_recBuffer, g_cdc_actual_length, &g_cdc_actual_length);
                    if (UX_SUCCESS != status)
                    {
                        PRINT_ERR_STR("_ux_device_class_cdc_acm_write API failed");
                    }
...
void usbx_peri_cdc_dfu_init(void)
{
    ...
    status = ux_device_stack_class_register (_ux_system_slave_class_cdc_acm_name, _ux_device_class_cdc_acm_entry, CONFIG_NUMB,
                                             INTERFACE_NUMB0, (void*) &g_ux_device_class_cdc_parameter);
    /* Error Handle */
    if (UX_SUCCESS != status)
    {
