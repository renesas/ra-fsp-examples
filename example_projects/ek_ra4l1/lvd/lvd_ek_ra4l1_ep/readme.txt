...
1. Project Overview:
    	...
    	the user has to increase back the input voltage value above LVD0 threshold value. By doing this, board gets powered up
    	...
    	The user has to rise input voltage above 2.8V again to detect LVD0 reset.
...
3. Hardware Connections:
	Connect External JLink with PC via a Type-B USB cable.
    	...
    	For EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA4M3, EK-RA6M5:
...
2) For EK-RA2E2, FPB-RA2E3, EK-RA2A2 boards, use external debugger for flashing the code, disconnect debugging mode,
   push on board reset button. The user needs to decrease the input Voltage belows 1.7V (Power-on Reset Voltage - VPOR) 
   then raises the input Voltage to 3.3V and open J-Link RTT Viewer to see the output by changing input voltage values.

3) For EK-RA8M1, EK-RA8D1, MCK-RA8T1, FPB-RA8E1 after change input voltage values from below low threshold (ex: 2.8V) 
