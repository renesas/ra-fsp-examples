        ...
        if (APP_CHECK_DATA)
        {
            ...
            switch (converted_rtt_input)
            {
                case WRITE_AND_READ:
                    ...
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("\r\n** SPI WRITE AND READ Demo failed **\r\n");
                        spi_clean_up();
                        APP_ERR_TRAP(err);
                    }
                    else
                ...
                case WRITE_READ:
                    ...
                    {
                        APP_PRINT("\r\n** SPI WRITE READ Demo Successful **\r\n");
                    }
                    break;
                case EXIT:
                    err = spi_exit_demo();
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("\r\n** SPI EXIT Demo failed **\r\n");
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("\r\n** SPI EXIT Demo Successful **\r\n");
                    }
                    break;
                default:
                    /* Menu for User Selection */
...
/*******************************************************************************************************************//**
 * @brief       This function is called at various points during the startup process.
