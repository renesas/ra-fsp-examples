...
/* NIST vector */
/* expected sha256 short message length to compare the output of crypto algorithm*/
...
static uint32_t netx_secure_crypto_rsa_2048_example (void)
{
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_initialize in rsa is failed");
        return status;
    }
    PRINT_INFO_STR("RSA2048 algorithm initialized and started successfully...");
    /* Encryption. */
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa_init in rsa is failed");
        return status;
    }
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa_operation in RSA is failed");
        return status;
    }
    /* compares the output data with the expected secret key data */
    status = (uint32_t) memcmp(output, secret_2048, sizeof(m_2048));
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("key match in RSA is failed");
        return status;
    }
    /* cleanup the rsa metadata */
    status = _nx_crypto_method_rsa_cleanup(metadata);
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa_cleanup in RSA is failed");
        return status;
    }
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa_init in RSA decrypt is failed");
        return status;
    }
    ...
    /* compare the output data with plain text of executing the decryption */
    status = (uint32_t) memcmp(output, plain_2048, sizeof(m_2048));
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("Key match in RSA decrypt is failed");
        return status;
    }
    PRINT_INFO_STR("RSA2048 algorithm executed successfully and encrypt/decrypt matches");
    /* cleanup the aes metadata */
    status = _nx_crypto_method_rsa_cleanup(metadata);
    if (NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_ecdsa_cleanup api is failed");
        return status;
    }
    return status;
}
...
static uint32_t netx_secure_crypto_rsa_4096_example (void)
{
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_initialize in rsa4096 is failed");
        return status;
    }
    PRINT_INFO_STR("RSA4096 algorithm initialized and started successfully...");
    /* Encryption. */
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa4096_init in rsa is failed");
        return status;
    }
    status = _nx_crypto_method_rsa_operation (NX_CRYPTO_ENCRYPT, handler, &crypto_method_rsa, (uint8_t*) public_e,
                                              RM_NETX_SECURE_CRYPTO_BYTES_TO_BITS(sizeof(public_e)),
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa_operation in RSA4096 is failed");
        return status;
    }
    /* comapre the output of encrypted data with the expected secret key */
    status = (uint32_t) memcmp(output, secret_4096, sizeof(m_4096));
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("key match in RSA4096 is failed");
        return status;
    }
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_rsa_cleanup in RSA4096 is failed");
        return status;
    }

    PRINT_INFO_STR("RSA4096 algorithm executed successfully and encrypt matches");
    return status;
}

/*******************************************************************************************************************//**
 * @brief     This is an example function for HASH sha256 algorythm using NetX cypto API
 * @param[IN] None
...
static uint32_t netx_secure_crypto_sha256_example(void)
{
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_initialize in sha_256 example is failed");
        return status;
    }

    PRINT_INFO_STR("SHA256 algorithm initialized and started successfully...");
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_sha256_init in rsa is failed");
        return status;
    }
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_sha256_operation in rsa at NX_CRYPTO_HASH_INITIALIZE is failed");
        return status;
    }
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_sha256_operation in rsa at NX_CRYPTO_HASH_UPDATE is failed");
        return status;
    }
    ...
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("_nx_crypto_method_sha256_operation in rsa at NX_CRYPTO_HASH_CALCULATE is failed");
        return status;
    }
    ...
    {
        PRINT_ERR_STR("key match in sha 256 is failed");
        return status;
    }
    /* cleanup the sha256 metadata */
    status = _nx_crypto_method_sha256_cleanup(metadata);
    if(NX_CRYPTO_SUCCESS != status)
    {
        PRINT_ERR_STR("sha 256 cleanup api failed");
        return status;
    }
    PRINT_INFO_STR("sha256 algorithm is executed successfully!");
    return status;
}
