...
# Introduction #
This example project demonstrates the typical use of the I3C Driver on Renesas RA MCUs based on Renesas FSP. The I3C Master on RA board demonstrates operations associated with an I3C slave running on another RA board. Once initialization is successful, I3C Master device assigns slave address to I3C slave device through DAA (Dynamic Address Assignment) using I3C Common Command Code (CCC). Once Dynamic Address Assignment is completed, the Master EP performs write/read operations, and displays slave device information based on user input. If the on-board switch is pressed on the slave side, it will initiate an IBI transfer request. Error and info messages will be printed on the J-Link RTT Viewer. Refer **[LED Status](#led-status)** section for LED observation.

Note: EK-RA8D1, EK-RA8P1, EK-RA8D2, EK-RA8M2, EK-RA8T2, EK-RA8P1 V2 boards support I3C HDR-DDR mode. The Master EP can perform write/read operations both in the SDR mode and HDR-DDR mode.
...
### Hardware ###
* Supported RA boards: EK-RA2E2, EK-RA4E2, EK-RA6E2, MCK-RA4T1, MCK-RA6T3, EK-RA8M1, EK-RA8D1, EK-RA4L1, EK-RA2L2, EK-RA8P1, EK-RA8D2, EK-RA8M2, EK-RA8T2, EK-RA8P1 V2.
...
![I3C](images/i3c_high_level_design.jpg "High Level Block Diagram")
...
6. Also on slave side the user can perform IBI transfer request using on-board push button (SW2) and verify result on J-Link RTT Viewer. For MCK-RA4T1/MCK-RA6T3: The user needs to use Toggle switch (S1) instead.
...
## LED Status ##
The observations of the LED operation on the slave board are as follows:
* For MCK-RA4T1, MCK-RA6T3:
...
* For all other supported boards:
