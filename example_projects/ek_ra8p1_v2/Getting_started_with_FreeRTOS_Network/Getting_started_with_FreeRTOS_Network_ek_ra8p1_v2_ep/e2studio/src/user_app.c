...
status_code_t http_server_handle(void)
{
    ...
    while (1)
    {
                                   ...
                                   GET_SWITCH1_COMMAND, GET_SWITCH1_COMMAND_LENGTH))
        {
            ...
            toggleLED((bsp_io_port_pin_t)g_bsp_leds.p_leds[LED1]);
            APP_PRINT("\r\nLED1 state has been toggled.\r\n");
        }
                                   ...
                                   GET_SWITCH2_COMMAND, GET_SWITCH2_COMMAND_LENGTH))
        {
            ...
            toggleLED((bsp_io_port_pin_t)g_bsp_leds.p_leds[LED2]);
            APP_PRINT("\r\nLED2 state has been toggled.\r\n");
        }
...
static void update_status_leds(void)
{
    ...
    {
        R_IOPORT_PinRead(&g_ioport_ctrl, (bsp_io_port_pin_t)g_bsp_leds.p_leds[i], &led_state);
...
status_code_t tcp_client_handle(char *destination)
{
    ...
    sscanf(destination, "%d.%d.%d.%d:%d", &ipv4_address[0], &ipv4_address[1], &ipv4_address[2],
           &ipv4_address[3], &port);
...
status_code_t udp_client_handle(char *destination)
{
    ...
    if (PING_SUCCESS == ping_status)
    {
        ...
        bytes_sent = FreeRTOS_sendto(client_socket, send_message, strlen((char*) send_message) + 1,
                                     0, &remote_address, sizeof(remote_address));
