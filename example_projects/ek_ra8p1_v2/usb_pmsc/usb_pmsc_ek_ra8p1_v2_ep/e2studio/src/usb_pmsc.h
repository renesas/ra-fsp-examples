...
#else
#define CARD_TYPE   "SD"
#endif /* BOARD_RA8T1_MCK, BOARD_RA8T2_MCK */
...
#else
#define EP_INFO     "\r\nThis example project demonstrates basic functionalities of USB Peripheral Mass Storage\r\n"\
                    "Class (PMSC) driver with FreeRTOS on Renesas RA MCUs based on Renesas FSP. FreeRTOS\r\n"\
                    "uses the underlying Block Media driver which utilizes USB driver. The PMSC driver\r\n"\
                    "internally configures memory device ("CARD_TYPE" Card) to pop up as a memory drive on host PC.\r\n"\
