# Introduction #
This example project demonstrates the basic functionalities of the GLCDC device driver on Renesas RA MCUs based on Renesas FSP. On successful initialization of the GLCDC module, the EP will read the display resolution and frame buffer set in the GLCDC module and draw color bands to cover the entire screen of external LCD connected to RA MCU. The user can view raw image in e2studio IDE. Errors and status information will be printed on J-Link RTT Viewer during the execution of the project.
...
* For EK-RA6M3G:
    * Connect Graphics Expansion Board to J1, please refer to section 8.1 in EK-RA6M3G - User Manual https://www.renesas.com/us/en/document/mat/ek-ra6m3g-v1-users-manual?r=1168091
* For EK-RA8D1:
    * Set the configuration switches (SW1) as below to avoid potential failures.

        | SW1-1 PMOD1 | SW1-2 TRACE | SW1-3 CAMERA | SW1-4 ETHA | SW1-5 ETHB | SW1-6 GLCDC | SW1-7 SDRAM | SW1-8 I3C |
        |-------------|-------------|--------------|------------|------------|------------|-------------|-----------|
    ...
    * If the user does not have Renesas APP-LCD-CONV1 board:
        * When using jumper wires to connect Graphic Expansion Board to RA board manually instead of using APP-LCD-CONV1 board. Please connect EK-RA8D1's pin to Graphics Expansion Board's pin by following the below table. 
...
* For EK-RA8E2:
    * Connect J1 on Graphics Expansion Board to J1 on the RA board. Please carefully align pin 2 on the Graphic Expansion Board with pin 1 on the RA board.
* For EK-RA8P1, EK-RA8P1 V2:
    * Connect J1 on Graphics Expansion Board to J1 on the RA board. Please carefully align pin 1 on the Graphic Expansion Board with pin 1 on the RA board.
...
* For EK-RA8D2:
    * Connect J1 on Graphics Expansion Board to J1 on the RA board. Please carefully align pin 1 on the Graphic Expansion Board with pin 1 on the the RA board.
...
| configuration.xml > Stacks > g_display Graphics LCD (r_glcdc) > Properties > Settings > Property > Module g_display Graphics LCD (r_glcdc) > Output > Timing > Sync edge | Falling edge | Falling edge | Sync signal is rising edge for LCD. |
...
| configuration.xml > Stacks > g_display Graphics LCD (r_glcdc) > Properties > Settings > Property > Module g_display Graphics LCD (r_glcdc) > TCON > Panel clock division ratio | 1/8 | 1/8 | Divider selection for generation of required specific frequency to the panel. |
...
* Before running the example project, refer to the steps below for hardware connections:
    * Connect J1 on Graphics Expansion Board to J1 on the RA board. Please carefully align pin 1 on the Graphic Expansion Board with pin 1 on the RA board.
    * Connect the USB Debug port on the RA board to the PC using a Type-C USB cable.

The image below showcases the output on J-Link RTT Viewer:

![glcdc_log_](images/rtt_log.jpg "RTT LOG")

The image below showcases the output on Graphic LCD:
...
The image below showcase the output in the Memory Monitor view in e2 studio:
