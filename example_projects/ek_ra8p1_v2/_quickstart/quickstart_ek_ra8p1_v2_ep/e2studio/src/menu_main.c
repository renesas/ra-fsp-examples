...
static st_menu_fn_tbl_t s_menu_items[] =
{
    ...
    {"AI Camera and Graphics Demonstration",    lcd_display_menu},
...
int8_t main_display_menu(void)
{
    ...
    if (-1 == g_next_menu_selection)
    {
		...
		sprintf (s_print_buffer, MODULE_NAME, g_product_full_name);
		 ...
		 if(true == ts_configured())
		 {
		     ...
		     if(true == check_ov5640_camera_connected())
		     {
		         ...
		         print_to_console("Camera Expansion Board: Connected and Enabled\r\n");
		 	     ...
		 	     sprintf(g_camera_welcome_message_str, "AI Camera and Graphics Demonstration");
		     }
		     else
		     {
                 print_to_console("\r\nParallel Graphics Expansion Board: Connected and Enabled\r\n");
...
//    	    configure_led(LED_CONFIGURED_NORMAL);
     ...
     while (0 != c)
    {
                ...
                ((LCD_FULL_BG_POPUP_MENU != on_screen_selection) && (-1 != on_screen_selection)))
        {
            ...
            if ((-1 != on_screen_selection) && (LCD_FULL_BG_POPUP_MENU != on_screen_selection))
            {
            ...
            if ((c > 0) && (c <= g_menu_limit))
            {
                ...
                if (-1 == g_next_menu_selection) // Screen selection will occur as option changed
                {
            ...
            {
                if (-1 == on_screen_selection) // Screen selection didn't occur
                {
                    start_key_check();
                }
            }
        ...
        if (LCD_FULL_BG_POPUP_MENU == on_screen_selection)
        {
