...
extern vision_ai_app_err_t image_classification (void); // @suppress("3.8a Internal function declaration")
extern fsp_err_t           print_to_console     (char_t * p_data);   // @suppress("3.8a Internal function declaration")
...
int8_t   g_model_buffer_int8[AI_INPUT_IMAGE_WIDTH * AI_INPUT_IMAGE_HEIGHT * AI_INPUT_IMAGE_BYTE_PER_PIXEL]
                                                                                     BSP_ALIGN_VARIABLE(8);
uint32_t g_model_buffer_int8_size = sizeof(g_model_buffer_int8);
 ...
 * Description  : Read the image classification result to a buffer which will be used by the display function.
 ...
 *  @retval      None
***********************************************************************************************************************/
void update_classification_result(unsigned index, unsigned short category, float probability) // @suppress("8.2a Non-static function declaration location") @suppress("3.2a Line length formatting")
{
    ...
    if ((AI_MAX_DETECTION_NUM - 1) == index)
    {
 ...
 * Description  : This function can be called to print debug message
 * Argument     : s (null terminated string)
 * Return Value : None
 *********************************************************************************************************************/
static void print_log(const char* s)
{
    print_to_console((char_t *)s);
}

/*********************************************************************************************************************
 * Function Name: ai_inference_thread_entry
 * Description  :
...
void ai_inference_thread_entry(void *pvParameters)
{
    ...
    xEventGroupWaitBits(g_ai_app_event, (HARDWARE_DISPLAY_INIT_DONE | HARDWARE_CAMERA_INIT_DONE),
