...
#define R_DISPLAY_THREAD_INTIALISATION_SETTLE_FRAMES (3)
...
uint8_t * gp_single_buffer, * gp_double_buffer;

/* User defined functions */
uint8_t g_sdram_buffer[DISPLAY_BUFFER_STRIDE_BYTES_INPUT1 * DISPLAY_VSIZE_INPUT1 * 2] BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram") = "";
st_full_image_rgb565_t g_user_clean_background __attribute__ ((section (".sdram"))) = {1024, 600, 2, {""}};

static void * graphics_get_back_buffer(void);

static void graphics_reset_blit_buffer(void);
...
static void call_user_draw(void);
 ...
 *****************************************************************************/
volatile static uint32_t g_vsync_flag = 0;
volatile static uint8_t s_drw_buf = 0;
volatile static uint8_t g_update_fps_text = 1;
...
uint8_t  g_SDRAM_pixel_data[1024 * 600 * 4  + 1]  BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram");
static d2_renderbuffer * renderbuffer;
static uint8_t * gp_camera_buffer;

static char_t s_print_buffer[128] = {};

extern st_overlay_image_rgb565_t g_menu_overlay_image;

st_rgb565_text_block_page_t g_gs_page = {
        ...
        " GETTING STARTED          ", /* Title */
        {
            ...
            {{PAGE_LEFT_EDGE, LINE(13) + BLOCK(2)},"Refer to renesas.com and the EK-RA8P1 v2 Quick Start Guide for the full disclaimer text."},
        }
};

volatile d2_s16 g_origin_x = 15;
volatile d2_s16 g_box_x    = 50;
volatile d2_s16 g_box_y    = 40;

extern void r_update_touchpad(void);
...
{
    if (LCD_FULL_BG_USER_LED == g_current_backgroud.active_demo)
    {
        d2_setblitsrc(gp_d2_handle, (uint8_t *)&g_sdram_buffer_led[0], 1024, 1024, 600, d2_mode_rgb565);
    }
    else if ((0 == g_current_backgroud.active_demo) || (LCD_FULL_BG_WELCOME == g_current_backgroud.active_demo))
    {
        d2_setblitsrc(gp_d2_handle, (uint8_t *)&g_sdram_buffer_welcome[0], 1024, 1024, 600, d2_mode_rgb565);
    }
    else
    {
        d2_setblitsrc(gp_d2_handle, (uint8_t *)&g_sdram_buffer[0], 1024, 1024, 600, d2_mode_rgb565);
    }
}
/**********************************************************************************************************************
...
void display_thread_entry(void *pvParameters)
{
    ...
    memset(&g_user_clean_background.pixel_data[0], 0,
            (size_t)(DISPLAY_BUFFER_STRIDE_BYTES_INPUT1 * DISPLAY_VSIZE_INPUT1 * 2));
    ...
    d2_color col = 0;
    volatile uint16_t settle_frames = R_DISPLAY_THREAD_INTIALISATION_SETTLE_FRAMES;
    ...
    while(1)
    {
        ...
        if ((false == g_current_backgroud.update))
        {
            draw_background();
        ...
        /* Constant icons (remember exit is NOT shown when welcome screen is displayed */
            ...
            if(LCD_FULL_BG_CAMERA_BACKGROUND == g_current_backgroud.active_demo)
            {
                ...
                /* Update the layer to display in the GLCDC (will be set on next Vsync) */
                    R_GLCDC_BufferChange(&g_plcd_display_ctrl,
                                         (uint8_t * const) gp_camera_buffer, DISPLAY_FRAME_LAYER_1);
                }
...
void call_user_draw(void)
{
    switch (g_current_backgroud.active_demo)
    {
        ...
        case LCD_FULL_BG_NEXT_STEPS:      // Next Steps
            if (NULL != g_current_backgroud.p_text_block)
            {
                d2_setcolor(gp_d2_handle, 0, 0x00000000);
                d2_renderbox( gp_d2_handle, (d2_point)0 << 4, (d2_point)0 << 4,
                        (d2_point)(1024) << 4, (d2_point)(600) << 4);
        ...
        case 2: // led demo
            if (&g_ref_001_user_led_background.pixel_data[0] == g_current_backgroud.p_bg_src)
            {
...
static void set_background(void)
{
    ...
    if ((true == g_current_backgroud.update))
    {
    ...
    if (LCD_FULL_BG_EXTERNAL_MEMORY != g_current_backgroud.active_demo)
    {
...
static void draw_menu_icon(void)
{
    ...
    col = 0xFFFFFF;
    d2_setcolor(gp_d2_handle, 0, col);
    d2_renderline(gp_d2_handle, (d2_point) ((g_origin_x) << 4), (d2_point) ((origin_y + offset) << 4),
            (d2_point) ((g_origin_x + 40) << 4), (d2_point) ((origin_y + offset) << 4), (d2_point) (2 << 4), 0);
    ...
    d2_setcolor(gp_d2_handle, 0, col);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x) << 4),         (d2_point) ((border_y) << 4),
            (d2_point) ((border_x + g_box_x) << 4),      (d2_point) ((border_y) << 4),      (d2_point) (2 << 4), 0);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x) << 4),         (d2_point) ((border_y + g_box_y) << 4),
            (d2_point) ((border_x + g_box_x) << 4), (d2_point) ((border_y + g_box_y) << 4), (d2_point) (2 << 4), 0);

    d2_renderline(gp_d2_handle, (d2_point) ((border_x + g_box_x) << 4), (d2_point) ((border_y) << 4),
            (d2_point) ((border_x + g_box_x) << 4),      (d2_point) ((border_y + g_box_y) << 4), (d2_point) (2 << 4), 0);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x) << 4),         (d2_point) ((border_y) << 4),
            (d2_point) ((border_x) << 4),              (d2_point) ((border_y + g_box_y) << 4), (d2_point) (2 << 4), 0);
}
/**********************************************************************************************************************
...
static void draw_exit_icon(void)
{
    ...
    col = 0xFFFFFF;
    d2_setcolor(gp_d2_handle, 0, col);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x) << 4),
            (d2_point) ((border_y) << 4), (d2_point) ((border_x + g_box_x) << 4),
            (d2_point) ((border_y) << 4),         (d2_point) (2 << 4), 0);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x) << 4),
            (d2_point) ((border_y + g_box_y) << 4), (d2_point) ((border_x + g_box_x) << 4),
            (d2_point) ((border_y + g_box_y) << 4), (d2_point) (2 << 4), 0);

    d2_renderline(gp_d2_handle, (d2_point) ((border_x + g_box_x) << 4),
            (d2_point) ((border_y) << 4), (d2_point) ((border_x + g_box_x) << 4),
            (d2_point) ((border_y + g_box_y) << 4), (d2_point) (2 << 4), 0);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x) << 4),
            (d2_point) ((border_y) << 4), (d2_point) ((border_x) << 4),
            (d2_point) ((border_y + g_box_y) << 4), (d2_point) (2 << 4), 0);

    d2_renderline(gp_d2_handle, (d2_point) ((border_x + 4) << 4),
            (d2_point) ((border_y + 4) << 4),         (d2_point) ((border_x + g_box_x - 4) << 4),
            (d2_point) ((border_y + g_box_y - 4) << 4), (d2_point) (2 << 4), 0);
    d2_renderline(gp_d2_handle, (d2_point) ((border_x + 4) << 4),
            (d2_point) ((border_y + g_box_y - 4) << 4), (d2_point) ((border_x + g_box_x - 4) << 4),
            (d2_point) ((border_y + 4) << 4),         (d2_point) (2 << 4), 0);
...
static void initialise_primary_display(void)
{
    ...
    gp_single_buffer = (uint8_t *) g_plcd_display_cfg.input[0].p_base;
...
void dsp_set_background(uint32_t choice)
{
    ...
    switch (choice)
    {
        ...
        case  LCD_FULL_BG_CAMERA_BACKGROUND:
            g_current_backgroud.p_bg_src = (uint8_t *)&g_user_clean_background.pixel_data[0];
            g_current_backgroud.menu_icon_active = true;
            g_current_backgroud.exit_icon_active = true;
            break;
        ...
        case  LCD_FULL_BG_GETTING_STARTED:
            g_current_backgroud.p_bg_src = (uint8_t *)&g_user_clean_background.pixel_data[0];
            g_current_backgroud.p_text_block = &g_gs_page;
            g_current_backgroud.menu_icon_active = true;
            g_current_backgroud.exit_icon_active = true;
