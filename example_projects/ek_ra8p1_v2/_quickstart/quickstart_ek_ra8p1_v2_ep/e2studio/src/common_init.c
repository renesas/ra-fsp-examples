 ...
 *********************************************************************************************************************/
char_t g_product_full_name[DISPLY_PRODUCT_NAME_LENGTH_CFG] = "";
...
fsp_err_t common_init(void)
{
    ...
    board_cfg_switch_init();

    board_cfg_read_eeprom();

    /* Configure product name */
    if((true == g_eeprom_data.populated) && (strlen(g_eeprom_data.p_identity_str) < DISPLY_PRODUCT_NAME_LENGTH_CFG))
    {
        sprintf(g_product_full_name, "%s", g_eeprom_data.p_identity_str);
    }
    else
    {
        sprintf(g_product_full_name, "%s", FULL_NAME);
    }
