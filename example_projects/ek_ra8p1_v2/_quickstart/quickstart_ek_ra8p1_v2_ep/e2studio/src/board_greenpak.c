...
fsp_err_t board_greenpak_init(void)
{
    ...
    R_IIC_MASTER_SlaveAddressSet(&g_board_i2c_master_ctrl,
                                 BSP_I2C_SLAVE_ADDR_GREENPAK_EEPROM,
    ...
    if (((uint8_t) GREENPAK_IO4_EXPECTED_VALUE != t0) || ((uint8_t) GREENPAK_IO4_EXPECTED_VALUE != t1))
    {
        ...
        R_IIC_MASTER_SlaveAddressSet(&g_board_i2c_master_ctrl,
                                     BSP_I2C_SLAVE_ADDR_GREENPAK_RAM,
                                     I2C_MASTER_ADDR_MODE_7BIT);
        for (uint8_t ndx = 0; ndx < 16; ndx++)
        {
            reg_val1 = reg_start + ndx;
