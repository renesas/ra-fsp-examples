...
static void lcd_connection_status(e_display_status_t * set);

volatile  lv_indev_data_t g_s_tp_data;
volatile  lv_indev_data_t g_tp_copy_data;

volatile  bool_t g_s_output_enabled = false;
volatile  bool_t g_s_ts_enabled = false;

bool_t g_display_set = true;     // is this needed?

st_bounding_box_t g_hb_menu     = {{   2,   2}, {  90,  90}};
st_bounding_box_t g_hb_cancel   = {{ 940,   2}, { 1020, 90}};

st_bounding_box_t g_menu_cancel = {{ 820,  78}, { 900, 154}}; // over sized hit box as button is small
st_bounding_box_t g_kis_demo    = {{ 164, 180}, { 280, 305}};
st_bounding_box_t g_gs_demo     = {{ 314, 180}, { 430, 305}};
st_bounding_box_t g_led_demo    = {{ 454, 180}, { 570, 305}};
st_bounding_box_t g_lcd_demo    = {{ 598, 180}, { 712, 305}};
st_bounding_box_t g_ns_demo     = {{ 742, 180}, { 856, 305}};

static e_display_status_t s_detected_status = DISP_CONNECTION_NONE;

void r_update_touchpad(void);

/**********************************************************************************************************************
 * Function Name: lcd_connection_status
 * Description  : Get the LCD connection status
 * Argument     : set
 * Return Value : .
 *********************************************************************************************************************/
...
/**********************************************************************************************************************
 * Function Name: draw_hitboxes
 * Description  : Draw the hitboxes on camera page when touch controller scan the touching point.
 * Return Value : .
 *********************************************************************************************************************/
void draw_hitboxes(void)
{
    if(false == g_overlay_selected)
    {
        if (g_current_backgroud.menu_icon_active)
        {
            ...
            d2_renderbox( gp_d2_handle, (d2_point)g_hb_menu.top_left.x << 4, (d2_point)g_hb_menu.top_left.y << 4,
                (d2_point)(g_hb_menu.bottom_right.x - g_hb_menu.top_left.x) << 4,
                (d2_point)(g_hb_menu.bottom_right.y - g_hb_menu.top_left.y) << 4);
        }

        if (g_current_backgroud.exit_icon_active)
        {
            d2_setcolor(gp_d2_handle, 0, 0x0000FF00);
            d2_renderbox( gp_d2_handle, (d2_point)g_hb_cancel.top_left.x << 4, (d2_point)g_hb_cancel.top_left.y << 4,
                    (d2_point)(g_hb_cancel.bottom_right.x - g_hb_cancel.top_left.x) << 4,
                    (d2_point)(g_hb_cancel.bottom_right.y - g_hb_cancel.top_left.y) << 4);
        }
    }
    else
    {
        d2_setcolor(gp_d2_handle, 0, 0x0000FF00);
        d2_renderbox( gp_d2_handle, (d2_point)g_menu_cancel.top_left.x << 4, (d2_point)g_menu_cancel.top_left.y << 4,
                (d2_point)(g_menu_cancel.bottom_right.x - g_menu_cancel.top_left.x) << 4,
                (d2_point)(g_menu_cancel.bottom_right.y - g_menu_cancel.top_left.y) << 4);

        d2_setcolor(gp_d2_handle, 0, 0x00A02020);
        d2_renderbox( gp_d2_handle, (d2_point)g_kis_demo.top_left.x << 4, (d2_point)g_kis_demo.top_left.y << 4,
                (d2_point)(g_kis_demo.bottom_right.x - g_kis_demo.top_left.x) << 4,
                (d2_point)(g_kis_demo.bottom_right.y - g_kis_demo.top_left.y) << 4);

        d2_setcolor(gp_d2_handle, 0, 0x00B04040);
        d2_renderbox( gp_d2_handle, (d2_point)g_gs_demo.top_left.x << 4, (d2_point)g_gs_demo.top_left.y << 4,
                (d2_point)(g_gs_demo.bottom_right.x - g_gs_demo.top_left.x) << 4,
                (d2_point)(g_gs_demo.bottom_right.y - g_gs_demo.top_left.y) << 4);

        d2_setcolor(gp_d2_handle, 0, 0x00C06060);
        d2_renderbox( gp_d2_handle, (d2_point)g_led_demo.top_left.x << 4, (d2_point)g_led_demo.top_left.y << 4,
                (d2_point)(g_led_demo.bottom_right.x - g_led_demo.top_left.x) << 4,
                (d2_point)(g_led_demo.bottom_right.y - g_led_demo.top_left.y) << 4);

        d2_setcolor(gp_d2_handle, 0, 0x00D08080);
        d2_renderbox( gp_d2_handle, (d2_point)g_lcd_demo.top_left.x << 4, (d2_point)g_lcd_demo.top_left.y << 4,
                (d2_point)(g_lcd_demo.bottom_right.x - g_lcd_demo.top_left.x) << 4,
                (d2_point)(g_lcd_demo.bottom_right.y - g_lcd_demo.top_left.y) << 4);

        d2_setcolor(gp_d2_handle, 0, 0x00E0A0A0);
        d2_renderbox( gp_d2_handle, (d2_point)g_ns_demo.top_left.x << 4, (d2_point)g_ns_demo.top_left.y << 4,
                (d2_point)(g_ns_demo.bottom_right.x - g_ns_demo.top_left.x) << 4,
                (d2_point)(g_ns_demo.bottom_right.y - g_ns_demo.top_left.y) << 4);
    }
}

/**********************************************************************************************************************
 * Function Name: process_menu_detection
 * Description  : Determines which hitbox (activation zone defined by the st_bounding_box_t structure) conatins
 *                supplied touch point.
...
int8_t process_menu_detection (lv_indev_data_t *data, bool_t enable_tp_read, int8_t active_menu_selection)
{
    ...
    if (1 == data->number_of_coordinates)
    {
        ...
        if (!g_overlay_selected)
        {
            ...
            if ((local.x > g_hb_menu.top_left.x) && (local.x < g_hb_menu.bottom_right.x) &&
               (local.y > g_hb_menu.top_left.y) && (local.y < g_hb_menu.bottom_right.y))
            {
            ...
            if (g_current_backgroud.exit_icon_active)
            {
                if ((local.x > g_hb_cancel.top_left.x) && (local.x < g_hb_cancel.bottom_right.x) &&
                   (local.y > g_hb_cancel.top_left.y) && (local.y < g_hb_cancel.bottom_right.y))
                {
        ...
        {
            /* menu_cancel */
            if ((local.x > g_menu_cancel.top_left.x) && (local.x < g_menu_cancel.bottom_right.x) &&
               (local.y > g_menu_cancel.top_left.y) && (local.y < g_menu_cancel.bottom_right.y))
            {
                g_overlay_selected = false;
                selection = 'c';
                g_next_menu_selection = -1;
            ...
            if ((local.x > g_kis_demo.top_left.x) && (local.x < g_kis_demo.bottom_right.x) &&
               (local.y > g_kis_demo.top_left.y) && (local.y < g_kis_demo.bottom_right.y))
            {
                g_overlay_selected = false;
                selection = '1';
            ...
            if ((local.x > g_gs_demo.top_left.x) && (local.x < g_gs_demo.bottom_right.x) &&
               (local.y > g_gs_demo.top_left.y) && (local.y < g_gs_demo.bottom_right.y))
            {
                g_overlay_selected = false;
            ...
            if ((local.x > g_led_demo.top_left.x) && (local.x < g_led_demo.bottom_right.x) &&
               (local.y > g_led_demo.top_left.y) && (local.y < g_led_demo.bottom_right.y))
            {
                g_overlay_selected = false;
                selection = '2';
            ...
            if ((local.x > g_lcd_demo.top_left.x) && (local.x < g_lcd_demo.bottom_right.x) &&
               (local.y > g_lcd_demo.top_left.y) && (local.y < g_lcd_demo.bottom_right.y))
            {
                g_overlay_selected = false;
                selection = '4';
            ...
            if ((local.x > g_ns_demo.top_left.x) && (local.x < g_ns_demo.bottom_right.x) &&
               (local.y > g_ns_demo.top_left.y) && (local.y < g_ns_demo.bottom_right.y))
            {
                g_overlay_selected = false;
                selection = '5';
...
void tp_thread_entry(void *pvParameters)
{
    ...
    g_s_output_enabled = false;
    g_s_ts_enabled = false;
    ...
    if (R_BYTE_UNINITALIZED != tp_detected)
    {
    ...
    memset((void *)&g_s_tp_data, 0, sizeof(g_s_tp_data));
    memset((void *)&g_tp_copy_data, 0, sizeof(g_tp_copy_data));
...
static void touchpad_init(void)
{
    ...
    R_ICU_ExternalIrqEnable(&g_external_irq29_ctrl);
}
/**********************************************************************************************************************
...
static void touchpad_get_xy(touch_event_t * touch_event, lv_indev_data_t *data)
{
    ...
    err = rd_sensor_reg8_8 (TD_STATUS, &tp_detected);
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        if (tp_detected > 0)
        {
            ...
            {
                rd_sensor_reg8_8 ((uint8_t) (TOUCHN_XL + 6 * (i)), &reg_val_x_lsb);
                rd_sensor_reg8_8 ((uint8_t) (TOUCHN_YL + 6 * (i)), &reg_val_y_lsb);
                rd_sensor_reg8_8 ((uint8_t) (TOUCHN_XH + 6 * (i)), &reg_val_x_msb);
                rd_sensor_reg8_8 ((uint8_t) (TOUCHN_YH + 6 * (i)), &reg_val_y_msb);
...
/**********************************************************************************************************************
 * Function Name: r_update_touchpad
 * Description  : Update touchpad.
 * Return Value : .
 *********************************************************************************************************************/
void r_update_touchpad(void)
{
    touchpad_read(&g_s_tp_data);
}
