...
#define OV5640_TEST_IMAGE_BAR_STYLE   (1) /* 0: 8-Bar, 1: Gradual Vertical
                                        Mode 1, 2: Gradual Horizontal,
                                             3: Gradual vertical Mode 2 */
...
volatile uint32_t g_xw = 0; /* IMAGE_WIDTH; */
volatile uint32_t g_xh = 0; /* IMAGE_HEIGHT;*/

static uint8_t ov5640_init(void);
static void    OV5640_EnableMIPIMode(void);
static void    ov5640_set_mipi_virtual_channel(uint32_t vchannel);
static void    OV5640_DspMIPIModeSettings(void);
...
/**********************************************************************************************************************
 * Function Name: image_rgb565_to_rgb888
 * Description  : Crop square, downsample and convert 640x480 RGB565 to 224x224 RGB888 image.
 * Arguments    : p_input_image_buff
 *              : p_output_image_buff
 *              : in_width
 *              : in_height
 *              : out_width
...
extern int8_t g_model_buffer_int8[];
...
void camera_thread_entry(void *pvParameters)
{
    ...
    R_BSP_SoftwareDelay((uint32_t)250, BSP_DELAY_UNITS_MILLISECONDS);
    ...
    while (1)
    {
        ...
        if (CAPTURE_STATE_IN_PROGRESS == capture_status.state)
        {
            if(LCD_FULL_BG_CAMERA_BACKGROUND == g_current_backgroud.active_demo)
            {
                image_rgb565_to_rgb888(vin_image_buffer_1, &g_model_buffer_int8[0],
                                             (uint16_t)IMAGE_WIDTH, (uint16_t)IMAGE_HEIGHT,
                                             (uint16_t)AI_INPUT_IMAGE_WIDTH, (uint16_t)AI_INPUT_IMAGE_HEIGHT);
...
uint8_t ov5640_init(void)
{
    ...
    ov5640_hw_reset();
    ov5640_sw_reset();              
    DELAY_MS(40); // Register access permitted after 20 ms
    ...
    if ((0x40 == reg_val) || (0x41 == reg_val) || (0x4C == reg_val))
    {
        /* Valid Camera */
        ;
    }
...
#if USE_768x450_CAMERA_WINDOW

    const float gp_sensor_max_x = 2642;
    const float gp_sensor_max_y = 1952;
    const float default_ratio = gp_sensor_max_x / gp_sensor_max_y;
...
#define CROP_SENSOR_X ((default_ratio) * CROP_SENSOR_Y)
    uint16_t sensor_cropped_x = (uint16_t)((gp_sensor_max_y-CROP_SENSOR_X) * (x_y_ratio / default_ratio));
    uint16_t sensor_cropped_y = (uint16_t)((gp_sensor_max_x-CROP_SENSOR_Y) / x_y_ratio);

    uint16_t cropped_pixels_x = (uint16_t)gp_sensor_max_x - sensor_cropped_x;
    uint16_t cropped_pixels_y = (uint16_t)gp_sensor_max_y - sensor_cropped_y;
    uint16_t sensor_x_start = cropped_pixels_x / 2;
    uint16_t sensor_x_end = (uint16_t)(gp_sensor_max_x - (cropped_pixels_x / 2) - 1); // Setting is one-indexed
    uint16_t sensor_y_start = 4 + cropped_pixels_y / 2;
    uint16_t sensor_y_end = (uint16_t)(gp_sensor_max_y - (cropped_pixels_y / 2) - 1); // Setting is one-indexed

#else // USE_640x480_CAMERA_WINDOW

    const float gp_sensor_max_x = 2592;
    const float gp_sensor_max_y = 1944;

    uint16_t sensor_cropped_x = (uint16_t)(2*IMAGE_WIDTH);
    uint16_t sensor_cropped_y = (uint16_t)(2*IMAGE_HEIGHT);
    sensor_cropped_x = (uint16_t)(sensor_cropped_x < gp_sensor_max_x ? gp_sensor_max_x: sensor_cropped_x);
    sensor_cropped_y = (uint16_t)(sensor_cropped_y < gp_sensor_max_y ? gp_sensor_max_y: sensor_cropped_y);
...
static uint32_t ov5640_configure_clocks()
{
    ...
    uint8_t scaling_enabled = ((ov5640_read_reg(0x5001) >> 5) & 0x01);
                ...
                for(s_sys_clock_div = 14; s_sys_clock_div > 0; s_sys_clock_div--)
                {
                    for(s_mipi_clock_div = 14; s_mipi_clock_div > 0; s_mipi_clock_div--)
                    {
                        ...
                        for(pll_pre_div = 1; pll_pre_div<=8; pll_pre_div += pll_div_inc)
                        {
                            ...
                            {
                                uint32_t pre_div_frequency = (uint32_t)(s_pll_multiplier *
                                        (MIPI_XCLK_HZ / (pll_pre_div * s_pll_root_div)));
                                ...
                                uint32_t clock_error = (uint32_t)abs((int)OV5640_CLOCK_HZ_TARGET -
                                                                     (int)post_div_frequency);
                                ...
                                uint32_t sys_pll = (uint32_t)(s_pll_multiplier * (MIPI_XCLK_HZ /
                                        (pll_pre_div * s_sys_clock_div)));
    ...
    volatile uint8_t mipi_bit_div = (0x0F & ov5640_read_reg(0x3034)) / 5;
    // SDE on, scale on, UV average off, CMX on, AWB on
    volatile uint8_t scaling_enabled = ((ov5640_read_reg(0x5001) >> 5) & 0x01);
    volatile uint8_t clock_settings[8]; // Temp location for 'best match' --- In same order as above

    clock_settings[0] = s_pll_multiplier;
    clock_settings[1] = s_pll_root_div;
    clock_settings[2] = pll_pre_div;
    ...
    uint8_t reg_3037_value = (2 == s_pll_root_div) << 4 | ((uint8_t) pll_pre_div) << 0;
