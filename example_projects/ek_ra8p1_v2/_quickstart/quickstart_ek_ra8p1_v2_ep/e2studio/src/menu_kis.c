...
#define SUB_OPTIONS_EEPROM "\r\n\x1b[2m\x1b[37m a) Kit name:                           %s "                            \
                        "\r\n\x1b[2m\x1b[37m b) Kit ordering part number:           %s "                               \
                        "\r\n\x1b[2m\x1b[37m c) RA Device part number:              %s"                                \
                        "\r\n\x1b[2m\x1b[37m d) RA Board Serial Number:             \x1b[32m%s \x1b[37m"   \
                        "\r\n\x1b[2m\x1b[37m e) RA Board MAC Address:               \x1b[32m%s \x1b[37m"   \
                        "\r\n\x1b[2m\x1b[37m f) RA MCU 128-bit Unique ID (hex):     "                                  \
                        "\x1b[32m%08x\x1b[37m-\x1b[32m%08x\x1b[37m-\x1b[32m%08x\x1b[37m-\x1b[32m%08x\x1b[37m"          \
                        "\r\n\x1b[2m\x1b[37m g) RA MCU Die temperature (F/C):       \x1b[32m%d.%02d/%d.%02d\x1b[37m"   \
                        "\r\n\x1b[2m\x1b[37m h) Blue LED blinking frequency (Hz):   \x1b[32m%d  \x1b[37m"              \
                        "\r\n\x1b[2m\x1b[37m i) Blue LED blinking intensity (%%):    \x1b[32m%d  \x1b[37m"             \
...
st_rgb565_text_block_page_t g_kis_page = {
	21, // Number of lines required (including blanks)
	" KIT Information", /* Title */
	{
       ...
       {{PAGE_1ST_TAB,   LINE(4)},""},
       {{PAGE_LEFT_EDGE, LINE(5)},""},
       {{PAGE_1ST_TAB,   LINE(5)},""},

       {{PAGE_LEFT_EDGE, LINE(6)},""},
       {{PAGE_1ST_TAB,   LINE(6)},""},

       {{PAGE_LEFT_EDGE, LINE(7)},""},
       {{PAGE_1ST_TAB,   LINE(7)},""},
       {{PAGE_LEFT_EDGE, LINE(8)},""},
       {{PAGE_1ST_TAB,   LINE(8)},""},
       {{PAGE_LEFT_EDGE, LINE(9)},""},
       {{PAGE_1ST_TAB,   LINE(9)},""},
       {{PAGE_LEFT_EDGE, LINE(10)},""},
       {{PAGE_1ST_TAB,   LINE(10)},""},
	}
};
...
test_fn kis_display_menu(void)
{
    ...
    uint16_t page_ndx = 6;
    if (true == g_eeprom_data.populated)
    {
        /* update EEPROM INFO  (if connected) */
        sprintf(g_kis_page.text_block[1].line, "%s", g_product_full_name);

        sprintf(g_kis_page.text_block[page_ndx++].line,"%s","RA Board Serial Number");
        sprintf(g_kis_page.text_block[page_ndx++].line,"%s",g_eeprom_data.p_serial_str);

        sprintf(g_kis_page.text_block[page_ndx++].line,"%s","RA Board MAC Address");
        sprintf(g_kis_page.text_block[page_ndx++].line,"%s", g_eeprom_data.mac_t.p_mac_str);

        sprintf(g_kis_page.text_block[page_ndx++].line,"RA MCU 128-bit Unique ID (Hex)");
        sprintf(g_kis_page.text_block[page_ndx++].line,"%s",mod_uuid);

        page_ndx +=2; // Whole blank line

        sprintf(g_kis_page.text_block[page_ndx++].line, "RA MCU Die temperature (F/C)");
        sprintf(g_kis_page.text_block[page_ndx++].line, "%d.%02d\'F/%d.%02d\'C",
                wn_mcu_temp_f, fr_mcu_temp_f, wn_mcu_temp_c, fr_mcu_temp_c);

        sprintf(g_kis_page.text_block[page_ndx++].line, "Blue LED blinking frequency (Hz)");
        sprintf(g_kis_page.text_block[page_ndx++].line, "%d", g_pwm_rates_data[g_board_status.led_frequency]);

        sprintf(g_kis_page.text_block[page_ndx++].line, "Blue LED blinking intensity (%%)");
        sprintf(g_kis_page.text_block[page_ndx++].line, "%d", g_pwm_dcs_data[g_board_status.led_intensity]);
    }
    else
    {
        sprintf(g_kis_page.text_block[1].line, g_product_full_name);
    ...
    while (CONNECTION_ABORT_CRTL != c)
    {
        ...
        if (true == g_eeprom_data.populated)
        {
            sprintf (s_print_buffer, SUB_OPTIONS_EEPROM, g_product_full_name,
                     PART_NUMBER, DEVICE_NUMBER, g_eeprom_data.p_serial_str,
                    g_eeprom_data.mac_t.p_mac_str,
        ...
        {
            sprintf (s_print_buffer, SUB_OPTIONS, g_product_full_name, PART_NUMBER, DEVICE_NUMBER,
                    (int_t)p_uid->unique_id_words[0],  (int_t)p_uid->unique_id_words[1], // typedef uint_t in stdint.h
                    (int_t)p_uid->unique_id_words[2],  (int_t)p_uid->unique_id_words[3], // as signed long long int64_t;
                    wn_mcu_temp_f, fr_mcu_temp_f, wn_mcu_temp_c, fr_mcu_temp_c,
                    g_pwm_rates_data[g_board_status.led_frequency], g_pwm_dcs_data[g_board_status.led_intensity]);
        ...
        if (key_pressed () || ((LCD_FULL_BG_POPUP_MENU != on_screen_selection) && (-1 != on_screen_selection)))
        {
