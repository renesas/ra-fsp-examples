...
/*                        "\r\n a) %s resources:    renesas.com/ek-ra8p1 " \ */
...
st_rgb565_text_block_page_t g_ns_page =
{
    ...
    " NEXT STEPS", /* Title */
	{
	   ...
	   {{PAGE_LEFT_EDGE, LINE(5)}, " "},      {{PAGE_1ST_TAB,   LINE(5)}, "renesas.com/ek-ra8p1"},
...
test_fn ns_display_menu(void)
{
    ...
    char_t tmp[DISPLY_PRODUCT_NAME_LENGTH_CFG] = "";
    char_t tmp2[] = "renesas.com/ek-ra8p1 ";
    ...
    sprintf (s_print_buffer, SUB_OPTIONS_M, g_product_full_name);

    sprintf(tmp, "renesas.com/ek-ra8p1");

    memcpy(&s_print_buffer[32], &tmp[0], 32);

    print_to_console(s_print_buffer);

    sprintf (s_print_buffer, SUB_OPTIONS_C);
    print_to_console(s_print_buffer);

    /* Ensure string ends*/
    tmp[22] = '\0';

    sprintf(tmp, "%s", g_product_full_name);

    sprintf(g_ns_page.text_block[4].line,"%s",  tmp);
    ...
    while ((CONNECTION_ABORT_CRTL != c))
    {
        ...
        {
            if ((!g_overlay_selected) && (-1 != on_screen_selection) && ('5' != on_screen_selection))
            {
