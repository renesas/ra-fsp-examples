...
#define GREEN_LED_PORT          (BSP_IO_PORT_01_PIN_11)
...
static int8_t map_tp_to_hitbox(uint32_t x_hp, uint32_t y_hp)
{
    ...
    if ((x_hp > g_led_screen_details.red.blinking_box.top_left.x) &&
        (x_hp < g_led_screen_details.red.blinking_box.bottom_right.x) &&
        (y_hp > g_led_screen_details.red.blinking_box.top_left.y) &&
        (y_hp < g_led_screen_details.red.blinking_box.bottom_right.y))
    {
        selected = RED_BLINK_RATE_SELECTED;
    }

    if ((x_hp > g_led_screen_details.green.blinking_box.top_left.x) &&
        (x_hp < g_led_screen_details.green.blinking_box.bottom_right.x) &&
        (y_hp > g_led_screen_details.green.blinking_box.top_left.y) &&
        (y_hp < g_led_screen_details.green.blinking_box.bottom_right.y))
    {
        selected = GREEN_BLINK_RATE_SELECTED;
    }

    if ((x_hp > g_led_screen_details.blue.blinking_box.top_left.x) &&
        (x_hp < g_led_screen_details.blue.blinking_box.bottom_right.x) &&
        (y_hp > g_led_screen_details.blue.blinking_box.top_left.y) &&
        (y_hp < g_led_screen_details.blue.blinking_box.bottom_right.y))
    {
        selected = BLUE_BLINK_RATE_SELECTED;
    }

    if ((x_hp > g_led_screen_details.red.brightness_box.top_left.x) &&
        (x_hp < g_led_screen_details.red.brightness_box.bottom_right.x) &&
        (y_hp > g_led_screen_details.red.brightness_box.top_left.y) &&
        (y_hp < g_led_screen_details.red.brightness_box.bottom_right.y))
    {
        selected = RED_BRIGHNESS_SELECTED;
    }

    if ((x_hp > g_led_screen_details.green.brightness_box.top_left.x) &&
        (x_hp < g_led_screen_details.green.brightness_box.bottom_right.x) &&
        (y_hp > g_led_screen_details.green.brightness_box.top_left.y) &&
        (y_hp < g_led_screen_details.green.brightness_box.bottom_right.y))
    {
        selected = GREEN_BRIGHNESS_SELECTED;
    }

    if ((x_hp > g_led_screen_details.blue.brightness_box.top_left.x) &&
        (x_hp < g_led_screen_details.blue.brightness_box.bottom_right.x) &&
        (y_hp > g_led_screen_details.blue.brightness_box.top_left.y) &&
        (y_hp < g_led_screen_details.blue.brightness_box.bottom_right.y))
    {
...
/**********************************************************************************************************************
 * Function Name: angle_to_frequency
 * Description  : Changing the interface angle from circle to frequency.
 * Argument     : angle
 * Return Value : .
 *********************************************************************************************************************/
...
test_fn led_display_menu(void)
{
    ...
    while (CONNECTION_ABORT_CRTL != c)
    {
        ...
        if (key_pressed () || (-1 != g_next_menu_selection)
                ||
                ((LCD_FULL_BG_POPUP_MENU != on_screen_selection) && (-1 != on_screen_selection)))
        {
            if ((!g_overlay_selected) && (-1 != on_screen_selection) && ('2' != on_screen_selection))
            {
        ...
        if (0 == data.number_of_coordinates)
        {
 ...
 *********************************************************************************************************************/
 static void close_led_timers(void)
{
    if (0 != ((gpt_instance_ctrl_t*) led_pulse_timer.p_ctrl)->open)
    {
        R_GPT_Close(led_pulse_timer.p_ctrl);
    }

    if (0 != ((gpt_instance_ctrl_t*) led_brightness_timer.p_ctrl)->open)
    {
...
void configure_led(e_led_configuration_t cfg)
{
    switch (cfg)
    {
        case LED_CONFIGURED_NORMAL:
            ...
            R_IOPORT_PinCfg(g_ioport.p_ctrl, BLUE_LED_PORT, (uint32_t) IOPORT_CFG_DRIVE_HIGH |
        ...
        case LED_CONFIGURED_DEMO:
            ...
            R_IOPORT_PinCfg(g_ioport.p_ctrl, BSP_IO_PORT_06_PIN_00, (uint32_t) IOPORT_CFG_DRIVE_HIGH |
 ...
 * Argument       gp_d2_handle - application pointer to address of a device structure
...
static void map_angle_to_screen(d2_device *handle, uint32_t angle_i, lv_point_t *origin_tp)
{
    ...
    d2_renderline(handle, (d2_point) ((origin_tp->x) << 4), (d2_point) ((origin_tp->y) << 4),
                 (d2_point) ((destination_tp.x) << 4), (d2_point) ((destination_tp.y) << 4),
                 (d2_point) (LED_LINE_THICKNESS << 4), 0);
