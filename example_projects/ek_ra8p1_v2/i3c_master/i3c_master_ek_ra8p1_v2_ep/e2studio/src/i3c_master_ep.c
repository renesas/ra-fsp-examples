...
/* Timer related timeout function */
...
void i3c_master_entry(void)
{
    ...
    /* Initialize the AGT timer module */
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        /* De-initialize the opened I3C and AGT timer modules */
        i3c_deinit();
        agt_deinit();
        APP_ERR_TRAP(err);
    }

    /* Set the I3C device information through device table entries */
    ...
    /* Wait for the bus initialization */
            ...
            switch (converted_rtt_input)
            {
                ...
                case MASTER_WRITE_READ_HDR:
                {
                    if (I3C_HDR_DDR_MODE == g_device_slave_info.slave_info.bcr_b.hdr_capable)
                    {
                        ...
                        if (FSP_SUCCESS != err)
                        {
                            APP_ERR_PRINT("\r\nERROR: master_write_read_hdr_verify function failed.\r\n");
                            /* De-initialize the opened I3C and AGT timer modules */
                            i3c_deinit();
                            agt_deinit();
                            APP_ERR_TRAP(err);
...
static fsp_err_t hot_join_request_process(void)
{
    ...
    APP_PRINT("\r\nINFO: DAA using CCC transmission completed, \r\nplease check by pressing user "
              "input 1 (available at menu option) for slave information\r\n");
    return FSP_SUCCESS;
}
/***********************************************************************************************************************
 ...
 *  Description  : This function sends a broadcast or directs command to slave devices on the bus.
 *  Arguments    : None.
 *  Return Value : FSP_SUCCESS     Upon successful operation.
 *                 Any other error code apart from FSP_SUCCESS Unsuccessful operation.
 **********************************************************************************************************************/
 ...
 *  Description  : This function starts the timer and waits for the event set in the i3c callback till
 *                 specified timeout.
...
uint32_t i3c_app_event_notify(uint32_t set_event_flag_value, uint32_t timeout)
{
    ...
    while (!b_process_timeout)
    {
        ...
        /* Check for event received from i3c callback function is similar to event which the user wants */
 ...
 *  Description  : This function checks the slave device is present on bus and displays slave device information.
 *  Arguments    : None.
 *  Return Value : FSP_SUCCESS     Upon successful operation.
 *                 Any other error code apart from FSP_SUCCESS Unsuccessful operation.
 **********************************************************************************************************************/
 ...
 *  Description  : This function performs the I3C master write and read operations.
 *  Arguments    : None.
 *  Return Value : FSP_SUCCESS     Upon successful operation.
 *                 Any other error code apart from FSP_SUCCESS Unsuccessful operation.
 **********************************************************************************************************************/
 ...
 *  Description  : This function resets the counter value and starts the AGT timer.
 *  Arguments    : timeout_ms      Specific timeout in millisecond.
 *  Return Value : FSP_SUCCESS     Upon successful operation.
 *                 Any other error code apart from FSP_SUCCESS Unsuccessful operation.
 **********************************************************************************************************************/
static fsp_err_t start_timeout_timer_with_defined_ms(uint32_t timeout_ms)
