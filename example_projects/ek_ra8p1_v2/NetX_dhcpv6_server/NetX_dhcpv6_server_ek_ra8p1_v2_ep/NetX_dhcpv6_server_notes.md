...
### Hardware ###
* Supported RA boards: EK-RA6M3, EK-RA6M4, EK-RA6M5, EK-RA8D1, EK-RA8M1, MCK-RA8T1, EK-RA8P1, EK-RA8D2, EK-RA8M2, MCK-RA8T2, EK-RA8T2, EK-RA8P1 V2.
    * 1 x Renesas RA board.
    * 1 x USB cable for programming and debugging (USB cable type varies by board model).
    * 1 × PC supported ethernet or another RA board running DHCPv6 Client.
...
* For EK-RA6M3, EK-RA6M4, EK-RA6M5, EK-RA8M1, EK-RA8D1:
	* Without ethernet switch:
		* Connect the ethernet port of the RA board to the ethernet port of the PC/another RA board running DHCPv6 Client using a LAN cable.
	* With ethernet switch: (Optional)
        * Connect the ethernet port of the RA board and the ethernet port of the PC/another RA board running DHCPv6 Client to an ethernet switch using two LAN cables.
    * For EK-RA8D1:
...
* For EK-RA8P1, EK-RA8D2, EK-RA8M2, EK-RA8P1 V2:
    ...
    * For EK-RA8P1 V2: The user must set the configuration switches (SW6) as below to use the on-board debug functionality.
...
* For MCK-RA8T2:
    * Without ethernet switch:
        * Connect the ethernet port 0 (CN14) of the RA board to the ethernet port of the PC/another RA board running DHCPv6 Client using a LAN cable.
    * With ethernet switch: (Optional)
        * Connect the ethernet port 0 (CN14) of the RA board and the ethernet port of the PC/another RA board running DHCPv6 Client to an ethernet switch using two LAN cables.

* For EK-RA8T2:
    * Without ethernet switch:
		* Connect the ethernet port 0 (J15) of the RA board to the ethernet port of the PC/another RA board running DHCPv6 Client using a LAN cable.
	* With ethernet switch: (Optional)
        * Connect the ethernet port 0 (J15) of the RA board and the ethernet port of the PC/another RA board running DHCPv6 Client to an ethernet switch using two LAN cables.
...
| configuration.xml > Stacks > Threads > DHCPv6 Thread > DHCPv6 Thread Stacks > g_layer3_switch0 Switch (r_layer3_switch) > Properties > Settings > Property > Module g_layer3_switch0 Switch (r_layer3_switch) > Port > Port 1 > MAC address | 00:11:22:33:44:55 | 00:11:22:33:44:66 | Sets the MAC address for Port 1. |
...
**Note:** When testing with a PC, the IPv6 address may not be leased, as shown in the RTT output below.

![dhcpv6_server_rtt_log_ip_not_leased](images/dhcpv6_server_rtt_log_ip_not_leased.jpg "DHCPv6 Server RTT IP Not Leased Log")

This behavior is expected due to cached IPv6 information on the client side when the PC already has an IPv6 address within the DHCPv6 Server EP lease range (`2001:EE08:0:0:0:0:110` to `2001:EE08:0:0:0:0:150`). For clean verification, ensure that all IPv6 addresses within this range are released on the PC before starting the test.

The image below shows an example of the PC already holding an IPv6 address within the DHCPv6 Server EP lease range.

![dhcpv6_server_cmd](images/dhcpv6_server_cmd.jpg "PC IPv6 Address Already Assigned")
