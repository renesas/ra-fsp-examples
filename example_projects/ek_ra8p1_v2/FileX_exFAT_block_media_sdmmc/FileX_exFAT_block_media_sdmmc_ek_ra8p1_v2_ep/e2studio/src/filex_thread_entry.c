...
/*******************************************************************************************************************//**
 * @brief       This function initializes FileX and configures system date/time.
 * @param[in]   None.
...
static UINT file_system_init(void)
{
    ...
    system_time.date = (UINT) atoi((const char *)(__DATE__) + DATE_INDEX);
    system_time.year = (UINT) atoi((const char *)(__DATE__) + YEAR_INDEX);

    /* Extract current Time from compiler MACROS */
    system_time.hour = (UINT) atoi((const char *)(__TIME__) + HOUR_INDEX);
    system_time.min  = (UINT) atoi((const char *)(__TIME__) + MIN_INDEX);
    system_time.sec  = (UINT) atoi((const char *)(__TIME__) + SEC_INDEX);
...
/*******************************************************************************************************************//**
 * @brief       This function performs file system operations based on the request type.
 * @param[in]   request         Operation request.
 * @retval      FX_SUCCESS      Upon successful operation.
 * @retval      Any other error code apart from FX_SUCCESS.
 **********************************************************************************************************************/
static UINT file_system_operation(ULONG request)
