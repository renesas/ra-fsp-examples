...
1. Project Overview:
	...
	RA MCU. The FileX file system with exFAT format enabled, the project will perform various file system
	operations based on the user's selection from the menu options, such as media management (open, get property,
	format, close), directory management (create, get property, delete), and file management (create, write, read,
	delete). The J-Link RTT Viewer is used as the user interface. Menu options and system messages (errors and
	information messages) will be printed on J-Link RTT Viewer during the execution of the project.

2. Hardware Requirements:
	Supported RA boards: EK-RA4M2, EK-RA4M3, EK-RA6M1, EK-RA6M2, EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA6M5,
			     FPB-RA6E1, EK-RA8M1, EK-RA8D1, EK-RA8P1, MCK-RA8T2, EK-RA8M2, EK-RA8D2, EK-RA8P1 V2.
 	1 x Renesas RA board.
	1 x USB cable for programming and debugging (USB cable type varies by board model).
	For EK-RA4M2, EK-RA4M3, EK-RA6M1, EK-RA6M2, EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA6M5, FPB-RA6E1, EK-RA8M1,
	    EK-RA8D1, EK-RA8P1, EK-RA8M2, EK-RA8D2, EK-RA8P1 V2:
...
4. Hardware Connections:
	...
	For EK-RA6M2: (SDHI 4 bits width by default)
		...
		4 Bits Width: Trace cut Jumper E15 and solder to E12. Above mentioned connections for 1 Bit Width.
		In addition below connections:
	...
	For EK-RA8D1: The user must turn-off SW1-3, SW1-5, SW1-8 on board to use SDHI channel 1
	              (SDHI 4 bits width by default).
		1 Bit Width: 
	...
	For EK-RA8M2: The user must place jumper J6 on pins 2-3, J8 on pins 1-2, J9 on pins 2-3, and J29 on pins 1-2,
	              3-4, 5-6, 7-8 to use the on-board debug functionality (SDHI 4 bits width by default).
		1 Bit Width:
	...
	For EK-RA8D2: The user must turn-off SW4-5, SW4-6 on board to use SDHI channel 1 (SDHI 4 bits width by default).
		1 Bit Width:
			SD1CMD  P401 (J17:5)    --->   Pmod SD MOSI (J1:2)
			SD1DAT0 P402 (J17:14)   --->   Pmod SD MISO (J1:3)
			SD1CLK  P400 (J17:6)    --->   Pmod SD SCK  (J1:4)
	...
	For EK-RA8P1 V2:
		...
		The user must open E9, E17 to use PD05, PD04 for SDHI channel 0 (SDHI 4 bits width by default).
		1 Bit Width:
...
5. Verifying Operation:
	...
	b) Connect the RA board USB debug port to the host PC using the appropriate USB cable for EP
	...
	g) After the main menu is displayed on the RTT Viewer, the user can select options to perform file system
	   operations as needed. Note that the media must be opened before performing any other operations.

Note:
1) SD/MicroSD card must be in exFAT format before verifying the EP.

2) In case the SD/MicroSD card is not in exFAT format, the user can format the SD/MicroSD card in exFAT format using this EP
   using the following procedure: Open Media -> Format Media -> Close Media -> Open Media.
