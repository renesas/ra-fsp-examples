...
static void sd_safely_eject(void);                          /* Perform SD/MicroSD card safely disconnect operation */
static void update_buffer(void);                            /* Update write buffer with data and clear read buffer */
static fsp_err_t sd_init(void);                             /* Initialize SD/MicroSD card device */
...
void sdhi_thread_entry(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        {
            APP_ERR_PRINT("\r\n\r\n"CARD_TYPE" Card disconnected without eject option.\r\n");
            APP_ERR_PRINT("\r\nConnect the "CARD_TYPE" Card and execute Safely Eject option to make sure"\
                          " file operations works correctly\r\n");
            /* Wait until SD/MicroSD Card Device is connected */
...
/*******************************************************************************************************************//**
 * @brief       This function initiates the SD/MicroSD Card operation by calling respective functions.
...
/*******************************************************************************************************************//**
 * @brief       This function performs SD/MicroSD Card write operation.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void sd_write_operation(void)
{
    ...
    /* Double check the connection again to ensure the SD/MicroSD Card is still mounted */
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        if (NULL != file_pointer)
        {
            ...
            APP_PRINT("\r\n"CARD_TYPE" Card write operation will be initiated\r\n");
    ...
    {
        APP_ERR_PRINT(""CARD_TYPE" Card Device disconnected or not initialized after eject command\r\n");
    }
}

/*******************************************************************************************************************//**
 * @brief       This function performs SD/MicroSD Card read operation.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void sd_read_operation(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        if (NULL != file_pointer)
        {
            APP_PRINT("\r\n"CARD_TYPE" Card read operation will be initiated\r\n");
...
/*******************************************************************************************************************//**
 * @brief       This function performs SD/MicroSD Card format operation.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void format_sd_device(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        APP_PRINT("\r\n"CARD_TYPE" Card formatting will be initiated. Formatting will take time "\
                  "depending on "CARD_TYPE" Card capacity.\r\n");
        APP_PRINT("Do not disconnect the "CARD_TYPE" Card while formatting is in progress. Please wait...\r\n");

        /* Set the number of partition before formatting SD/MicroSD Card */
        ff_err = FF_Partition(&my_disk, &partition_params);
        if (FSP_SUCCESS != ff_err)
        {
            APP_ERR_PRINT("\r\nFF_Partition API failed %d. Check the "CARD_TYPE" Card device.\r\n",
                          FF_GetErrMessage(ff_err));
            APP_PRINT("%d\r\n", FF_GetErrMessage(ff_err));
        }
        /* Format time varies with SD/MicroSD Card capacity. Might take longer time for higher capacity
         * SD/MicroSD Card. */
        ff_err = FF_Format(&my_disk, my_disk.xStatus.bPartitionNumber, pdFALSE, pdFALSE);
        if (FF_ERR_NONE != ff_err)
        {
            APP_ERR_PRINT("\r\nFF_Format API failed %d. Check the "CARD_TYPE" Card.\r\n", FF_GetErrMessage(ff_err));
            APP_PRINT("%d\r\n", FF_GetErrMessage(ff_err));
        }
        else
        {
            APP_PRINT("\r\n"CARD_TYPE" Card formatted successfully\r\n");
    ...
    {
        APP_PRINT(""CARD_TYPE" Card disconnected or not initialized after eject command\r\n");
    }
}
...
{
    /* Check the SD/MicroSD Card connection before formating */
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        xEventGroupValue = xEventGroupGetBits(g_SDHI_EventGroupHandle);

        APP_PRINT("\r\n"CARD_TYPE" Card can be safely removed now\r\n");
    }
    else
    {
        /* SD/MicroSD Card disconnected */
        APP_PRINT(""CARD_TYPE" Card disconnected or not initialized after eject command\r\n");
    }
}
...
/*******************************************************************************************************************//**
 * @brief       This function is a callback for FreeRTOS+FAT and is triggered when SD/MicroSD Card is
 *              removed or inserted.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
...
static fsp_err_t sd_init(void)
{
    ...
    /* Wait for SD/MicroSD Card connection */
    APP_PRINT("Connect "CARD_TYPE" Card...\r\n");

    /* Wait until SD/MicroSD Card is connected */
    xEventGroupValue = xEventGroupWaitBits(g_SDHI_EventGroupHandle,
                                           RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_INSERTED,
                                           pdTRUE,
                                           pdFALSE,
    ...
    else
    {
        ...
        if (FF_FS_ADD_FAILURE == ff_err)
        {
            ...
            /* Close the FREERTOS_PLUS_FAT_Close instance on safely ejecting */
            fat_clean_up();
            /* Return FSP error in case of failure */
            return ((fsp_err_t )FF_ERR_DEVICE_DRIVER_FAILED);
        }
    }

    /* Set this flag to let application know that SD/MicroSD Card is initialized */
    return err;
}
