    ...
    while (true)
    {
        ...
        switch (p_usb_event->event)
        {
            case USB_STATUS_CONFIGURED:
            {
                ...
                /* Send the HID request (set protocol) to HID device */
                err = set_protocol(&g_usb_basic_ctrl, BOOT_PROTOCOL, p_usb_event->device_address);
                /* Handle error */
                if (FSP_SUCCESS != err)
                {
                    APP_ERR_PRINT("** Sending the HID request (SetProtocol) to the HID device FAILED **\r\n");
                    /* Close the opened USB driver and trap error */
                    deinit_usb();
                    APP_ERR_TRAP(err);
                }
            ...
            case USB_STATUS_READ_COMPLETE:
            {
                ...
                if (pdTRUE != err_queue)
                {
                    APP_ERR_PRINT("\r\nFailed to send USB data through the queue\r\n");
                    /* Close the opened USB driver and trap error */
                    deinit_usb();
                    APP_ERR_TRAP(err_queue);
                }
            ...
            case USB_STATUS_REQUEST_COMPLETE:
            {
                ...
                APP_PRINT("\r\nEnter any key as specified from a to z, A to Z, 0 to 9, and special characters\r\n"
                          "(e.g., !, @, #, $, %, ^, &, *, (, )) on keyboard\r\n\n");
                break;
            }
...
/*******************************************************************************************************************//**
 * @brief     Closes the USB module using the HAL-level API. Handles any internal errors with appropriate messages.
 *            The application is responsible for any remaining cleanup.
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
