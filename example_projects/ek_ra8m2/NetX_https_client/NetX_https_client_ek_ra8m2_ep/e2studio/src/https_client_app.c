...
/*******************************************************************************************************************//**
 * @brief      This function disconnects from the HTTP Client.
 * @param[in]  None.
...
/*******************************************************************************************************************//**
 * @brief      This function reads response from the server.
 * @param[in]  None.
...
UINT readResponsefromServer(void)
{
    ...
    while (NX_WEB_HTTP_GET_DONE != body_get_status)
    {
         ...
         * Successive calls to this routine should be made until the return status of
        ...
        status = nx_packet_data_extract_offset(receive_packet, RESET_VALUE, g_receive_buffer, BUFFER_LENGTH,
                                               &bytes_copied);
        if (NX_SUCCESS != status)
        {
...
/*******************************************************************************************************************//**
 * @brief      This function sends additional header to the server.
...
/*******************************************************************************************************************//**
 * @brief      This function processes GET request and reads the response from the server.
 * @param[in]  None.
 * @retval     NX_SUCCESS                   Upon successful execution of GET request from the Adafruit Server.
 *                                          Data received from the Adafruit Server is displayed on the RTT Viewer.
...
/*******************************************************************************************************************//**
 * @brief      This function processes POST request and reads the response from the server.
 * @param[in]  adc_val                      The ADC reading for the MCU die Temperature.
 * @retval     NX_SUCCESS                   Upon successful execution of POST request to the Adafruit Server.
 *                                          The temperature data from the ADC is added to the Adafruit Server.
...
UINT processPostRequest(float adc_val)
{
    ...
    /* Buffer to store ADC data for HTTP client */
    CHAR http_request[BUFFER_SIZE] = {NULL_CHAR};
    ...
    memset(http_request, RESET_VALUE, sizeof(http_request));
    size = (UINT)snprintf(http_request, BUFFER_SIZE, "{\"value\":\"%.02f\"}", adc_val);
...
/*******************************************************************************************************************//**
 * @brief      This function processes PUT request and reads the response from the server.
 * @param[in]  adc_val                      The ADC reading for the MCU die Temperature.
 * @retval     NX_SUCCESS                   Upon successful execution of PUT request to the Adafruit Server.
 *                                          The temperature data from the ADC is added to the Adafruit Server.
...
/*******************************************************************************************************************//**
 * @brief      This function creates a callback to setup TLS parameters for secure HTTPS.
...
UINT tls_setup_callback(NX_WEB_HTTP_CLIENT *p_client_ptr, NX_SECURE_TLS_SESSION *p_tls_session)
{
    ...
    for (count = RESET_VALUE; count < NO_OF_TRUSTED_CERTS; count++)
    {
        ...
        if (NX_SUCCESS != status)
        {
            PRINT_ERR_STR("nx_secure_x509_certificate_initialize API failed.");
            ERROR_TRAP(status);
        }
