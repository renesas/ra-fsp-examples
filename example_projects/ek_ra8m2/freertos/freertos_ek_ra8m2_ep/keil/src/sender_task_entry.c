...
void sender_task_entry(void *pvParameters)
{
    ...
    /* Start GPT timer to start sending message to Message Queue periodically at 1 second from GPT callback */
            ...
            if (pdPASS == xQueueReset(g_queue))
            {
                ...
                if (FSP_SUCCESS != fsp_err)
                {
                    if (FSP_SUCCESS != R_GPT_Close(&g_periodic_timer_msgq_ctrl))
                    {
                        ...
                        APP_ERR_PRINT("\r\nSender_Task: R_GPT_Close API failed\r\n");
                    }
                    /* Print out in case of error */
                    APP_ERR_PRINT("\r\nSender_Task: R_GPT_Stop API failed\r\n");
                    APP_ERR_TRAP(fsp_err);
                }
...
/*******************************************************************************************************************//**
 * @brief      User-defined GPT IRQ callback for the Message Queue.
