...
/*******************************************************************************************************************//**
 * @brief     Initializes IIC Master and Slave drivers.
 * @param[in] None
 * @retval    FSP_SUCCESS       IIC Master and Slave drivers opened successfully.
 * @retval    err               Any error code other than FSP_SUCCESS, indicating an unsuccessful open.
 **********************************************************************************************************************/
...
/*******************************************************************************************************************//**
*  @brief      Performs I2C Slave write/read operations. Toggles the LED on success;
*              turns the LED ON to indicate failure.
*              If an I2C Slave transaction fails, the application halts by turning the LED ON
*              as a sign of failure. Failure messages are also displayed in the RTT Viewer.
*  @retval     FSP_SUCCESS    If the R_IIC_Master and Slave write/read operations complete successfully.
*  @retval     err            Any other error code apart from FSP_SUCCESS, indicating an unsuccessful write/read
*                             operations.
**********************************************************************************************************************/
fsp_err_t process_slave_WriteRead(void)
{
    ...
    switch (g_slave_RW)
    {
        case SLAVE_READ:
        {
            /* Turn OFF the LED before beginning the operation */
            set_led(LED_OFF);
            ...
            {
                /* Toggle the LED as a sign of success */
                toggle_led();
            ...
            /* Set the variable value to perform the Slave write operation */
        ...
        case SLAVE_WRITE:
        {
            ...
            /* Set the variable to perform the Slave read operation */
...
/*******************************************************************************************************************//**
 *  @brief       Performs a Slave write operation.
 *  @param[in]   None
 *  @retval      FSP_SUCCESS               All data successfully written to the Master.
 *  @retval      FSP_ERR_TRANSFER_ABORTED  Callback event failure.
 *  @retval      FSP_ERR_ABORTED           Data mismatch occurred.
 *  @retval      FSP_ERR_TIMEOUT           No callback event received within the timeout period.
 *  @retval      write_err                 Error returned by the API, if any.
**********************************************************************************************************************/
static fsp_err_t iic_slave_write(void)
{
    ...
    /* Update the Slave transmit buffer and clear the Master buffer (recipient) */
    ...
    /* Wait until the Slave write and Master read processes complete */
    while ((I2C_SLAVE_EVENT_TX_COMPLETE != g_slave_event) || (I2C_MASTER_EVENT_RX_COMPLETE != g_master_event))
    {
        ...
        {
            /* Start timeout check to avoid an infinite loop */
            --write_time_out;

            /* Check for timeout */
            if (RESET_VALUE == write_time_out)
            {
                /* Timeout reached: no I2C event occurred */
            	APP_ERR_PRINT("** No event received during Slave write operation **\r\n");
    ...
    /* Compare the Slave write buffer with the data received by the Master device */
...
/*******************************************************************************************************************//**
 *  @brief      Performs a Slave read operation.
 *  @param[in]  None
 *  @retval     FSP_SUCCESS               All data successfully received from the Master.
 *  @retval     FSP_ERR_TRANSFER_ABORTED  Callback event failure.
 *  @retval     FSP_ERR_ABORTED           Data mismatch occurred.
 *  @retval     FSP_ERR_TIMEOUT           No callback event received within the timeout period.
 *  @retval     read_err                  Error returned by the API, if any.
 **********************************************************************************************************************/
static fsp_err_t iic_slave_read(void)
{
    ...
    /* Update the Master transmit buffer and clear the Slave RX buffer (recipient) */
    ...
    /* Wait until the Slave read and Master write processes complete */
    while ((I2C_SLAVE_EVENT_RX_COMPLETE != g_slave_event) || (I2C_MASTER_EVENT_TX_COMPLETE != g_master_event))
    {
        ...
        else
        {
            ...
            {
            	/* Timeout reached: no I2C event occurred */
            	APP_ERR_PRINT("** No event received during Slave read operation **\r\n");
    ...
    /* Compare the Slave read buffer with the data written by the Master device */
...
/*******************************************************************************************************************//**
 *  @brief      Handles I2C Master callback events.
 *  @param[in]  p_args    Pointer to callback argument structure.
 *  @retval     None
 **********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 *  @brief      Handles I2C Slave callback events.
 *  @param[in]  p_args  Pointer to callback argument structure.
 *  @retval     None
 **********************************************************************************************************************/
void i2c_slave_callback(i2c_slave_callback_args_t * p_args)
{
    ...
    if (NULL != p_args)
    {
        switch (p_args->event)
        {
            ...
            {
            	/* Perform a Slave read operation */
#if (BSP_PERIPHERAL_IICA_PRESENT)
                ...
                if (FSP_SUCCESS != err)
                {
                	/* Store the API return error for later handling */
            ...
            default:
                ...
                /* Record the event in the global variable for other Slave events */
...
/*******************************************************************************************************************//**
 * @brief     Toggles the on-board LEDs that are connected and supported by the BSP.
 * @param[in] None
 * @retval    None
 **********************************************************************************************************************/
static void toggle_led(void)
{
    /* Get LED pin information for this board */
...
/*******************************************************************************************************************//**
 *  @brief       Turns the on-board LED ON or OFF.
 *  @param[in]   led_state   LED_ON or LED_OFF states.
 *  @retval      None
 **********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 * @brief     Closes the external I2C Master and Slave modules using the HAL-level API.
 *            Errors are handled internally and reported via RTT messages.
 *            The application handles any remaining tasks.
 * @param[in] None
 * @retval    None
 **********************************************************************************************************************/
void deinit_i2c_driver(void)
{
    ...
    /* Close the opened IIC Master module */
#if (BSP_PERIPHERAL_IICA_PRESENT)
    ...
    /* Close the opened IIC Slave module */
#if (BSP_PERIPHERAL_IICA_PRESENT)
