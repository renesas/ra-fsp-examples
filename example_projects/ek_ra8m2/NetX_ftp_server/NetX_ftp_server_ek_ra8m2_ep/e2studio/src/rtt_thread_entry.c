    ...
    while (true)
    {
        ...
        if (APP_CHECK_DATA)
        {
            ...
            else
            {
                ...
                char fname[IP_MSG_MAX_SIZE] = {'\0'};

                remove_newline(fname, rtt_buffer);

                p_data->msg_data_size = strlen(fname) + 1;

                memcpy(p_data->p_msg, fname, p_data->msg_data_size);
    ...
    do
    {
        ...
        if (err == TX_SUCCESS)
        {
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                {
                    /* Print user message */
                    APP_PRINT(USER_DISP_MSG);
                    break;
                }

                case RTT_OUTPUT_MESSAGE_APP_CLIENT_PORT:
                {
                    /* Assign lease time to local buffer */
                    UINT cport = *(UINT*)rtt_op_data->p_msg;
                ...
                case RTT_OUTPUT_MESSAGE_APP_FTP_BYTE_SENT:
                {
                    ULONG sbytes = *(ULONG *)rtt_op_data->p_msg;
                    APP_PRINT("\r\nFTP_no_of_sent_bytes: %d\r\n", sbytes);
                    break;
                }

                case RTT_OUTPUT_MESSAGE_APP_FTP_BYTE_RECV:
                {
                    ULONG rbytes = *(ULONG *)rtt_op_data->p_msg;
...
/*******************************************************************************************************************//**
 * @brief      This function waits for the RTT Thread to initialize and sleep before sending data to it.
