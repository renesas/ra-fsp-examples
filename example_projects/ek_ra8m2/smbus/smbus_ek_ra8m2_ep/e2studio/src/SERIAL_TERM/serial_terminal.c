...
#else
    status = R_SCI_B_UART_Open(&g_uart_ctrl, &g_uart_cfg);
...
#if (BSP_PERIPHERAL_SCI_PRESENT)
    status = R_SCI_UART_BaudCalculate(SERIAL_BAUD_RATE, SERIAL_MODULATION, SERIAL_ERR_X1000, g_uart_cfg_extend.p_baud_setting);
#else
    status = R_SCI_B_UART_BaudCalculate(SERIAL_BAUD_RATE, SERIAL_MODULATION, SERIAL_ERR_X1000, g_uart_cfg_extend.p_baud_setting);
#endif /* BSP_PERIPHERAL_SCI_PRESENT */
    if (FSP_SUCCESS != status)
    {
        return status;
...
#if (BSP_PERIPHERAL_SCI_PRESENT)
    status = R_SCI_UART_BaudSet(&g_uart_ctrl, g_uart_cfg_extend.p_baud_setting);
#else
    status = R_SCI_B_UART_BaudSet(&g_uart_ctrl, g_uart_cfg_extend.p_baud_setting);
#endif /* BSP_PERIPHERAL_SCI_PRESENT */
    if (FSP_SUCCESS != status)
    {
        return status;
