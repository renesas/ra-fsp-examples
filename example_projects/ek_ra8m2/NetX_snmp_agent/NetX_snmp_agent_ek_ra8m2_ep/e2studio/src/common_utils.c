...
UINT rtt_framework_init(VOID)
{
    ...
    err = tx_queue_create(&g_rtt_ip_data_queue, (CHAR *)"dhcp_client_thread_input_queue", 1, &queue_memory_ip_rtt_obj,
...
/* Can be called anywhere after rtt_framework_init() function call for graceful clean up */
void rtt_thread_service_cleanup(void)
{
#if (BSP_CFG_RTOS == AZURE_RTOS)
    tx_byte_pool_delete(&g_byte_pool);

    /* If any user-specific queue has been added, it should be cleaned up as per specific EP */
    tx_queue_delete(&g_rtt_op_data_queue);
