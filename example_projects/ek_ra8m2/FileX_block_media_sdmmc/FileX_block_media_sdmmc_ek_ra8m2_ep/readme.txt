...
3) Format the SD/MicroSD card before performing any operation. Operation is not guaranteed if the card is unformatted
   or formatted with an unsupported file system. If the card is not formatted when first used, please run the "sdhi"
   example project and select option 2 "Format SD Card device" to format it before executing this EP.
