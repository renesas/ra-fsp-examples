...
static uint32_t netx_secure_crypto_aes256cbc_multipart_example(void)
{
    ...
    /* Encryption */
    status = _nx_crypto_method_aes_operation(NX_CRYPTO_ENCRYPT, handler, &crypto_method_aes_cbc_256, NULL,\
    ...
    /* Decryption */
    status = _nx_crypto_method_aes_operation(NX_CRYPTO_DECRYPT, handler, &crypto_method_aes_cbc_256, NULL,\
...
static uint32_t netx_secure_crypto_hmac_sha256_example(void)
{
    ...
    status = _nx_crypto_method_hmac_sha256_operation(NX_CRYPTO_HASH_UPDATE, handler, &crypto_method_hmac_sha256,\
                                                     NX_CRYPTO_NULL, NX_CRYPTO_NULL, (UCHAR*) plain_256,\
                                                     sizeof(plain_256), NX_CRYPTO_NULL, NX_CRYPTO_NULL, NX_CRYPTO_NULL,\
    ...
    status = _nx_crypto_method_hmac_sha256_operation(NX_CRYPTO_HASH_CALCULATE, handler, &crypto_method_hmac_sha256,\
                                                     ...
                                                     NX_CRYPTO_NULL, (UCHAR*) output, sizeof(output), metadata,\
                                                     metadata_size, NX_CRYPTO_NULL, NX_CRYPTO_NULL);
    if (NX_CRYPTO_SUCCESS != status)
    {
...
static uint32_t netx_secure_crypto_aes128cbc_with_trng_example (void)
{
   ...
   fsp_err_t err = FSP_SUCCESS;
    ...
    /* Encryption */
    status = _nx_crypto_method_aes_operation(NX_CRYPTO_ENCRYPT, handler, &crypto_method_aes_cbc_128, NULL,\
    ...
    /* Decryption */
    status = _nx_crypto_method_aes_operation (NX_CRYPTO_DECRYPT, handler, &crypto_method_aes_cbc_128, NULL,\
                                              NX_CRYPTO_NULL, (uint8_t*) generated_ciphertext,\
                                              sizeof(generated_ciphertext), (uint8_t*) random_data,\
                                              generated_plaintext, sizeof(generated_plaintext), metadata,\
                                              metadata_size, NX_CRYPTO_NULL, NX_CRYPTO_NULL);
    if (NX_CRYPTO_SUCCESS != status)
    {
...
static uint32_t netx_secure_crypto_aes256gcm_multipart_example(void)
{
    ...
    status = _nx_crypto_method_aes_init(&crypto_method_aes_256_gcm_16, (uint8_t *) key_gcm_256,\
                                        sizeof(key_gcm_256) << 3U, &handler, metadata, metadata_size);
    if (NX_CRYPTO_SUCCESS != status)
    {
    ...
    status = _nx_crypto_method_aes_operation(NX_CRYPTO_SET_ADDITIONAL_DATA, handler,\
                                             &crypto_method_aes_256_gcm_16, NULL, 0,\
                                             (uint8_t *) aad_gcm_128, sizeof(aad_gcm_128),\
                                             NULL, NULL, 0, metadata, metadata_size,\
                                             NX_CRYPTO_NULL, NX_CRYPTO_NULL);
    if (NX_CRYPTO_SUCCESS != status)
    {
    ...
    /* Encryption */
    status = _nx_crypto_method_aes_operation(NX_CRYPTO_ENCRYPT, handler, &crypto_method_aes_256_gcm_16,\
                                             NULL, 0, (uint8_t *) plain_gcm_128, sizeof(plain_gcm_128),\
                                             (uint8_t *) iv_gcm_256, generated_ciphertext,\
                                             sizeof(generated_ciphertext), metadata, metadata_size,\
                                             NX_CRYPTO_NULL, NX_CRYPTO_NULL);
    if (NX_CRYPTO_SUCCESS != status)
    {
    ...
    /* Decryption */
    status = _nx_crypto_method_aes_operation(NX_CRYPTO_DECRYPT, handler, &crypto_method_aes_256_gcm_16,\
                                             NULL, 0, (uint8_t *) generated_ciphertext,\
                                             sizeof(secret_gcm_128), /* ciphertext size + tag size */\
                                             (uint8_t *) iv_gcm_256, generated_plaintext,\
