...
fsp_err_t board_greenpak_init(void)
{
    ...
    static uint8_t s_ram_reg_array[32] = {};
    static uint8_t s_eeprom_reg_array[32] = {};
    ...
    for (uint8_t ndx = 0; ndx < 16; ndx++)
    {
        ...
        s_eeprom_reg_array[ndx] = reg_val2;
    }

    uint8_t t0 = s_eeprom_reg_array[0] & GREENPAK_IO4_EXPECTED_VALUE;
    uint8_t t1 = s_eeprom_reg_array[1] & GREENPAK_IO5_EXPECTED_VALUE;

    if((t0 != GREENPAK_IO4_EXPECTED_VALUE) || (t1 != GREENPAK_IO4_EXPECTED_VALUE))
    {
        sprintf(g_greenpak_warning_str,LOCAL_GREENPAK_WARNING_MSG);

        s_ram_reg_array[0] = s_eeprom_reg_array[0] & GREENPAK_IO4_MASK;
        s_ram_reg_array[0] = s_ram_reg_array[0] | GREENPAK_IO4_EXPECTED_VALUE;

        s_ram_reg_array[1] = s_eeprom_reg_array[1] & GREENPAK_IO5_MASK;
        s_ram_reg_array[1] = s_ram_reg_array[1] | GREENPAK_IO5_EXPECTED_VALUE;
