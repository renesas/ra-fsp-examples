...
#define SUB_OPTIONS     "\r\nCompares the write and read times to and from external Octo-SPI" \
                        "\r\nflash memory\r\n" \
                        "\r\n> Enter the text block size (in multiples of 2 KB, max 64 KB)\r\n" \
                        "and press tab to continue, or space bar to abort : "
...
/* Cast OSPI_START_ADDRESS to point to uint8_t  */
static uint8_t * const sp_gp_ospi_cs1 = (uint8_t *) OSPI_START_ADDRESS;
        ...
        uint32_t *ospi_performance_read_result)
{
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        {
            /* Erase a block of OSPI memory at an offset from the CS1 base address.
             * The cast to (uint8_t *) is required because pointer arithmetic is used
             * to compute the target address, and the erase API expects a uint8_t* type.
             */
            err = g_ospi0.p_api->erase (g_ospi0.p_ctrl, (uint8_t *) (sp_gp_ospi_cs1 + (erase_size * nb)), erase_size);
...
test_fn ext_display_menu(void)
{
    ...
    /* Keep trying to read a valid text block size complete the loop in one of two ways:
    ...
    while (false == valid_block_size)
    {
        ...
        while ((CONNECTION_ABORT_CRTL != c))
        {
            ...
            if ((CARRAGE_RETURN != c) && (BACK_SPACE != c))
            {
                ...
                /* Cast 'c' to char_t to ensure it matches the expected type for formatted output.
                 * This is important if 'c' is defined as a wider or different integer type,
                 * ensuring correct character representation in sprintf.*/
...
static fsp_err_t write_test_opi(uint32_t num_blocks)
{
    ...
    /* Cast the result of rand() to uint8_t to ensure only the lower 8 bits are stored.
     * This is important because rand() returns an int, but we only need a byte-sized value.
     * The cast prevents unintended truncation and ensures compatibility with 8-bit operations.
     */
    ...
    for (s_test_current_block = 0; s_test_current_block < num_blocks; s_test_current_block++)
    {
        ...
        while (s_test_remain > 0)
        {
            ...
            /* cast uint8_t from rand function */
            data                                = (uint8_t)rand();
        ...
        /* Cast the target address to uint8_t* to match the expected type for the OSPI write API.
         * This ensures byte-level access to the calculated memory location, which is derived
         * from the CS1 base address plus an offset based on the current block index.
         */
        err = g_ospi0.p_api->write (g_ospi0.p_ctrl, (s_g_test_data),
...
static fsp_err_t read_test_opi(uint32_t block_size)
{
    ...
    srand(0);
    volatile uint8_t  data   = (uint8_t) rand(); /* cast uint8_t from rand function */
    ...
    {
        data = (uint8_t) rand(); /* cast uint8_t from rand function */
...
static void ospi_b_change_omclk(bsp_clocks_octaclk_div_t divider)
{
    ...
    /* PLL2R should be set up to give a clock speed of 266 MHz */
