...
static void jlink_console_write(const char_t *buffer)
{
    ...
    /* API function required uint8_t, while application buffer required char_t which on this device is the same size */
    ...
    switch (p_args->event)
    {
        ...
        case UART_EVENT_RX_CHAR:
        {
            ...
            if ((sizeof(s_out_of_band_received)) > s_g_out_of_band_index)
            {
                ...
                {
                    /* cast to truncate generic data to fit uint8_t and loose the rest of the data */
