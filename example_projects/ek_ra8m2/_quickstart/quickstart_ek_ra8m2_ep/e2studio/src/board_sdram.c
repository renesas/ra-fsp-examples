 ...
 **********************************************************************************************************************/
    ...
    #error \
    "BSP_PRV_SDRAM_ENDIAN_MODE must be either of 0(same endian as operating mode) " \
    "or 2(another endian against operating mode)"
#endif

#if ((BSP_PRV_SDRAM_CONTINUOUS_ACCESSMODE != 0) && (BSP_PRV_SDRAM_CONTINUOUS_ACCESSMODE != 1))
    #error \
    "BSP_PRV_SDRAM_CONTINUOUS_ACCESSMODE must be either of 0(continuous access is disabled) " \
    "or 1(continuous access is enabled)"
#endif
...
void bsp_sdram_init (void)
{
    ...
    if(false == is_sdram_initialised())
    {
        ...
        {
            /* According to h/w maual,
             * need to confirm that all the status bits in SDSR are 0 before SDIR modification. */
            __NOP();
        }
        ...
        {
            /* According to h/w maual,
             * need to confirm that all the status bits in SDSR are 0 before SDICR modification. */
            __NOP();
        }
        ...
        {
            /* According to h/w maual,
             * need to confirm that all the status bits in SDSR are 0 before SDMOD modification. */
            __NOP();
        }
