 ...
 *                 FSP_ERR_TIMEOUT      Upon execution time expiry
 **********************************************************************************************************************/
static fsp_err_t wait_transfer_end(void)
{
    /* Reset timeout value */
    ...
    if (RESET_VALUE == timeout)
    {
        ...
        {
            APP_ERR_RET(FSP_ERR_ASSERTION != FSP_SUCCESS, FSP_ERR_ASSERTION, "SAU SPI transfer occurs the read overflow error\r\n");
        }

        /* If neither event is detected, the function returns a timeout error. */
         APP_ERR_RET(FSP_ERR_TIMEOUT != FSP_SUCCESS, FSP_ERR_TIMEOUT, "SAU SPI transfer exceeds the timeout period\r\n");
    }
...
static fsp_err_t communicate_with_master_board(void)
{
    ...
    /* Read data and store it in spi_rx_buf, and write data in write_buf to the response */
    ...
    else if (CFG_READ_CMD == spi_rx_buf[INDEX_ZERO])
    {
        ...
        {
            APP_PRINT("\r\nStart transmit MCU temperature to Master.\r\n");
 ...
 *  Description  : This function de-initilizes opened SAU SPI Slave module.
 *  Arguments    : None
 *  Return Value : None
 **********************************************************************************************************************/
 ...
 *  Description  : This function de-initilizes opened External IRQ module.
 *  Arguments    : None
 *  Return Value : None
 **********************************************************************************************************************/
static void active_transfer_deinit(void)
