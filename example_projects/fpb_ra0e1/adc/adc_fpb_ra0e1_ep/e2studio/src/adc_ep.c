...
/* stops the adc scan if the adc is sequential mode and then close the module */
static fsp_err_t adc_scan_stop(void);

#ifndef BOARD_RA0E1_FPB
...
static fsp_err_t adc_scan_start(void)
{
    ...
    if (false == g_ready_to_read)
    {
        ...
        {
            /* ADC Failure message */
            APP_ERR_PRINT("** R_ADC_D_ScanCfg API failed ** \r\n");
            return err;
        }
#else
...
fsp_err_t adc_read_data(void)
{
    ...
    {
        /* ADC Failure message */
        APP_ERR_PRINT("** R_ADC_D_Read API failed ** \r\n");
        return err;
    }
#else
