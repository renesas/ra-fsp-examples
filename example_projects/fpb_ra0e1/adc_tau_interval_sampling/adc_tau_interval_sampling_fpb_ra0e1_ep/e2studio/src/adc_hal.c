...
/*flags to capture ADC group events*/
...
transfer_info_t g_transfer_adc_group[] =
{
 [ZERO] =
 {
  ...
  .p_src = (void const*) &R_ADC_D->ADCR0,
  .num_blocks = ZERO,
 ...
 [ONE] =
 {
  ...
  .p_src = (void const*) &R_ADC_D->ADCR1,
  .num_blocks = ZERO,
  .length = NUM_SAMPLES_PER_CHANNEL,
 },
...
void g_adc0_callback(adc_callback_args_t *p_args)
{
    ...
    switch(event)
    {
        ...
        {
            /*set the flag for adc 0 group, when ADC_EVENT_SCAN_COMPLETE_GROUP occured */
