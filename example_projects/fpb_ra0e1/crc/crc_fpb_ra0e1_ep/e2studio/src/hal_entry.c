...
void hal_entry(void)
{
	...
	/* Open uart module */
#if  defined (BOARD_RA6T2_MCK) || defined (BOARD_RA8D1_EK)|| defined (BOARD_RA8M1_EK)
...
static fsp_err_t crc_operation (void)
{
    ...
    #if defined (BOARD_RA4E2_EK) || defined (BOARD_RA0E1_FPB)
	    crc_input_t rx_input_data;
	    uint32_t rx_normal_crc_value       = RESET_VALUE; //CRC value in Normal mode

        /* Perform SCI UART (BOARD_RA4E2_EK) loop-back transmission from TX to RX*/
