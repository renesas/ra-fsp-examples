...
void pcdc_thread_entry(void *pvParameters)
{
    ...
    /* Open the UART with initial configuration */
    {
        ...
        /* Synchronize UART line coding with USB settings for the first time */
        set_pcdc_line_coding (&g_line_coding, &g_uart_test_cfg);

        /* Initialize semaphores to allow the first UART and USB transfer */
        {
    ...
    while (1)
    {
        /* Wait for an event from the USB or UART peripheral */
        err_queue = xQueueReceive(g_event_queue, (void * const)&q_instance, (portMAX_DELAY));
        if(pdTRUE != err_queue)
        {
            APP_ERR_PRINT("\r\nNo event received from USB or UART. Please check the connection.\r\n");
        }
        ...
        if (PERIPHERAL_USB == q_instance.peripheral)
        {
            ...
            if ((true == b_usb_attach))
            {
                ...
                {
                    /* Wait until previously queued data is out completely */
                    {
...
/*******************************************************************************************************************//**
 * @brief     Callback function for USB PCDC class when using FreeRTOS.
 * @param[in] p_pcdc_event   Pointer to USB event information.
 * @param[in] task           USB task handle (unused).
