...
void sdhi_thread_entry(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        if((RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED == xEventGroupValue))
        {
            ...
            APP_ERR_PRINT("\r\nConnect the "CARD_TYPE" Card and execute Safely Eject option to make sure file operations works"\
                          " correctly\r\n");
...
static void format_sd_device(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        /* Formatting time varies with SD/MicroSD Card capacity. Might take longer time for higher capacity SD/MicroSD Card. */
