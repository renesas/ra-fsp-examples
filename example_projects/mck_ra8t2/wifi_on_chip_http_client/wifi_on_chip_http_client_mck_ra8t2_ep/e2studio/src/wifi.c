 ...
 *              Gives the user the option to select the AP from that list to connect.
 * @param[IN]   None.
...
static WIFIReturnCode_t scan_wifi(void)
{
    ...
    do
    {
        ...
        /* The user selects SSID from WiFi APs list */
...
static void manual_connect(void)
{
    ...
    /* The user inputs for SSID of WiFi AP */
    APP_PRINT("\r\nEnter SSID of WiFi Access Point:");
    user_input((char*) g_net_params.ucSSID);

    /* The user selects security type of WiFi AP */
    do
    {
...
/***********************************************************************************************************************
 * @brief       This function gets the IP configuration and prints out the obtained IP configuration.
 * @param[in]   None.
