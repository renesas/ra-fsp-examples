...
extern SocketParameters_t g_socket;
...
uint8_t g_packet_pool0_pool_memory[G_PACKET_POOL0_PACKET_NUM * (G_PACKET_POOL0_PACKET_SIZE + \
        sizeof(NX_PACKET))] BSP_ALIGN_VARIABLE(4) WIFI_BUFFER_PLACE_IN_SECTION;
#endif
...
void azure_wifi_thread_entry(void)
{
    ...
    do
    {
        ...
        switch(connect_menu_option)
        {
            case SCAN_AP:
            {
                ...
                if(FSP_SUCCESS != err)
                {
                    PRINT_ERR_STR("WiFi Access Point scanning failed. If multiple failures, "\
                                  "please enter WiFi AP credentials.");
                    PRINT_ERR_STR("Please Use option 2 for manual connect to desired WiFi AP available.");
                }
                else
                {
                    /* Connecting to the user selected WiFi AP */
            ...
            case ENTER_SSID:
            {
                /* The user manually enter WiFi AP credentials to connect */
                err = manual_connect();
                if(FSP_SUCCESS != err)
                {
                    PRINT_ERR_STR("Incorrect Access Point credentials entered. Try again.");
    ...
    do
    {
        ...
        else
        {
            ...
            if(FSP_SUCCESS != err)
            {
                ...
                PRINT_INFO_STR("Network device may be off-line or invalid URL/IP address."\
    ...
    status = nx_tcp_client_socket_connect(&tcp_client_socket,SOCKETS_HTONL(g_socket.ip_addr_server),\
                                          g_socket.port, NETXDUO_TESTS_SOCKET_TIMEOUT);
    if(NX_SUCCESS != status)
    {
    ...
    while (true)
    {
        ...
        else
        {
            ...
            status = nx_packet_data_retrieve(p_nx_packet, g_socket.recv_buff, (ULONG*)&bytes_copied);
            if(NX_SUCCESS != status)
            {
 ...
 || defined (BOARD_RA8T2_MCK)
                ...
                status = nx_packet_data_append(p_nx_packet, g_socket.send_buff, sizeof(g_socket.send_buff),\
                                               &g_packet_pool0, NETXDUO_TESTS_SOCKET_TIMEOUT);
                if(NX_SUCCESS != status)
                {
                    PRINT_ERR_STR("nx_packet_data_append API failed");
                    ERROR_TRAP(status);
