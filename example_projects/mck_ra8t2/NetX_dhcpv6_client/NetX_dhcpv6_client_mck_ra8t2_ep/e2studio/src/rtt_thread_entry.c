...
/*******************************************************************************************************************//**
 * @brief     This is entry function of RTT thread.
 * @param[IN] None.
 * @retval    None.
 **********************************************************************************************************************/
void rtt_thread_entry(void)
{
    ...
    while (1)
    {
        ...
        /* Print any message on RTT output if the queue has data, then the same will be processed */
        process_rtt_op_msg();
#if (BSP_CFG_RTOS == AZURE_RTOS)
        ...
        if (err == TX_SUCCESS)
        {
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                case RTT_OUTPUT_MESSAGE_APP_VAL:
                {
                    ...
                    ULONG lease_info[4] = {RESET_VALUE};
                    memcpy(&lease_info, (ULONG *) rtt_op_data->p_msg, sizeof(lease_info));
                    APP_PRINT("\r\nIA address renew time = %lu\r\nIA address rebind time = %lu\r\n"\
                              "Time when IA address is deprecated = %lu\r\n"\
                              "Time when IA address is expired = %lu\r\n",\
                              lease_info[0], lease_info[1], lease_info[2], lease_info[3]);
                }
                break;

                case RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_IP:
                {
                    /* Assign IPv6 address to local buffer */
                    NXD_ADDRESS client_addr = {0};
                    UCHAR str[50] = {'\0'};
                    memcpy(&client_addr, (NXD_ADDRESS *)rtt_op_data->p_msg, sizeof(NXD_ADDRESS));
                    str_ipv6(&str[0], client_addr);
                    APP_PRINT("\r\nClient is bound to IPv6 address %s\r\n", &str[0]);
                }
                break;
                ...
                {
                    /* Assign IPv6 address to local buffer */
                    NXD_ADDRESS dns_server_addr = {0};
                    UCHAR str[50] = {'\0'};
                    memcpy(&dns_server_addr, (NXD_ADDRESS *)rtt_op_data->p_msg, sizeof(NXD_ADDRESS));
                    str_ipv6(&str[0], dns_server_addr);
                    APP_PRINT("\r\nDNS server address is %s\r\n", &str[0]);
                }
                break;
...
UINT app_rtt_print_data(event_id_t id, uint32_t size, void * const p_data)
{
    ...
    /* Allocate memory for RTT Viewer display message data structure */
...
/*******************************************************************************************************************//**
 * @brief      This is user input read function. To be called by user thread for receiving RTT input.
...
/*******************************************************************************************************************//**
 * @brief      This is used to wait for the RTT Thread to initialize and sleep before sending data to it.
