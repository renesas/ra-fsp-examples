...
/***********************************************************************************************************************
 * @brief     This function queues one raw byte for UART transmission and returns as soon as it is
 *            queued - it does NOT wait for the byte to actually go out on the wire. The byte is
 *            always pushed into the TX ring buffer first (never handed to UART_WRITE() from a
 *            local/stack variable - see the g_tx_buf comment for why); if the UART is currently
 *            idle, a transmission is kicked off for whatever is now queued (which may already
 *            include bytes from a still-in-progress printf() call, letting them go out as one
 *            batch). In the rare case the ring buffer is completely full (e.g. a boot banner
 *            longer than the buffer), this blocks until TX_COMPLETE drains room for it - IRQs
 *            stay enabled while waiting so that drain can actually happen - rather than giving up
 *            after a fixed number of iterations and silently dropping the byte.
...
{
    /* Wait for room - IRQs enabled so TX_COMPLETE can drain the buffer */
    while (UART_CONSOLE_TX_BUFFER_SIZE <= g_tx_count)
    {
    }
