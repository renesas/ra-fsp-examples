...
/*******************************************************************************************************************//**
 * @brief       This function initializes SPI Master module.
 * @param[IN]   None
...
#if (BSP_PERIPHERAL_SPI_B_PRESENT)
    err = R_SPI_B_Write(&g_spi_master_ctrl, (uint8_t *)&spi_length, ONE_BYTE, SPI_BIT_WIDTH_8_BITS);
#else
...
#if (BSP_PERIPHERAL_SPI_B_PRESENT)
    err = R_SPI_B_Read(&g_spi_master_ctrl, (uint8_t *)&returned_size, ONE_BYTE, SPI_BIT_WIDTH_8_BITS);
#else
...
#if (BSP_PERIPHERAL_SPI_B_PRESENT)
    err = R_SPI_B_Write(&g_spi_master_ctrl, (uint8_t *)&validate_result, ONE_BYTE, SPI_BIT_WIDTH_8_BITS);
#else
...
/*******************************************************************************************************************//**
 * @brief       This function demos R_SPI_WriteRead()/R_SPI_B_WriteRead() for Master simultaneously.
 * @param[IN]   None
 * @retval      FSP_SUCCESS     Upon successful Write and Read on Master.
 * @retval      Any other error code apart from FSP_SUCCESS  Unsuccessful Write and Read on Master.
 **********************************************************************************************************************/
fsp_err_t spi_write_read(void)
{
...
#if (BSP_PERIPHERAL_SPI_B_PRESENT)
    err = R_SPI_B_WriteRead(&g_spi_master_ctrl, g_master_tx_buff, g_master_rx_buff, BUFF_LEN, SPI_BIT_WIDTH_32_BITS);
#else
    err = R_SPI_WriteRead(&g_spi_master_ctrl, g_master_tx_buff, g_master_rx_buff, BUFF_LEN, SPI_BIT_WIDTH_32_BITS);
#endif /* BSP_PERIPHERAL_SPI_B_PRESENT */
    ...
    /* Master sends message to Slave and receives message from Slave */
#if (BSP_PERIPHERAL_SPI_B_PRESENT)
    err = R_SPI_B_WriteRead(&g_spi_master_ctrl, g_master_rx_buff, g_master_rx_validate_buff, BUFF_LEN,
                            SPI_BIT_WIDTH_32_BITS);
#else
    err = R_SPI_WriteRead(&g_spi_master_ctrl, g_master_rx_buff, g_master_rx_validate_buff, BUFF_LEN,
                          SPI_BIT_WIDTH_32_BITS);
#endif /* BSP_PERIPHERAL_SPI_B_PRESENT */
...
/*******************************************************************************************************************//**
 * @brief       This function closes SPI Master module.
 * @param[IN]   None 
