    ...
    do
    {
        ...
        if (err == TX_SUCCESS)
        {
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                {
                    /* Print DHCPv6 Server menu options */
                    APP_PRINT(DHCPV6_SERVER_MENU);
                }
                break;
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_DATA:
                {
                    ...
                    str_mac(&str[0], lease_info.mac_high, lease_info.mac_low);
                    APP_PRINT("\r\nClients MAC address : %s\r\n", &str[0]);
                }
                break;
