...
1. Project Overview:
	...
	manually by an external interrupt or automatically by a timer to exit LPM mode. Turn off the LED0 and display the LPM Mode
	Name before entering each LPM mode. Turn on the LED0 and display the cancel source name after exiting each LPM mode.The EP 
	log will print to the Host PC via the UART interface at a baud rate of 115200 bps. The LED1 will turn on if an error occurs.
...
5. The steps to run LPM Example Projects are as follows:
	...
	5) Press the user push-button S1 or wait 10 seconds to enter and cancel LPM mode. It will turn off LED0 before entering each LPM mode 
	   and turn on LED0 after exiting each LPM mode.
