...
1. Project Overview:	
	...
	DAC0 reference voltage, Comparator output status is HIGH and on-board LED is turned ON. When DAC1 input is less
	...
	input voltage source is greater than reference voltage, Comparator output status is HIGH and on-board LED is
	turned ON. When input voltage source is less than reference voltage, output status is LOW and the LED is
	turned OFF.

	For FPB-RA2T1:
	The example project demonstrates the typical use of the ACMPHS HAL module APIs. In this project Internal
	Reference Voltage (Vref) is used as reference voltage source and CMPIN01 is used as input voltage source for
	ACMPHS module, which is supplied by external DC power supply. Internal Reference Voltage value is set to 1.44V.
	The user can input DC power value within permitted range to provide an analog signal input for CMPIN01. When input
...
2. Hardware Requirements:
	Supported RA Boards: EK-RA6M1, EK-RA6M2, EK-RA6M3, EK-RA6M3G, RSSK-RA6T1, MCK-RA6T2, MCK-RA4T1, MCK-RA6T3,
                             EK-RA8M1, EK-RA8D1, MCK-RA8T1, FPB-RA8E1, EK-RA8E2, FPB-RA2T1.
...
1) For boards that support DAC module:
   ...
   b. The user is expected to enter data of size not exceeding 15 bytes.
