/***********************************************************************************************************************
 * File Name    : iso7816_T0_protocol.c
...
/*******************************************************************************************************************//**
 * Initialize parameters that are used by T=0 protocol.
 * @param   t0:                 Pointer to parameters for T=0 protocol.
...
fsp_err_t ISO7816_T0_SET_PARAMETERS(smci_t0_parameters_t * t0, smc_atr_message_t * sc_returned_atr, uint32_t etu_us)
{
    ...
    /* Get the information of current used F, D, frequency */
    t0->F = 372;
    t0->D = smci_speed_params.di;
    t0->frequency = max_clock_frequency;

    /* Get the WI from ATR TC2 parameter */
    ...
    if(sc_returned_atr->Tc[0] != 0)
    {
        ...
        /* Calculate the GT */
        T0_GT = (uint32_t)( (((int64_t) t0->F) * ((int64_t) t0->gti) * 1000000) / \
                 (((int64_t) t0->D) * ((int64_t) t0->frequency)) ); /* Microsecond base */
 ...
 * The response TPDU is mapped onto the response APDU according to the acceptance of Le and according to the processing
 * of the command.
 *
 ...
 * The response TPDU from the card indicates that the command is aborted due to a wrong length and that the right length
 * is La: (SW1=’6C’ and SW2 codes La).
 * If the transmission system does not support the service of re-issuing the same command, it shall map the response
 * TPDU onto the response APDU without any change
