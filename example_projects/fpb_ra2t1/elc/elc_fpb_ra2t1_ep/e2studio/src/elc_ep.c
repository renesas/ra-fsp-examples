...
void elc_ep_entry(void)
{
     ...
     * For elc_ep run with GPT timer: 1. ELC Software event as start source for GPT0 and GPT2
     *                                2. GPT2 counter overflow event as stop source for GPT0
    ...
    /* On successful operations on ELC and TAU/GPT modules,
     * the user is asked enter any key as J-link RTT Viewer input */
    APP_PRINT(STATUS_DISPLAY);
    /* Wait until data is available in J-link RTT Viewer */
    while (true)
    {
        if (APP_CHECK_DATA)
        {
