/***********************************************************************************************************************
 * File Name    : dhcpv6_thread_entry.c
 * Description  : Contains functions from the dhcpv6 main thread.
 **********************************************************************************************************************/
/***********************************************************************************************************************
...
uint8_t g_dhcpv6_server_stack_memory[G_DHCPV6_SERVER_TASK_STACK_SIZE] BSP_PLACE_IN_SECTION(".stack.g_dhcpv6_server")
        BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);

/* User defined variables */
/* Set the DUID time as the start of the millennium */
...
/*******************************************************************************************************************//**
 * @brief     This function is the DHCPv6 Server thread entry.
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
void dhcpv6_thread_entry(void)
{
    ...
    ipv6_info_t leased_info =
    {
         .ipv6_address = {RESET_VALUE, {RESET_VALUE}},
         .mac_high = RESET_VALUE,
         .mac_low  = RESET_VALUE,
    };
    ...
    /* Create DHCPv6 IP address range */
        ...
        switch (read_data)
        {
            ...
            case PRINT_DHCPV6_LEASED_IP_INFO:
            {
                if (true == b_serverReady)
                {
                    ...
                    for (ULONG idx = RESET_VALUE;
                         g_dhcpv6_server.nx_dhcpv6_lease_list[idx].nx_dhcpv6_lease_assigned_to != NULL; idx++)
                    {
                        /* Assign and print leased IPv6 address, clients MAC address */
                        memset(&leased_info, RESET_VALUE, sizeof(ipv6_info_t));
                        memcpy(&leased_info.ipv6_address,
                               &g_dhcpv6_server.nx_dhcpv6_lease_list[idx].nx_dhcpv6_lease_IP_address,
                               sizeof(NXD_ADDRESS));
                        leased_info.mac_high = g_dhcpv6_server.nx_dhcpv6_lease_list[idx].nx_dhcpv6_lease_assigned_to->\
                                               nx_dhcpv6_client_duid.nx_link_layer_address_msw;
                        leased_info.mac_low  = g_dhcpv6_server.nx_dhcpv6_lease_list[idx].nx_dhcpv6_lease_assigned_to->\
                                               nx_dhcpv6_client_duid.nx_link_layer_address_lsw;
                        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_DATA, sizeof(ipv6_info_t ),
                                           &leased_info);
...
/*******************************************************************************************************************//**
 * @brief     Creates the IP instance and enables ARP, UDP, ICMP, IPv6.
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
static void ip_init0(void)
{
    ...
    /* Enable the ARP management component for the specified IP instance */
...
/*******************************************************************************************************************//**
 * @brief     Creates the DHCPv6 Server instance.
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
static void dhcpv6_server_init0(void)
