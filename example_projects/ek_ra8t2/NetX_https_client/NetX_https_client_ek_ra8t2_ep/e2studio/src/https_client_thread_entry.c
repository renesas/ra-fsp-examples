...
static UINT process_input_request(uint8_t input_data);
...
void https_client_thread_entry(void)
{
    ...
    while (true)
    {
        ...
        status = process_input_request(read_data);
        if (NX_SUCCESS != status)
        {
            PRINT_ERR_STR("process_input_request() function failed.");
            deinit_https_client();
            ERROR_TRAP(status);
        }
...
/*******************************************************************************************************************//**
 * @brief     This function initializes the DHCP v4 Client by creating the DHCP Client instance
 ...
 *            allocates the required packet pool. It process the DNS Server IP address.
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
static UINT dns_client_init(void)
...
/*******************************************************************************************************************//**
 * @brief      This function processes user input request and sends or receives the temperature data to
 *             the Adafruit Server.
 * @param[in]  input_data                 User input data.
