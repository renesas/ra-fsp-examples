...
/*******************************************************************************************************************//**
 *  @brief      This function will initialize the UART module which transfer the EP log to the host PC.
 *  @param      None.
...
/*******************************************************************************************************************//**
 *  @brief      This function will de-initialize UART module.
 *  @param      None.
 *  @retval     None.
 **********************************************************************************************************************/
void uart_deinit(void)
{
    /* De-initialize UART if it is opened */
    if (g_uart_ctrl.open)
    {
#if (BSP_PERIPHERAL_SCI_PRESENT)
...
#elif (BSP_PERIPHERAL_SAU_UART_PRESENT)
    err = R_SAU_UART_Write(&g_uart_ctrl, p_buf, len);
#else
...
fsp_err_t uart_ep_info_print(void)
{
    ...
    /* Print EP version and FSP version to terminal on the host PC via UART */
    ...
    /* Print EP information to terminal on the host PC via UART */
...
fsp_err_t uart_error_info_print(uint8_t * p_func, uint8_t * p_info)
{
    ...
    /* Print to terminal on the host PC via UART */
    err = uart_print(uart_buffer);
    return err;
}
