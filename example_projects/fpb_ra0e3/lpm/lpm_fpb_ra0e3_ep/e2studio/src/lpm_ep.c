...
static fsp_err_t lpm_ep_startup(void)
{
    ...
    /* Initialize UART module first to print log to terminal */
    err = uart_init();
...
static fsp_err_t lpm_enter_procedure(app_lpm_states_t lpm_mode)
{
    ...
    /* UART module Stop with Undefined in SW Standby and Deep SW Standby mode */
...
#if defined(BOARD_RA8D1_EK) || defined(BOARD_RA8M1_EK) || defined(BOARD_RA8T1_MCK) || defined(BOARD_RA8E1_FPB) ||\
    defined(BOARD_RA8E2_EK) || defined(BOARD_RA8P1_EK)
    /* Re-initialize UART after SW Standby Mode */
    if (APP_LPM_SW_STANDBY_STATE == lpm_mode)
    {
        uart_init();
    }
 ...
 *  @param[in]  p_source      Pointer to source data to be written.
