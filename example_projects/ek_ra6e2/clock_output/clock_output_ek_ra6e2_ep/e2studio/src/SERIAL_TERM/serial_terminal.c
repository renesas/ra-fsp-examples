 ...
 * Description  : Contains declarations of data structures and functions.
 **********************************************************************************************************************/
/***********************************************************************************************************************
...
/*******************************************************************************************************************//**
 *  @brief      This function is used to initialize the SCI_UART_B module to transfer the EP log to the host PC
 *  @param      None
...
fsp_err_t uart_init(void)
{
    ...
    /* Initialize SCI_UART_B channel with baud rate 115200 */
    err = R_SCI_B_UART_Open(&g_uart_ctrl, &g_uart_cfg);
#else
    /* Initialize SCI_UART channel with baud rate 115200 */
...
/*******************************************************************************************************************//**
 *  @brief      This function is used to de-initialize the SCI_UART_B module
 *  @param      None
 *  @retval     None
 **********************************************************************************************************************/
void uart_deinit(void)
{
    ...
    /* De-initialize SCI_UART_B if it opened */
    if (MODULE_CLOSE != g_uart_ctrl.open)
    {
        ...
        if(FSP_SUCCESS != err)
        {
            APP_ERR_PRINT(err, "Failed to close SCI UART interface.\r\n");
        }
    }
}
...
uint8_t serial_write_message (char * format, ...)
{
    ...
    /* Reset SCI_UART_B callback variable */
    g_uart_event = RESET_VALUE;

    /* Write message to UART interface */
#if (BSP_FEATURE_SCI_VERSION == 2U)
    status = (uint8_t)R_SCI_B_UART_Write(&g_uart_ctrl, g_tx_buffer, len);
#else
    status = (uint8_t)R_SCI_UART_Write(&g_uart_ctrl, g_tx_buffer, len);
#endif
