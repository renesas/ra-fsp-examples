        ...
        if(err == TX_SUCCESS)
        {
            ...
            switch(rtt_op_data->msg_id)
            {
                ...
                case RTT_OUTPUT_MESSAGE_MCU_IP_ADDRESS:
                {
                    ...
                    APP_PRINT("\r\nMCU IP address: %d.%d.%d.%d\r\n",mcu_ip[INDEX_ZERO] ,mcu_ip[INDEX_ONE],mcu_ip[INDEX_TWO],mcu_ip[INDEX_THREE]);
                }
                break;
                ...
                {
                    WIFIScanResult_t scan_data={RESET_VALUE};
                    scan_data = *(WIFIScanResult_t*)rtt_op_data->p_msg;
                    APP_PRINT("%s: Signal strength: %d, Channel: %d",scan_data.ucSSID,scan_data.cRSSI, scan_data.ucChannel);
                }
                break;
