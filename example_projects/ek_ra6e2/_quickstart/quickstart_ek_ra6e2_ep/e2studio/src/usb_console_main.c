...
void usb_console_main(void)
{
    ...
    static fsp_err_t             s_err              = FSP_SUCCESS;
    ...
    /* Open USB instance */
    s_err = R_USB_Open (&g_basic0_ctrl, &g_basic0_cfg);

    /* Handle error */
    if (FSP_SUCCESS != s_err)
    {
        /* Fatal error */
        SYSTEM_ERROR
    }
    ...
    while (true)
    {
        ...
        switch (s_apl_event_info.event)
        {
            ...
            case USB_STATUS_CONFIGURED:
            {
                ...
                s_err = R_USB_Read(&s_apl_event_info, g_buf, READ_BUF_SIZE, g_usb_class_type);
            ...
            {
                s_err = R_USB_Read (&s_apl_event_info, g_buf, 1, g_usb_class_type);

                if (FSP_SUCCESS == s_err)
                {
                    memcpy(&s_response , &g_buf, 1);
            ...
            case USB_STATUS_REQUEST : /* Receive Class Request */
            {
                ...
                if (USB_PCDC_SET_LINE_CODING == (s_usb_setup.request_type & USB_BREQUEST))
                {
                    ...
                    s_err =  R_USB_PeriControlDataGet (&s_apl_event_info,
                ...
                else if (USB_PCDC_GET_LINE_CODING == (s_usb_setup.request_type & USB_BREQUEST))
                {
                    ...
                    s_err =  R_USB_PeriControlDataSet (&s_apl_event_info,
                                                        (uint8_t *) &s_line_coding,
                                                        LINE_CODING_LENGTH);
                }
                else if (USB_PCDC_SET_CONTROL_LINE_STATE == (s_usb_setup.request_type & USB_BREQUEST))
                {
                    s_err = R_USB_PeriControlStatusSet (&s_apl_event_info, USB_SETUP_STATUS_ACK);
                }
                else
                {
                    /* none */
...
static fsp_err_t check_for_write_complete(void)
{
    ...
    do
    {
        ...
        s_err = (fsp_err_t)USB_STATUS_NONE;
