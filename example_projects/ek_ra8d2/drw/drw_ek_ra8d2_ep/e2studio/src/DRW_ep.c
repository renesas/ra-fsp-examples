...
/* Triangle variables */
static d2_point g_t_x1 = TRIANGLE_X1, g_t_y1 = TRIANGLE_Y1, g_t_x2 = TRIANGLE_X2, g_t_y2 = TRIANGLE_Y2,\
                g_t_x3 = TRIANGLE_X3, g_t_y3 = TRIANGLE_Y3;
...
/* Line variables */
static d2_point g_l_x1 = LINE_VAL_X1, g_l_y1 = LINE_VAL_Y1, g_l_x2 = LINE_VAL_X2, g_l_y2 = LINE_VAL_Y2,\
                g_l_w = LINE_VAL_WIDTH;
...
/* This table of commands was adapted from sample code provided by FocusLCDs
...
/*******************************************************************************************************************//**
 * @brief      This function initializes and manages the GLCDC and DRW engines for display rendering.
 * @param      None
 * @retval     None
 **********************************************************************************************************************/
void DRW_entry(void)
{
    ...
    APP_PRINT("\r\nEnter any key to stop rendering and disable the GLCDC driver:\r\n");
    ...
    while(true)
    {
        ...
        do
        {
            /* Now that the frame buffer is ready, update the GLCDC buffer pointer on the next Vsync */
    #if defined(DISPLAY_IN_FORMAT_32BITS_RGB888_0)
        ...
        /* Draw the new frame buffer now */
        display_draw(p_framebuffer);

        /* Check input data from RTT */
        if (APP_CHECK_DATA)
        {
            APP_PRINT("Terminating render....\r\n");
            break;
        }
    }
...
/*******************************************************************************************************************//**
 * @brief      User defined function to initialize GLCDC module.
 * @param      None
 * @retval     None
 **********************************************************************************************************************/
static void glcdc_init(void)
...
/*******************************************************************************************************************//**
 * @brief      User defined function to initialize DRW module.
 * @param      None
 * @retval     None
 **********************************************************************************************************************/
static void drw_init(void)
{
    ...
    if (D2_OK == DRW_err)
    {
        APP_PRINT("\r\n** Alpha blend mode set successfully **\r\n");
    }
    else
    {
        error_handler(DRW_err, gp_davey);
...
/*******************************************************************************************************************//**
 * @brief      Callback function for GLCDC interrupts.
...
static void display_draw (uint32_t * framebuffer)
{
    /* DRW operations happen here */
    /* Animation of different figure */
    drw_animation();
    ...
    /* Render a circle */
    DRW_err = d2_rendercircle(gp_davey,(d2_point)(g_c_c_x1 << SHIFT_VALUE), (d2_point)(g_c_c_y1 << SHIFT_VALUE),
                              (d2_width)(g_c_r << SHIFT_VALUE), (d2_width)(g_c_w << SHIFT_VALUE));
    error_handler(DRW_err, gp_davey);
    ...
    /* Render a triangle */
    DRW_err = d2_rendertri(gp_davey, (d2_point)( g_t_x1 << SHIFT_VALUE),(d2_point)( g_t_y1 << SHIFT_VALUE),
                           (d2_point)( g_t_x2 << SHIFT_VALUE),(d2_point)( g_t_y2 << SHIFT_VALUE),
                           (d2_point)( g_t_x3 << SHIFT_VALUE),(d2_point)( g_t_y3 << SHIFT_VALUE),d2_edge2_shared);
    error_handler(DRW_err, gp_davey);
    ...
    /* Render a box */
    DRW_err = d2_renderbox(gp_davey,(d2_point)(g_b_x1 << SHIFT_VALUE), (d2_point)(g_b_y1 << SHIFT_VALUE),
                           (d2_width)(g_b_l << SHIFT_VALUE), (d2_width)(g_b_w << SHIFT_VALUE));
    error_handler(DRW_err, gp_davey);
    ...
    /* Render a line */
    DRW_err = d2_renderline(gp_davey, (d2_point) (g_l_x1 << SHIFT_VALUE), (d2_point) (g_l_y1 << SHIFT_VALUE),
                            (d2_point) (g_l_x2 << SHIFT_VALUE), (d2_point) (g_l_y2 << SHIFT_VALUE),
...
/*******************************************************************************************************************//**
 * @brief      User-defined function to set framebuffer properties.
 * @param[in]  framebuffer    Pointer to frame buffer.
 * @retval     None
 **********************************************************************************************************************/
#if defined(DISPLAY_IN_FORMAT_32BITS_RGB888_0)
static void drw_set(uint32_t * framebuffer)
{
    ...
    DRW_err = d2_framebuffer(gp_davey, framebuffer, DRW_TEST_IMAGE_RES_H, DRW_TEST_IMAGE_RES_H,
                             DRW_TEST_IMAGE_RES_V, d2_mode_rgb888);
    error_handler(DRW_err, gp_davey);
...
static void drw_set(uint8_t * framebuffer)
{
    ...
    /* Set the frame buffer */
    DRW_err = d2_framebuffer(gp_davey, framebuffer, DRW_TEST_IMAGE_RES_H, DRW_TEST_IMAGE_RES_H,
                             DRW_TEST_IMAGE_RES_V, d2_mode_rgb565);
    error_handler(DRW_err, gp_davey);
...
/*******************************************************************************************************************//**
 * @brief      User defined function to handle animation of different figures.
 * @param      None
 * @retval     None
 **********************************************************************************************************************/
static void drw_animation(void)
