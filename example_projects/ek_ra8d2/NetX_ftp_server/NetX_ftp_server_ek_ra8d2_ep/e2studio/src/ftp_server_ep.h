...
#define HANDLE_API_FAILURE_IF_FAIL(api_ret, err, msg)   ({\
		                                                if (((api_ret) == (err)))\
														{\
		                                                    FSP_PARAMETER_NOT_USED ((api_ret));\
														}\
														else\
														{\
														    PRINT_ERR_STR((msg));ERROR_TRAP((api_ret));\
														}\
                                                        })
...
#define EP_INFO                 "\r\nThis project demonstrates the basic functionality of NetX Duo FTP Server\r\n"\
                                ...
                                "FTP Server is created, the user can communicate through server through valid\r\n"\
								"FTP Client. Error and info messages will be printed on J-Link RTT Viewer.\r\n\n\n"

#define MIN_FILE_NAME_LENGTH        (2U)

#define LOGIN_EVENT_FLAG            ((UINT)(1<<2))
#define DEFAULT_SIZE                ((ULONG)40)
#define SAMPLE_DATA_BYTES           {82, 101, 110, 101, 115, 97, 115, 95, 102, 116, 112, 95, 115, 101, 114, 118,\
		                             101, 114, 69, 112}
