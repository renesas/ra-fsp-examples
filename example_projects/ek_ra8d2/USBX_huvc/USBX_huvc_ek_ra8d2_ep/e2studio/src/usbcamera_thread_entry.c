...
* @param[in]  UX_TRANSFER *transfer_request.
* @retval     None
***********************************************************************************************************************/
VOID uvc_transfer_request_done_callback(UX_TRANSFER *transfer_request)
{
...
* @param[in]  UCHAR **p_mjpeg,  UINT img_size.
...
* @param[in]  event       The event code (ULONG).
* @param[in]  host_class  Pointer to the UX_HOST_CLASS structure.
* @param[in]  instance    Pointer to instance data.
...
/*******************************************************************************************************************//**
* @brief      This function is invoked to start or stop video capture based on user input from the RTT Viewer.
* @param[in]  None
* @retval     None
***********************************************************************************************************************/
static void usbx_huvc_operation(void)
{
    ...
    /* Receive the user input from RTT Viewer */
...
/*******************************************************************************************************************//**
* @brief      This function waits for camera insertion and sets up the video channel.
* @param[in]  None
* @retval     None
**********************************************************************************************************************/
static void wait_for_camera_insertion(void)
{
    ...
    tx_event_flags_get(&g_device_insert_eventflag, thread_data->device_inserted_eventflag,\
                       TX_OR, (ULONG*)&actual_flags, TX_WAIT_FOREVER);
    /* This delay is required for now to get valid ISO IN UX_ENDPOINT instance */
    tx_thread_sleep (WAIT_100_TICKS);

    /* Set camera parameters */
     status = ux_host_class_video_frame_parameters_set(thread_data->video_host_class,\
                                                       UX_HOST_CLASS_VIDEO_VS_FORMAT_MJPEG, VIDEO_RESOLUTION_X,\
                                                       VIDEO_RESOLUTION_Y, VIDEO_FRAME_INTERVAL_30FPS);
    ...
    /* Clear semaphore to zero; this ensures that while reading transfer requests, we don't trick ourselves into
     * thinking we read one */
...
void usbcamera_thread_entry(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        /* Un-initializes the host stack for USB */
    ...
    PRINT_INFO_STR("RA USBX Host UVC has been successfully initialized");
    ...
    /*  Wait until USB video camera is inserted */
    wait_for_camera_insertion();
    while (true)
    {
        /* Wait until device is inserted if USB is unplugged */
        tx_event_flags_get(&g_device_insert_eventflag, thread_data->device_inserted_eventflag, TX_OR,\
                            (ULONG*) &actual_flags, TX_NO_WAIT);
        ...
        if (STOP != g_video_status)
        {
            ...
            if (ZERO != thread_data->completed_transfer_length[completed_transfer_index])
            {
                ...
                jpeg_stream.p_pointer = thread_data->video_data_ptrs[completed_transfer_index]
                                        + UVC_VIDEO_PAYLOAD_HEADER_SIZE;
                jpeg_stream.size = thread_data->completed_transfer_length[completed_transfer_index]
                                   - UVC_VIDEO_PAYLOAD_HEADER_SIZE;
...
/*******************************************************************************************************************//**
 * @brief       This function closes the opened USB instance and initializes the host stack for USB.
 * @param[in]   None
 * @retval      None
 **********************************************************************************************************************/
static void usb_deinit(UINT err, const char *err_str)
