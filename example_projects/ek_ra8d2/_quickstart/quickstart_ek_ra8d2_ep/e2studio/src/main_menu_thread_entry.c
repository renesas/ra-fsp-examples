...
void main_menu_thread_entry(void *pvParameters)
{
    ...
    printf_colour("[WHITE]Welcome to Quick Start Example Project for %s!\r\n", FULL_NAME);
    printf_colour("Configuring connected devices, please wait\r\n");

    common_init();

    /* Import graphics to SDRAM */
    memcpy(&g_sdram_buffer_welcome[0], &g_ref_000_welcome_screen.pixel_data[0], DISPLAY_BUFFER_STRIDE_BYTES_INPUT1 * DISPLAY_VSIZE_INPUT1 * 2 + 1);
    memcpy(&g_sdram_buffer_led[0], &g_ref_001_user_led_background.pixel_data[0], DISPLAY_BUFFER_STRIDE_BYTES_INPUT1 * DISPLAY_VSIZE_INPUT1 * 2 + 1);
