...
3) Operation is not guaranteed for any user input value other than integer, char (e.g., float, special char)
...
5) The user is expected to set RTC calendar Date and Time (option 1) before other operations.

6) The user is expected to enter Time values to set calendar date and time (Option 1) as shown below.
	Sample input: 23:10:2019 16:14:55 (Format: DD:MM:YYYY HH:MM:SS)

7) For RTC: The user is expected to enter Second values to set calendar alarm date and time (Option 2) as shown below.
	Sample input: 35 (Format: SS).
   For RTC_C: The user is expected to enter Hour and Minute values to set calendar alarm date and time (Option 2) as
   shown below.
	Sample input: 15:24 (Format: HH:MM).

8) For RTC_C: Alarm and periodic interrupts are retained after pressing the reset pin.
