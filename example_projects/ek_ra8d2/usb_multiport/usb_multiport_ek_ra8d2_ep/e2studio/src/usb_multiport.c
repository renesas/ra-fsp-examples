...
void usb_read_operation(void)
{
    ...
    if (true == check_usb_connected())
    {
            ...
            while (!ff_feof(file_pointer))
            {
                ...
                if (READ_WRITE_FAILURE == bytes_read)
                {
                    ...
                    if (SUCCESS != file_error)
                    {
                        ...
                        APP_PRINT("FreeRTOS FAT error code: %d\r\n", stdioGET_ERRNO());
                    }
                    return;
                }
        ...
        {
            APP_ERR_PRINT("File does not exist or ff_fopen API failed.\r\n");
            /* Add extra %d as parses string and prints %d as-is */
            APP_PRINT("FreeRTOS FAT error code: %d\r\n", stdioGET_ERRNO());
            APP_PRINT("Please perform the write operation before the read operation.\r\n");
            return;
        }
    }
    else
    {
        APP_PRINT("USB is not detected.\n");
    }
}
