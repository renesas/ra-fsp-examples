/***********************************************************************************************************************
 * File Name    : phid_handler_thread_entry.c
 * Description  : Contains functions from the phid handler thread.
 **********************************************************************************************************************/
/***********************************************************************************************************************
...
static void usb_status_update(void)
{
    ...
    else if (USB_SET_REPORT == (p_usb_basic_event->setup.request_type & USB_BREQUEST))
    {
        /* Print Caps Lock status */
        if (g_key_numlock & 0x02)
        {
            APP_PRINT("\r\nCaps Lock is ON");
        }
        else
