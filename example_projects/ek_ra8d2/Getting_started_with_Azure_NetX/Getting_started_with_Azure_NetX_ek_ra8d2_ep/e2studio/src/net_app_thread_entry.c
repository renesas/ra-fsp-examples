...
static UINT server_request_callback(NX_WEB_HTTP_SERVER *p_server, UINT request_type, CHAR * p_resource,\
                                    NX_PACKET * p_packet);
...
static uint8_t g_web_http_server0_stack_memory[G_WEB_HTTP_SERVER0_STACK_SIZE] BSP_PLACE_IN_SECTION\
                                              (".stack.g_web_http_server0") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
/*******************************************************************************************************************//**
 * @brief     This function is triggered whenever a FileX block media event occurs.
...
/*******************************************************************************************************************//**
 * @brief     This function runs indefinitely, waiting for user input from the terminal, and performs the
 *            corresponding network operation (TCP client, UDP client, DNS client, Web server).
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
void net_app_thread_entry(void)
{
    ...
    TERM_ERR_TRAP(NX_SUCCESS != status, status, "net_app_create failed");
    ...
    while (true)
    {
        ...
        select = RESET_VALUE;
        sscanf(input, "%d %63s", &select, argv);
...
/*******************************************************************************************************************//**
 * @brief     This function creates network application components, including TCP and UDP sockets, DNS instance,
 *            and a web HTTP server.
 * @param[in] None.
...
static UINT net_app_create(void)
{
    ...
    APP_ERR_RET(NX_SUCCESS != status, status, "nx_tcp_socket_create failed");
    ...
    APP_ERR_RET(NX_SUCCESS != status, status, "nx_tcp_client_socket_bind failed");
    ...
    APP_ERR_RET(NX_SUCCESS != status, status, "nx_udp_socket_create failed");
    ...
    APP_ERR_RET(NX_SUCCESS != status, status, "web_http_server_file_system_create failed");
    ...
    APP_ERR_RET(NX_SUCCESS != status, status, "nx_web_http_server_create failed");

    return status;
}

/*******************************************************************************************************************//**
 * @brief     This function initializes the FileX block media driver and opens the media for file system operations.
 *            It returns the status of these operations.
 * @param[in] None.
...
static UINT file_system_create(void)
{
    ...
    APP_ERR_RET(FX_SUCCESS != status, status, "RM_FILEX_BLOCK_MEDIA_Open failed");
    ...
    APP_ERR_RET(FX_SUCCESS != status, status, "fx_media_open failed");

    return status;
}

/*******************************************************************************************************************//**
 * @brief     This function performs TCP client operations including connecting to a server, sending data,
...
static UINT tcp_client_operation(CHAR * p_input)
{
    ...
    status = nx_packet_data_append(p_packet, NET_TCP_CLIENT_SEND_DATA, sizeof(NET_TCP_CLIENT_SEND_DATA),\
                                   &g_packet_pool0, NX_IP_PERIODIC_RATE);
    if (NX_SUCCESS != status)
    {
...
/*******************************************************************************************************************//**
 * @brief     This function performs UDP client operations including sending data to a server and receiving a response.
 * @param[in] p_input   A string containing the server IP address and port in the format "x.x.x.x:port".
 * @retval    NX_SUCCESS on successful operation, other error codes otherwise.
 **********************************************************************************************************************/
static UINT udp_client_operation(CHAR * p_input)
{
    ...
    /* Write fixed data into the packet payload */
    status = nx_packet_data_append(p_packet, NET_UDP_CLIENT_SEND_DATA, sizeof(NET_UDP_CLIENT_SEND_DATA),\
                                   &g_packet_pool0, NX_IP_PERIODIC_RATE);
    if (NX_SUCCESS != status)
    {
...
/*******************************************************************************************************************//**
 * @brief     This function performs DNS client operations, including resolving host name to IP addresses and
 *            vice versa.
 * @param[in] p_input   A string containing either a host name or an IP address in the format "x.x.x.x".
 * @retval    NX_SUCCESS on successful operation, other error codes otherwise.
 **********************************************************************************************************************/
static UINT dns_client_operation(CHAR * p_input)
{
    ...
    TERM_INFO_RET(NX_SUCCESS != status, "The Internet connection is not available");
    ...
    if (NX_TRUE == request)
    {
        /* Perform a DNS query to resolve a given host name into its corresponding IP address */
        status = nx_dns_host_by_name_get(&g_dns_client, (UCHAR *)p_input, &ipv4, NET_DNS_CLIENT_WAIT);
        TERM_INFO_RET(NX_SUCCESS != status, "Resolving the IP address by the host name failed");
    ...
    else
    {
        ...
        /* Clean the buffer that stores the host name */
        memset(name, RESET_VALUE, sizeof(name));

        /* Perform a DNS query to determine the host name associated with a given IP address */
        status = nx_dns_host_by_address_get(&g_dns_client, ipv4, (UCHAR *)name, sizeof(name), NET_DNS_CLIENT_WAIT);
        TERM_INFO_RET(NX_SUCCESS != status, "Resolving the host name by the host's IP address failed");
...
/*******************************************************************************************************************//**
 * @brief     This function starts the web server, waits for user input, and then stops the server.
 * @param[in] None.
 * @retval    NX_SUCCESS on successful operation, other error codes otherwise.
 **********************************************************************************************************************/
static UINT web_server_operation(void)
...
/*******************************************************************************************************************//**
 * @brief     This function processes incoming HTTP GET requests and sends appropriate responses
...
/*******************************************************************************************************************//**
 * @brief     This function generates and sends an HTTP response containing the current network configuration,
...
static UINT server_callback_response_net_info(NX_WEB_HTTP_SERVER * p_server, CHAR * p_resource)
{
    ...
    length = (UINT)snprintf(content_body, sizeof(content_body), JSON_NET_INFO,
                            g_network_info.phy_addr.block.a, g_network_info.phy_addr.block.b,
                            g_network_info.phy_addr.block.c, g_network_info.phy_addr.block.d,
                            g_network_info.phy_addr.block.e, g_network_info.phy_addr.block.f,
...
/*******************************************************************************************************************//**
 * @brief     This function processes the client request, updates the content body with the LED status,
...
static UINT server_callback_response_led_status(NX_WEB_HTTP_SERVER * p_server, CHAR * p_resource)
{
    ...
    length = (UINT)snprintf(content_body, sizeof(content_body), JSON_LED_STATUS,
...
/*******************************************************************************************************************//**
 * @brief     This function generates and sends an HTTP response indicating the status of a switch request.
 * @param[in] p_server      Pointer to the web server instance.
 * @param[in] p_resource    Pointer to the requested resource string.
 * @retval    NX_SUCCESS on successful operation, other error codes otherwise.
 **********************************************************************************************************************/
