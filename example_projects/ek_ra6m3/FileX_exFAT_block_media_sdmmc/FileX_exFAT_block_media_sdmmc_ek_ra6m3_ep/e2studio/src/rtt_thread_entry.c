...
void rtt_thread_entry(void)
{
    ...
    {
        APP_PRINT("\r\nERROR: rtt_frame_work_init failed\r\n");
        APP_ERR_TRAP(tx_status);
    }

    /* Send a start event to the Console thread */
    tx_status = (ULONG)tx_event_flags_set (&g_request_event,
...
UINT rtt_framework_init(void)
{
    ...
    tx_status = tx_byte_pool_create(&g_rtt_byte_pool, RTT_BYTE_POOL_NAME, (VOID *)g_byte_pool_memory, RTT_BYTE_POOL_SIZE);
    if (TX_SUCCESS != tx_status)
    {
        return tx_status;
    }
...
UINT rtt_input_handle(void)
{
    ...
    if(APP_CHECK_DATA)
    {
        ...
        tx_status = tx_byte_allocate(&g_rtt_byte_pool, (VOID **)&p_rtt_msg, sizeof(rtt_msg_t) + rtt_len + RTT_INPUT_ALIGN , TX_WAIT_FOREVER);
        if (TX_SUCCESS != tx_status)
        {
            ...
            switch (p_rtt_msg->id)
            {
                ...
                case RTT_OUTPUT_APP_MENU:
                    ...
                    case RTT_OUTPUT_APP_MEDIA_PROPERTY:
                        ...
                        conv_ul64_to_str (media.total_zise, str_size, STR_UL64_MAX_LEN);
