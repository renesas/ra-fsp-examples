...
void hal_entry(void)
{
    ...
    time_t rawtime = RESET_VALUE;
    struct tm * time_info = {RESET_VALUE};
    ...
    /* The usage of time related APIs from time.h (time(), localtime(), and asctime()) are for demonstration purpose.
     * While implementing stand-alone application, user needs to avoid these APIs or have the proper implementation to take care of this.
     * For now, this will work only in the Debugger environment.
     * Note: If you flash the code and connect the RTT alone, it will be stuck in the loop waiting for the time();
     *       In the Debugger mode this works, as the Debugger takes care of the syscal to get the time. */

    /* Calculate current time in seconds since epoch */
    time(&rawtime);
    /* Convert seconds value to calendar date and time. Default time-zone: GMT */
    time_info = localtime(&rawtime);
    /* Prints local Calendar Date and Time referenced to GMT. */
    APP_PRINT("\r\nCurrent local date and time referenced to GMT: %s", asctime(time_info));
