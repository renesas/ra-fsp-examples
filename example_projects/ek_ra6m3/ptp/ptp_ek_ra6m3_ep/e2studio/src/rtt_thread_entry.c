    ...
    do
    {
        ...
        if(err == TX_SUCCESS)
        {
            ...
            switch(rtt_op_data->msg_id)
            {
                ...
                {
                    local_ptp_time_t print_time = {RESET_VALUE};
                    memcpy(&print_time, (local_ptp_time_t *)rtt_op_data->p_msg, sizeof(local_ptp_time_t));
                    /* Print Date and Time in user readable format.*/
                    APP_PRINT("Date : %d/%02d/%02d \nTime : %02d : %02d : %02d.%09ld \r\n", print_time.local_time.tm_year,
                            print_time.local_time.tm_mon , print_time.local_time.tm_mday, print_time.local_time.tm_hour,
                            print_time.local_time.tm_min, print_time.local_time.tm_sec, print_time.tm_nsec);
                }
                break;
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_IP:
                {
                    ...
                    APP_PRINT("\r\nINFO : RA board IP address is : %s", ipv4_address);
                }
                break;
