...
The EP will perform different low power mode transitions based on the user's selection of Low Power Mode, request, and cancel sources in the RA Configurator.
...
Turn off the blue LED and display the LPM Mode Name before entering each LPM mode. Turn on the blue LED and display the cancel source name after exiting each LPM mode.
The EP log will print to the Host PC via the UART interface at a baud rate of 115200 bps. The red LED will turn on if an error occurs.
...
3. Hardware and software setups:
	The pin configuration file for LPM mode in each example project is named with the suffix "LPM". you can change these configurations 
...
4. 	The hardware modifications required for measuring the MCU's current.
	...
	EK-RA2L1, EK-RA2E1, EK-RA2E2, EK-RA4M2, EK-RA4M3, EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA6M5, EK-RA8M1, FPB-RA2E3, EK-RA8D1 kit:
