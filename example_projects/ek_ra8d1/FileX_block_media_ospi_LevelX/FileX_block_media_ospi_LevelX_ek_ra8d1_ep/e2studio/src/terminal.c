 ...
 *  Description  : SCI UART callback function.
 ...
 *  Description  : This function initializes the SCI UART module which transfer the EP log to the host PC.
 *  Arguments    : None
 ...
 *  Description  : This function de-initializes the SCI UART module.
 *  Arguments    : None
 *  Return Value : None
 **********************************************************************************************************************/
...
/***********************************************************************************************************************
 *  Function Name: terminal_framework_init
 *  Description  : This function allocates heap memory for the terminal frame.
 *  Arguments    : None
 *  Return Value : None
 **********************************************************************************************************************/
...
/***********************************************************************************************************************
 *  Function Name: terminal_printf
 *  Description  : This function transfers a message to the host PC.
...
/***********************************************************************************************************************
 *  Function Name : terminal_get_input_queue
 *  Description   : This function gets and converts data to an integer from the terminal input queue.
 *                  To be called by the User/Application Thread.
 *  Arguments[out]: p_data         Pointer to data buffer
...
UINT terminal_get_input_queue(uint16_t * p_data)
{
    ...
    if (TERMINAL_INPUT_MESSAGE == p_terminal_msg->id)
    {
        ...
        *p_data =  (uint16_t)(atoi(p_terminal_msg->msg));
    }
...
/***********************************************************************************************************************
 *  Function Name: terminal_send_output_queue
 *  Description  : This function prepares the terminal message structure and sends it to the terminal output queue.
 *  Arguments[in]: id            Message id
 *                 size          Message size
 *                 p_data        Pointer to the message string
 *  Return Value : TX_SUCCESS    Upon successful operation
 *                 Any Other Error code apart from TX_SUCCESS
 **********************************************************************************************************************/
UINT terminal_send_output_queue(terminal_event_t id, uint32_t size, void * const p_data)
{
    ...
    status = tx_byte_allocate(&g_terminal_byte_pool, (VOID **)&p_terminal_msg, sizeof(terminal_msg_t) + size, TX_WAIT_FOREVER);
    if (TX_SUCCESS != status)
    {
        return status;
    }
...
/***********************************************************************************************************************
 *  Function Name: terminal_input_handle
 *  Description  : This function takes input data from the PC then puts it into the terminal input queue.
 *  Arguments    : None
 *  Return Value : TX_SUCCESS    Upon successful operation
 *                 Any Other Error code apart from TX_SUCCESS
 **********************************************************************************************************************/
...
#endif
        /* Allocate memory for terminal message structure and input message */
        status = tx_byte_allocate(&g_terminal_byte_pool, (VOID **)&p_terminal_msg, sizeof(terminal_msg_t) + sizeof(g_rx_buffer), TX_WAIT_FOREVER);
        ...
        if (TX_SUCCESS == status)
        {
            switch (p_terminal_msg->id)
            {
                ...
                case TERMINAL_OUTPUT_APP_MENU:
                    ...
                    terminal_printf(gp_menu_content[menu]);
                    break;
                ...
                case TERMINAL_OUTPUT_APP_MEDIA_PROPERTY:
                    ...
                    terminal_printf("Format type of flash is:    %s", media.format_type);
...
/***********************************************************************************************************************
 *  Function Name: terminal_thread_init_check
 *  Description  : This function waits for the terminal thread to initialize and sleep before sending data to it.
 *  Arguments    : None
 *  Return Value : None
 **********************************************************************************************************************/
