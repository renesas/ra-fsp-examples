...
#include <ai_apps/common/log_macros.h>
...
namespace app {
            ...
            uint32_t topNCount, bool useSoftmax)
    {
        if (outputTensor == nullptr) {
            error("Output vector is null pointer.\n");
            return false;
        }
        ...
        if (totalOutputSize < topNCount) {
            error("Output vector is smaller than %" PRIu32 "\n", topNCount);
            return false;
        } else if (topNCount == 0) {
            error("Top N results cannot be zero\n");
            return false;
        }
        ...
        switch (outputTensor->type) {
            ...
            default:
                error("Tensor type %s not supported by classifier\n",
        ...
        if (!resultState) {
            error("Failed to get top N results set\n");
            return false;
        }
