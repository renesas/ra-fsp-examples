...
#include <ai_apps/common/tensorflow/lite/micro/micro_graph.h>
...
// struct for TFLM.
    ...
    inline TfLiteStatus MicroContextRequestScratchBufferInArena (TfLiteContext * ctx, size_t bytes, int * buffer_idx) {
    ...
    inline TfLiteEvalTensor * MicroContextGetEvalTensor (const struct TfLiteContext * context, int tensor_idx) {
        return GetMicroContext(context)->GetEvalTensor(tensor_idx);
    }

    inline TfLiteExternalContext * MicroContextGetExternalContext (TfLiteContext           * context,
