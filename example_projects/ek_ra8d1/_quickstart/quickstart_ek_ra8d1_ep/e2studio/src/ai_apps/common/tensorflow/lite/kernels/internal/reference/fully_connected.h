...
namespace tflite {
    namespace reference_ops {
        inline void FullyConnected (const FullyConnectedParams &params,
            ...
            const int output_depth       = MatchingDim(weights_shape,
                                                       weights_dims_count - 2,
                                                       output_shape,
                                                       output_dims_count - 1);
            ...
            for (int b = 0; b < batches; ++b)
            {
                for (int out_c = 0; out_c < output_depth; ++out_c)
                {
                    ...
                    output_data[out_c + output_depth * b] = ActivationFunctionWithMinMax(total + bias_value,
        ...
        inline void FullyConnected (const FullyConnectedParams &params,
            ...
            const int output_depth     = MatchingDim(filter_shape,
                                                     filter_dim_count - 2,
                                                     output_shape,
                                                     output_dim_count - 1);
        ...
        inline void ShuffledFullyConnected (const FullyConnectedParams &params,
                                            const RuntimeShape         &input_shape,
                                            const uint8_t             * input_data,
                                            const RuntimeShape         &weights_shape,
                                            const uint8_t             * shuffled_weights_data,
                                            const RuntimeShape         &bias_shape,
                                            const int32_t             * bias_data,
                                            const RuntimeShape         &output_shape,
                                            int16_t                   * output_data,
                                            uint8_t                   * shuffled_input_workspace_data) {
            const int32_t output_multiplier     = params.output_multiplier;
            const int     output_shift          = params.output_shift;
            const int32_t output_activation_min = params.quantized_activation_min;
            const int32_t output_activation_max = params.quantized_activation_max;
