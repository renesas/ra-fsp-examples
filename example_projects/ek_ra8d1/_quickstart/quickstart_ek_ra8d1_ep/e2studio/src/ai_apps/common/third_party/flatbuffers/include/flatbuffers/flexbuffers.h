...
#include <ai_apps/common/third_party/flatbuffers/include/flatbuffers/util.h>
#include <map>
...
// type.
    ...
    inline Type ToTypedVector (Type t, size_t fixed_len=0) {
        ...
        switch (fixed_len)
        {
            ...
            default:
                FLATBUFFERS_ASSERT(0); return FBT_NULL;
        }
    }
...
// TODO(wvo): Could introduce an #ifdef that makes this into an assert?
    ...
    inline Reference Map::operator[] (const char * key) const {
        ...
        switch (keys.byte_width_)
        {
            ...
            default:
                FLATBUFFERS_ASSERT(false); return Reference();
        }
...
private:
        ...
        static BitWidth WidthB (size_t byte_width) {
            switch (byte_width)
            {
                ...
                default:
                    FLATBUFFERS_ASSERT(false); return BIT_WIDTH_64;
            }
        }
...
private:
        ...
        size_t ScalarVector (const T * elems, size_t len, bool fixed) {
            ...
            stack_.push_back(Value(static_cast < uint64_t > (vloc), ToTypedVector(vector_type, fixed ? len : 0),
                                   bit_width));

            return vloc;
        }

        Value CreateVector (size_t        start,
                            size_t        vec_len,
                            size_t        step,
                            bool          typed,
                            bool          fixed,
                            const Value * keys=nullptr) {
            FLATBUFFERS_ASSERT(!fixed ||
...
#define FLEX_CHECK_VERIFIED(P, PACKED_TYPE)                         \
    if (reuse_tracker_) {                                           \
        ...
        bool VerifyVector (Reference r, const uint8_t * p, Type elem_type) {
            ...
            FLEX_CHECK_VERIFIED(p - size_byte_width, PackedType(Builder::WidthB(size_byte_width), r.type_));
        ...
        bool VerifyKeys (const uint8_t * p, uint8_t byte_width) {
            ...
            return VerifyVector(Reference(p, byte_width, key_byte_with, FBT_VECTOR_KEY), p - off, FBT_KEY);
        }
...
// parameters.
    inline bool VerifyBuffer (const uint8_t * buf, size_t buf_len, std::vector < uint8_t > * reuse_tracker=nullptr) {
