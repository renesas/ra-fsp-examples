...
#include <ai_apps/common/tensorflow/lite/core/c/builtin_op_data.h>
...
// initial output value is not exceptionally large.
        void MatrixScalarMultiplyAccumulate(const int8_t * matrix,
                                            int32_t        scalar,
                                            int32_t        n_row,
                                            int32_t        n_col,
                                            int32_t      * output);
...
// Cwise product of a vector and a batch-vector.
        template < typename T >
        inline void VectorBatchVectorCwiseProduct (const T * vector,
                                                   int       v_size,
...
// values.
        template < typename T >
        inline void VectorBatchVectorCwiseProductAccumulate (const T * vector,
                                                             int       v_size,
                                                             const T * batch_vector,
                                                             int       n_batch,
                                                             T       * result) {
...
// scaling factor used to quantize the values.
        void SymmetricQuantizeFloats(const float * values,
                                     const int     size,
...
// appropriate scaling factor to quantize the values.
        ...
        void AsymmetricQuantizeFloats(const float * values,
                                      const int     size,
                                      int8_t      * quantized_values,
                                      float       * scaling_factor,
                                      int32_t     * offset);
...
// do_asymmetric      controls if the quantization should be asymmetric.
        inline void BatchQuantizeFloats (const float * float_data_ptr,
                                         int           n_batch,
                                         int           n_data,
                                         int8_t      * quantized_data_ptr,
            ...
            for (int b = 0; b < n_batch; ++b)
            {
                ...
                if (do_asymmetric)
                {
                    tensor_utils::AsymmetricQuantizeFloats(float_data_ptr + offset,
                                                           n_data,
                                                           quantized_data_ptr + offset,
                                                           &scaling_factors[b],
                                                           &zero_points[b]);
                }
                else
                {
                    ...
                    tensor_utils::SymmetricQuantizeFloats(float_data_ptr + offset,
                                                          n_data,
                                                          quantized_data_ptr + offset,
                                                          &unused_min,
                                                          &unused_max,
...
// case) so that there's no incomplete block.
        void SparseMatrixBatchVectorMultiplyAccumulate1x4(const float * __restrict__   matrix,
...
// 2. m_cols < 254 * 16 so that block index can be represented by uint8.
        void SparseMatrixBatchVectorMultiplyAccumulate(const float * __restrict__   matrix,
                                                       const uint8_t * __restrict__ ledger,
                                                       int                          m_rows,
                                                       int                          m_cols,
                                                       const float * __restrict__   vector,
                                                       int                          n_batch,
...
// input, output and filter are zero.
        void SparseMatrixBatchVectorMultiplyAccumulate1x16(const int8_t * __restrict__  matrix,
                                                           const int32_t * __restrict__ segments,
                                                           const int32_t * __restrict__ indices,
                                                           int                          m_rows,
                                                           int                          m_cols,
                                                           const int8_t * __restrict__  vector,
                                                           const int32_t * __restrict__ bias_vector,
                                                           int                          n_batch,
...
// 1. m_cols is a multiple of 16 so that all blocks are full blocks.
// 2. m_cols < 254 * 16 so that block index can be represented by uint8.
        void SparseMatrixBatchVectorMultiplyAccumulate(const int8_t * __restrict__  matrix,
                                                       const uint8_t * __restrict__ ledger,
                                                       const int                    m_rows,
                                                       const int                    m_cols,
                                                       const int8_t * __restrict__  vectors,
...
// prepare() function.
        void MatrixBatchVectorMultiply(const int8_t * input,
...
// - output:  the 16 bit output
        void ApplyLayerNorm(const int16_t * input,
                            const int16_t * layer_norm_weights,
                            const int32_t * bias,
                            int32_t         layer_norm_scale_a,
                            int32_t         layer_norm_scale_b,
                            int32_t         variance_limit,
                            int             n_batch,
                            int             n_input,
                            int16_t       * output);
...
// The input is in Qm.15-m format and the output is in Q0.15 format.
        void ApplyTanh(int32_t intger_bits, const int16_t * input, int32_t n_batch, int32_t n_input, int16_t * output);
...
// - Output has Q0.15 as scale.
        void ApplyTanhFloat(const int16_t * input,
                            int32_t         n_batch,
                            int32_t         n_input,
...
// Output does not need to be initialized.
        void CwiseMul(const int16_t * input_1,
...
// - output:     the 8 bit output of size n_batch * n_input.
// Output does not need to be initialized.
        void CwiseAdd(const int16_t * input_1, const int16_t * input_2, int n_batch, int n_input, int16_t * output);
...
// x_nbatch_1 * y_nbatch_1 + ... + x_nbatch_vsize * y_nbatch_vsize]
        template < typename T >
        inline void BatchVectorBatchVectorDotProduct (const T * vector1,
                                                      const T * vector2,
                                                      int       v_size,
                                                      int       n_batch,
                                                      T       * result) {
...
// Same as above, but inputs are 16bit integer and output is 16bit integer.
        void VectorBatchVectorCwiseProductAccumulate(const int16_t * vector,
                                                     int             v_size,
...
// added to get one element of output.
        void ReductionSumVector(const float * input_vector, float * output_vector, int output_size, int reduction_size);

// Same as above but input/output is 32 bit integer.
        void ReductionSumVector(const int32_t * input_vector,
                                int32_t       * output_vector,
                                int             output_size,
                                int             reduction_size);

// Same as above but input is 8 bit integer.
        void ReductionSumVector(const int8_t * input_vector,
                                int32_t      * output_vector,
                                int            output_size,
                                int            reduction_size);
...
// Saturate Add with rescale on both inputs.
        void TwoGateSaturatingAdd(const int8_t * input,
                                  int8_t         input_zp,
                                  const int8_t * recurrent,
                                  int8_t         recurrent_zp,
...
// scaling_factor_scratch buffer.
        inline void MatrixBatchVectorMultiplyAccumulate (const int8_t * __restrict__ matrix,
                                                         const int                   m_rows,
                                                         const int                   m_cols,
                                                         const int8_t * __restrict__ vectors,
                                                         const float                 matrix_scaling_factor,
                                                         const float               * vector_scaling_factors,
                                                         int                         n_batch,
                                                         float * __restrict__        result,
                                                         const float               * per_channel_scale,
                                                         const int32_t             * input_offset,
            ...
            MatrixBatchVectorMultiplyAccumulate(matrix,
                                                m_rows,
                                                m_cols,
                                                vectors,
                                                scaling_factor_scratch,
                                                n_batch,
                                                result,
                                                per_channel_scale,
                                                input_offset,
