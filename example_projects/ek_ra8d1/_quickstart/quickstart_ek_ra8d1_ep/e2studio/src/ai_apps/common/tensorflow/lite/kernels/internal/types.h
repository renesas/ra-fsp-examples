...
#include <ai_apps/common/tensorflow/lite/kernels/internal/runtime_shape.h>
#include <algorithm>
#include <cstdint>
#include <cstring>
...
// TODO(kanlig): uses Dims to represent dimensions.
    inline size_t ReducedOutputOffset (const int   num_dims,
                                       const int * dims,
                                       const int * index,
                                       const int   num_axis,
                                       const int * axis) {
...
// Get common shape dim, DCHECKing that they all agree.
    inline int MatchingDim (const RuntimeShape &shape1, int index1, const RuntimeShape &shape2, int index2) {
...
// arrays.
    ...
    template < int N >
    inline int MatchingFlatSize (const Dims < N > &dims,
...
// Flat size calculation, checking if their extended shapes match.
    inline int MatchingExtendedShapeFlatSize (const RuntimeShape &shape, const RuntimeShape &check_shape_0) {
        ...
        {
            TFLITE_DCHECK_EQ(shape.Dims(shape_dims - 1 - i), check_shape_0.Dims(check_shape_0_dims - 1 - i));
        }
    ...
    inline int MatchingExtendedShapeFlatSize (const RuntimeShape &shape,
                                              ...
                                              const RuntimeShape &check_shape_3) {
        const int flat_size = MatchingExtendedShapeFlatSize(shape, check_shape_0);
        TFLITE_DCHECK_EQ(MatchingExtendedShapeFlatSize(shape, check_shape_1, check_shape_2, check_shape_3), flat_size);
...
// A combination of MatchingFlatSize() and FlatSizeSkipDim().
    ...
    template < int N >
    inline int MatchingFlatSizeSkipDim (const Dims < N > &dims,
                                        int               skip_dim,
                                        const Dims < N > &check_dims_0,
                                        const Dims < N > &check_dims_1) {
        for (int i = 0; i < N; ++i)
...
// A combination of MatchingFlatSize() and FlatSizeSkipDim().
    inline int MatchingFlatSizeSkipDim (const RuntimeShape &shape, int skip_dim, const RuntimeShape &check_shape_0) {
        const int dims_count = shape.DimensionsCount();
        for (int i = 0; i < dims_count; ++i)
        {
            if (i != skip_dim)
    ...
    inline int MatchingFlatSizeSkipDim (const RuntimeShape &shape,
                                        int                 skip_dim,
                                        const RuntimeShape &check_shape_0,
                                        const RuntimeShape &check_shape_1) {
        const int dims_count = shape.DimensionsCount();
