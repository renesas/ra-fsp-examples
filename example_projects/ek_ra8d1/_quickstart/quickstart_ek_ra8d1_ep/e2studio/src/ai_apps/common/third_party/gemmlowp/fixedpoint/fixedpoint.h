...
#include <ai_apps/common/third_party/gemmlowp/internal/detect_platform.h>
#include <algorithm>
#include <cassert>
#include <cmath>
#include <cstdint>
...
// So far this is only needed for int16.
    ...
    inline std::int16_t SaturatingAdd (std::int16_t a, std::int16_t b) {
        ...
        return static_cast < std::int16_t > (
            std::min(static_cast < std::int32_t > (32767), std::max(static_cast < std::int32_t > (-32768), sum)));
    }
...
public:
        ...
        static FixedPoint FromDouble (double x) {
            ...
            return FromScalarRaw(static_cast < ScalarRawType >
                                 (std::min(std::max(round(x * static_cast < double > (1ll << kFractionalBits)),
                                                    min_bound),
                                           max_bound)));
        }
...
#define MAKE_FIXEDPOINT_BINARY_FUNC(FuncName, ImplFuncName)                                     \
    template < typename tRawType, int tIntegerBits >                                            \
    FixedPoint < tRawType, tIntegerBits > FuncName(FixedPoint < tRawType,                       \
                                                   tIntegerBits > a,                            \
                                                   FixedPoint < tRawType,                       \
                                                   tIntegerBits > b) {                          \
...
#undef MAKE_FIXEDPOINT_BINARY_FUNC_RETURNING_RAW
    ...
    template < typename tRawType, int tIntegerBits >
    FixedPoint < tRawType, tIntegerBits > SelectUsingMask(tRawType if_mask,
                                                          FixedPoint < tRawType,
                                                          tIntegerBits > then_val,
                                                          FixedPoint < tRawType,
                                                          tIntegerBits > else_val)
    {
        return FixedPoint < tRawType,
               tIntegerBits > ::FromRaw(SelectUsingMask(if_mask, then_val.raw(), else_val.raw()));
    }
    ...
    template < typename tRawType, int tIntegerBits >
    FixedPoint < tRawType, tIntegerBits > SaturatingAdd(FixedPoint < tRawType,
                                                        tIntegerBits > a,
                                                        FixedPoint < tRawType,
                                                        tIntegerBits > b)
    {
    ...
    template < typename tRawType, int tIntegerBits >
    FixedPoint < tRawType, tIntegerBits > AddSaturatingIf16Bit(FixedPoint < tRawType,
                                                               tIntegerBits > a,
                                                               FixedPoint < tRawType,
                                                               tIntegerBits > b)
    {
...
// rounding-shifted by 16 bits to the right.
 ...
 #define GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPointType,                                \
                                              ScalarRawInt32Value, DoubleValue)              \
    (FixedPointType::FromScalarRaw(gemmlowp::RescaleConstantInitializer < FixedPointType > ( \
                                       ScalarRawInt32Value)))
#endif
...
#define GEMMLOWP_EXP_BARREL_SHIFTER(Exponent, FixedPointMultiplier)                                           \
    if (kIntegerBits > Exponent) {                                                                            \
        const ResultF kMultiplier = GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(ResultF,                             \
                                                                         FixedPointMultiplier,                \
                                                                         std::exp(-std::pow(2.0, Exponent))); \
...
#ifdef GEMMLOWP_NEON
 #include <ai_apps/common/third_party/gemmlowp/fixedpoint/fixedpoint_neon.h>
#elif defined(GEMMLOWP_AVX2)
 #include "./fixedpoint_avx.h"
#elif defined(GEMMLOWP_SSE4)
 #include <ai_apps/common/third_party/gemmlowp/fixedpoint/fixedpoint_sse.h>
