...
#include <ai_apps/common/tensorflow/lite/kernels/internal/max.h>
#include <ai_apps/common/tensorflow/lite/kernels/internal/min.h>
#include <ai_apps/common/tensorflow/lite/kernels/internal/quantization_util.h>
#include <ai_apps/common/tensorflow/lite/kernels/internal/types.h>
#include <ai_apps/common/third_party/ruy/ruy/profiler/instrumentation.h> // from @ruy
#include <algorithm>
...
// dimensions given in axis.
        ...
        inline bool Reduce (const In * input_data,
                            ...
                            const int num_axis,
                            int * input_iter,
                            Out reducer(Out current, const In in),
                            Out * output_data) {
            ...
            do
            {
                ...
                size_t output_offset = ReducedOutputOffset(input_num_dims, input_dims, input_iter, num_axis, axis);
...
// values, and returns a valid 'out_axis'
        inline bool ResolveAxis (const int     num_dims,
                                 const int   * axis,
...
// This method expects that output_data has been initialized.
        template < typename In, typename Out >
        inline bool ReduceSumImpl (const In  * input_data,
                                   const int * input_dims,
                                   const int * output_dims,
                                   const int   input_num_dims,
                                   const int   output_num_dims,
        ...
        template < typename T >
        inline bool InitTensorDataForReduce (const int * dims, const int num_dims, const T init_value, T * data) {
...
// dimensions given in axis. It needs to pass in init_value and reducer.
        template < typename T >
        inline bool ReduceGeneric (const T * input_data,
                                   const int * input_dims,
                                   const int input_num_dims,
                                   T * output_data,
                                   const int * output_dims,
...
// then divides it by the number of element in axis.
        ...
        inline bool Mean (const T   * input_data,
                          const int * input_dims,
                          const int   input_num_dims,
                          T         * output_data,
                          const int * output_dims,
...
// then divides it by the number of element in axis for quantized values.
        template < typename T, typename U >
        inline bool QuantizedMeanOrSum (const T   * input_data,
                                        int32_t     input_zero_point,
                                        float       input_scale,
                                        const int * input_dims,
                                        const int   input_num_dims,
            ...
            if (num_elements_in_axis > 0)
            {
                ...
                else
                {
                    ...
                    for (size_t idx = 0; idx < num_outputs; ++idx)
                    {
                        ...
                        float result = TfLiteMin(TfLiteRound(float_mean * scale + bias) + output_zero_point,
                                                 static_cast < float > (std::numeric_limits < T > ::max()));
                        result           = TfLiteMax(result, static_cast < float > (std::numeric_limits < T > ::min()));
        ...
        template < typename T >
        inline bool QuantizedReduceProd (const T           * input_data,
                                         int32_t             input_zero_point,
                                         const RuntimeShape &input_shape,
                                         T                 * output_data,
                                         int32_t             output_zero_point,
            ...
            // Resolve axis.
            int num_resolved_axis = 0;
            if (!ResolveAxis(input_shape.DimensionsCount(), axis, num_axis_dimensions, resolved_axis,
                             &num_resolved_axis))
            {
                return false;
            }
            ...
            // avoid an overflow.
            auto reducer_first = [&](T in)->int32_t {
                return in - input_zero_point;
            };
