...
#include <ai_apps/common/tensorflow/lite/kernels/internal/portable_tensor_utils.h>
...
// If we are broadcasting, then don't advance (i.e return 0).
        ...
        inline void BatchMatMul (const RuntimeShape &lhs_shape,
            ...
            const int batch_dim0 = batch_matmul::broadcast_dim(extended_lhs_shape.Dims(0), extended_rhs_shape.Dims(0));
            const int batch_dim1 = batch_matmul::broadcast_dim(extended_lhs_shape.Dims(1), extended_rhs_shape.Dims(1));
            const int batch_dim2 = batch_matmul::broadcast_dim(extended_lhs_shape.Dims(2), extended_rhs_shape.Dims(2));
        ...
        inline void BatchMatMul (const RuntimeShape &lhs_shape,
            ...
            if (!compute_row_sums || *compute_row_sums)
            {
                ...
                tensor_utils::ReductionSumVector(lhs_data, row_sums, num_weights_matrices * lhs_rows, accum_depth);
                if (compute_row_sums)
                {
                    ...
                    for (int b2 = 0; b2 < batch_dim2; ++b2)
                    {
                        ...
                        for (int j = 0; j < rhs_cols; ++j)
                        {
                            for (int i = 0; i < lhs_rows; ++i)
                            {
                                ...
                                int32_t total_scaled = MultiplyByQuantizedMultiplier(total,
                                                                                     output_multiplier,
                                                                                     output_shift);
