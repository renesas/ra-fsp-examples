        ...
        inline void Softmax (const SoftmaxParams &params,
            ...
            for (int i = 0; i < outer_size; ++i)
            {
                ...
                for (int c = 0; c < depth; ++c)
                {
                    ...
                    if (input_diff >= diff_min)
                    {
                        const int32_t input_diff_rescaled =
                            MultiplyByQuantizedMultiplierGreaterThanOne(input_diff,
                ...
                FixedPoint0 shifted_scale =
                    FixedPoint0::FromRaw(GetReciprocal(sum_of_exps.raw(), kAccumulationIntegerBits,
                                                       &num_bits_over_unit));
                ...
                for (int c = 0; c < depth; ++c)
                {
                    ...
                    if (input_diff >= diff_min)
                    {
                        ...
                        output_data[i * depth + c] = static_cast < OutputT >
                                                     (std::max(std::min(shifted_output,
                                                                        static_cast < int32_t >
...
// Computes exp(input - max_input)
        inline int16_t SoftMaxCalculateExp (const SoftmaxParams &params,
            ...
            int32_t scaled_diff = MultiplyByQuantizedMultiplier(input_diff,
            ...
            int16_t sat_sym_scaled_diff =
                std::min(std::max(sym_scaled_diff, static_cast < int32_t > (-32768)), static_cast < int32_t > (32767));
