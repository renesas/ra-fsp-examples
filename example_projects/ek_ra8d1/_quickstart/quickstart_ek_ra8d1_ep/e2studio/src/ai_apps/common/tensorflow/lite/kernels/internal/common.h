...
#include <ai_apps/common/tensorflow/lite/kernels/internal/cppmath.h>
#include <ai_apps/common/tensorflow/lite/kernels/internal/optimized/neon_check.h>
...
// Legacy function, left for compatibility only.
    ...
    inline void BiasAndClamp (float         clamp_min,
                              float         clamp_max,
                              int           bias_size,
        ...
        for ( ; array_ptr != array_end_ptr; array_ptr += bias_size)
        {
            ...
            for ( ; i < bias_size; i++)
            {
                array_ptr[i] = ActivationFunctionWithMinMax(array_ptr[i] + bias_data[i], clamp_min, clamp_max);
            }
        }
             ...
             array_offset += bias_size)
        {
            ...
            {
                array_data[array_offset + i] = ActivationFunctionWithMinMax(array_data[array_offset + i] + bias_data[i],
                                                                            clamp_min,
                                                                            clamp_max);
            }
        }
...
// Single-rounding MultiplyByQuantizedMultiplier
    ...
    inline int32_t MultiplyByQuantizedMultiplierSmallerThanOneExp (int32_t x, int32_t quantized_multiplier, int shift) {
        TFLITE_DCHECK_LE(shift, 0);

        return MultiplyByQuantizedMultiplier(x, quantized_multiplier, shift);
    }

    inline int32_t MultiplyByQuantizedMultiplierGreaterThanOne (int32_t x, int32_t quantized_multiplier, int shift) {
    ...
    inline int32x4x4_t MultiplyByQuantizedMultiplier4Rows (int32x4x4_t input_val,
        ...
        int32x4x4_t result;
        result.val[0] = vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[0], left_shift_dup), multiplier_dup),
                                   right_shift_dup);

        result.val[1] = vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[1], left_shift_dup), multiplier_dup),
                                   right_shift_dup);

        result.val[2] = vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[2], left_shift_dup), multiplier_dup),
                                   right_shift_dup);

        result.val[3] = vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[3], left_shift_dup), multiplier_dup),
                                   right_shift_dup);
...
// Round uses ARM's rounding shift right.
    inline int32x4x4_t MultiplyByQuantizedMultiplier4Rows (int32x4x4_t input_val,
        ...
        result.val[0] =
            vrshlq_s32(vqrdmulhq_s32(vshlq_s32(input_val.val[0], left_shift_dup), multiplier_dup), right_shift_dup);

        result.val[1] =
            vrshlq_s32(vqrdmulhq_s32(vshlq_s32(input_val.val[1], left_shift_dup), multiplier_dup), right_shift_dup);

        result.val[2] =
            vrshlq_s32(vqrdmulhq_s32(vshlq_s32(input_val.val[2], left_shift_dup), multiplier_dup), right_shift_dup);

        result.val[3] =
            vrshlq_s32(vqrdmulhq_s32(vshlq_s32(input_val.val[3], left_shift_dup), multiplier_dup), right_shift_dup);
...
// 126, 127].
    ...
    void > ::type LUTPopulate (float    input_scale,
...
// should be used for FloatT by default.
    ...
    inline typename std::enable_if < std::is_same < T, int16_t > ::value, void > ::type LUTPopulate (
        ...
        lut[nb_steps] = static_cast < T > (std::min < FloatT > (
                                               std::max < FloatT >
                                               (TfLiteRound(transform(input_max) * output_scaling_inv),
                                                table_min),
                                               table_max));
    }
...
// Deprecated and will be removed in future, please use LUTPopulate instead
    ...
    inline void gen_lut (FloatT (* func)(FloatT),
                         FloatT    input_min,
                         FloatT    input_max,
                         FloatT    output_min,
                         FloatT    output_max,
                         LutOutT * lut) {
        static_assert(std::is_same < LutInT, LutOutT > ::value, "Input and output type of the LUT must be the same.");
        static_assert(std::is_same < LutInT, int16_t > ::value, "Only int16_t type LUT are supported.");
        static_assert(std::is_same < FloatT, float > ::value, "Only float type is supported for FloatT.");
        using T = LutInT;
...
// TODO(b/77858996): Add these to gemmlowp.
    ...
    inline std::int32_t SaturatingAddNonGemmlowp (std::int32_t a, std::int32_t b) {
        ...
        return static_cast < std::int32_t >
               (std::min(static_cast < std::int64_t > (std::numeric_limits < std::int32_t > ::max()),
                         std::max(static_cast < std::int64_t > (std::numeric_limits < std::int32_t > ::min()), sum)));
    }

    template < typename tRawType, int tIntegerBits >
    gemmlowp::FixedPoint < tRawType, tIntegerBits > SaturatingAddNonGemmlowp(gemmlowp::FixedPoint < tRawType,
                                                                             tIntegerBits > a,
                                                                             gemmlowp::FixedPoint < tRawType,
                                                                             tIntegerBits > b)
    {
        return gemmlowp::FixedPoint < tRawType, tIntegerBits > ::FromRaw(SaturatingAddNonGemmlowp(a.raw(), b.raw()));
    }
    ...
    template < typename tRawType, int tIntegerBits >
    gemmlowp::FixedPoint < tRawType, tIntegerBits > SaturatingSub(gemmlowp::FixedPoint < tRawType,
                                                                  tIntegerBits > a,
                                                                  gemmlowp::FixedPoint < tRawType,
                                                                  tIntegerBits > b)
    {
...
// by a power of two has to be saturating/rounding, not exact anymore.
    ...
    {
        return gemmlowp::FixedPoint < tRawType,
               tIntegerBits > ::FromRaw(SaturatingRoundingMultiplyByPOTParam(a.raw(), exponent));
    }

// Convert int32_t multiplier to int16_t with rounding.
    inline void DownScaleInt32ToInt16Multiplier (int32_t multiplier_int32_t, int16_t * multiplier_int16_t) {
...
// is symmetric.
    ...
    log_x_for_x_greater_than_or_equal_to_1_impl(gemmlowp::FixedPoint < int32_t, InputIntegerBits > input_val)
    {
        ...
        const FixedPoint0 log_2          = GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 1488522236, std::log(2.0));
        ...
        const FixedPoint0 alpha_n = GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0,
                                                                         117049297,
                                                                         11.0 / 240.0 * std::sqrt(std::sqrt(2.0)));
        const FixedPoint0 alpha_d =
            GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 127690142, 1.0 / 20.0 * std::sqrt(std::sqrt(2.0)));
        const FixedPoint0 alpha_i =
            GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 1057819769,
                                                 2.0 / std::sqrt(std::sqrt(2.0)) - std::sqrt(std::sqrt(2.0)));
        const FixedPoint0 alpha_f =
            GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 638450708, 1.0 / 4.0 * std::sqrt(std::sqrt(2.0)));
        ...
        const FixedPointAccum z_a_pow_2_adj =
            SaturatingAddNonGemmlowp(FixedPointAccum::FromRaw(SaturatingRoundingMultiplyByPOTParam(static_cast <
                                                                                                   int32_t >
                                                                                                   (InputIntegerBits -
                                                                                                    z_a_headroom_plus_1),
                                                                                                   31 -
        ...
        const FixedPointAccum z_b_pow_2_adj =
            SaturatingSub(FixedPointAccum::FromRaw(SaturatingRoundingMultiplyByPOTParam(static_cast < int32_t >
                                                                                        (InputIntegerBits -
                                                                                         z_b_headroom),
    ...
    {
        static_assert(OutputIntegerBits >= min_log_x_output_bits(InputIntegerBits),
    ...
    inline int32_t GetReciprocal (int32_t x, int x_integer_digits, int * num_bits_over_unit) {
        ...
        *num_bits_over_unit = x_integer_digits - headroom_plus_one;
        ...
        gemmlowp::FixedPoint < int32_t, 0 > shifted_scale =
            gemmlowp::one_over_one_plus_x_for_x_in_0_1(gemmlowp::FixedPoint < int32_t,
                                                       0 > ::FromRaw(shifted_sum_minus_one));

        return shifted_scale.raw();
    }

    inline void GetInvSqrtQuantizedMultiplierExp (int32_t   input,
                                                  int       reverse_shift,
...
// Copies dims to desc, calculating strides.
    template < int N >
    inline void CopyDimsToDesc (const RuntimeShape &input_shape, NdArrayDesc < N > * desc_out) {
        int desc_stride = 1;
        for (int i = N - 1; i >= 0; --i)
        {
    ...
    template < int N >
    inline void NdArrayDescsForElementwiseBroadcast (const RuntimeShape &input0_shape,
...
// to its own more sensible logic for its own workload.
    ...
    inline int LegacyHowManyThreads (int max_num_threads, int rows, int cols, int depth) {
        ...
        if (thread_count > 1)
        {
            ...
            thread_count = std::min(thread_count, static_cast < int > (cubic_size / min_cubic_size_per_thread));
        }
