...
fsp_err_t flash_hp_code_flash_operations(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_FLASH_HP_Erase API failed, Restart the Application");
        return err;
    }
    APP_PRINT("\r\nErase successful");
    ...
    err = R_FLASH_HP_BlankCheck(&g_flash_ctrl, FLASH_HP_CF_BLOCK_8, FLASH_HP_CF_BLOCK_SIZE_2KB, &blank_check_result);
#else
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_FLASH_HP_BlankCheck API failed, Restart the Application");
        return err;
    }
    APP_PRINT("\r\nR_FLASH_HP_BlankCheck API Successful");
...
#if (BSP_FEATURE_FLASH_SUPPORTS_ACCESS_WINDOW == 1U) /* Macro for MCU supports Access Window Features */
    /* Set Access window.
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_FLASH_HP_AccessWindowSet API failed, Restart the Application");
        return err;
    }
    APP_PRINT("\r\nR_FLASH_HP_AccessWindowSet API successful");
    ...
    APP_PRINT("\r\nR_FLASH_HP_Erase API successful");
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_FLASH_HP_AccessWindowClear API failed, Restart the Application");
        return err;
    }
...
void flash_hp_deinit(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_FLASH_HP_Close API failed in BlankCheck API");
    }
}
