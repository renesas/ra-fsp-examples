 ...
 * File Name    : lp_mode.c
 * Version      : 1.0
 * Description  : Support for entering and leaving low power modes.
 *********************************************************************************************************************/
/**********************************************************************************************************************
...
/* set 1 and disable RTC period wake in FSP to make the wake up time longer (1 minute) but the alarm cannot be used*/
#define  MINTUES_WAKE               (1)

/* Define this to turn on red LED when entering standby mode, note that this will draw significant current so measuring
 * standby current will be invalid. */
#undef STANDBY_WITH_LEDS
...
rtc_alarm_time_t g_mins_wake            = { 0 };
...
static int8_t lpm_check_input (void);
 ...
 * Description  : Manage the console entries when in Low Power mode Example.
 * Return Value : Zero, reserved for future use.
 *********************************************************************************************************************/
test_fn lpm_main_display_menu(void)
{
    ...
    {
        xEventGroupSetBits (g_update_console_event, STATUS_LPM_MODE_MENU);
        ...
        else if (LPM_SWITCH_MODE == (g_s_lp_bits & 1))
        {
        ...
        if (!(LPM_SWITCH_MODE == (g_s_lp_bits & 1)))
        {
            print_lpm_menu();
            start_key_check();
        }
    ...
    xEventGroupClearBits (g_update_console_event, STATUS_LPM_MODE_MENU);
    gpt_start();
    return 0;
}
/**********************************************************************************************************************
 ...
 * Description  : Check the user input and if valid enter the requested state.
 * Return Value : NONE.
 *********************************************************************************************************************/
static void lpm_execute(void)
{
 ...
 * Description  : Stop the LED Timers to reduce current draw.
 * Return Value : NONE.
 *********************************************************************************************************************/
static void gpt_stop(void)
{
 ...
 * Description  : Re-Start the LED Timers to restore operation.
 * Return Value : NONE.
 *********************************************************************************************************************/
static void gpt_start(void)
{
 ...
 * Description  : Draw the sample menu on the console.
 * Return Value : NONE.
 *********************************************************************************************************************/
static void print_lpm_menu(void)
{
 ...
 * Description  : Complete preparation to enter a low power mode. Provide opportunity to store critical data before
 ...
 * Description  : Enter the selected example state.
 * Argument     : lpm_mode
 * Return Value : Zero, Reserved for future use.
 *********************************************************************************************************************/
static test_fn lp_mode_demo(void)
...
static fsp_err_t lpm_operation(e_app_lpm_states_t lpm_mode)
{
    ...
    /* Enter to LPM; Turn off the function to reduce power */
    R_RTC_CalendarTimeGet (&g_rtc_ctrl, &s_latest_update_time);

    /* set alarm to wake the software standby mode*/
#ifdef MINTUES_WAKE
    ...
    g_mins_wake.time        = s_latest_update_time;
    g_mins_wake.time.tm_min = g_mins_wake.time.tm_min + 1;
    g_mins_wake.min_match   = true;

    R_RTC_CalendarAlarmSet (&g_rtc_ctrl, &g_mins_wake);
#else
 ...
 * Description  : Disables all ancillary operations and clocks to minimise current draw.
 * Return Value : NONE
 *********************************************************************************************************************/
static void lpm_sidefunction_stop(void)
{
    ...
    xEventGroupSetBits (g_update_console_event, STATUS_LPM_MODE_START);
    ...
    /* Update the register setting by following section 10 in hardware manual */
    R_SYSTEM->LDOCR_b.CHG0    = 0;
    R_MSTP->MSTPCRD_b.MSTPD2  = 0;
    R_SYSTEM->PSMCR           = 0;
    R_SYSTEM->SRMPSCR_b.SPSC0 = true;
    R_SYSTEM->SRMPSCR_b.SPSC1 = true;
    R_SYSTEM->SRMPSCR_b.SPSC2 = true;

    g_timer1_ctrl.p_reg->AGT32.CTRL.AGTMR1_b.TCK = 6; /* Set clock to SUBCLK */
    g_timer1_ctrl.p_reg->AGT32.CTRL.AGTMR2_b.CKS = 1; /* Set the clock divider to 2 */
    g_timer1_ctrl.p_reg->AGT32.AGT               = 32768 / 2;


    clk_set.divider_cfg.sckdivcr_w      = 0;
    ...
    while (0 != R_SYSTEM->SOPCCR_b.SOPCMTSF)
    {
        ;
    }

    R_SYSTEM->SOPCCR_b.SOPCM = 1;
...
static fsp_err_t lpm_enter_procedure(e_app_lpm_states_t lpm_mode)
{
    ...
    /* Start/enable hardware switch interrupt to cancel LPM mode */
...
static fsp_err_t lpm_wait_start(e_app_lpm_states_t lpm_mode)
{
    ...
    while (0 == g_s_lp_bits)
    {
        ...
        if (g_alarm_ring)
        {
            ...
            R_RTC_CalendarTimeGet (&g_rtc_ctrl, &s_current_time);
            set_time_on_slcd (&s_current_time);
            g_mins_wake.time        = s_current_time;
            g_mins_wake.time.tm_min = g_mins_wake.time.tm_min + 1;
            R_RTC_CalendarAlarmSet (&g_rtc_ctrl, &g_mins_wake);
            g_alarm_flash = 0;
            g_alarm_ring  = false;
    ...
    if (g_rtc_alarm.dayofweek_match) /* Check is the alarm set */
    {
        set_segment_icon_named (ALARM_ICON, ICON_ON);
    }
 ...
 * Description  : Re-Enables all ancillary operations and clocks when leaving low power mode.
 * Return Value : NONE.
 *********************************************************************************************************************/
static void lpm_sidefunction_start(void)
{
    ...
    clk_set.loco_state                          = CGC_CLOCK_CHANGE_NONE;
    ...
    clk_set.subosc_state                        = CGC_CLOCK_CHANGE_NONE;
    g_cgc.p_api->clocksCfg(g_cgc.p_ctrl, &clk_set);
    g_timer1_ctrl.p_reg->AGT32.CTRL.AGTMR1_b.TCK = 6;
    g_timer1_ctrl.p_reg->AGT32.CTRL.AGTMR2_b.CKS = 0;
    g_timer1_ctrl.p_reg->AGT32.AGT               = 32768;

    R_SYSTEM->SRMPSCR = 0x3;
    R_SYSTEM->SRMPSCR = 0x1;
    R_SYSTEM->SRMPSCR = 0x0;
    R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MICROSECONDS);
    jlink_console_init();
    common_init();
    R_GPT_Stop (g_blinker.p_ctrl);
    R_GPT_Stop (g_gpt_blue.p_ctrl);
    xEventGroupClearBits (g_update_console_event, (STATUS_LPM_MODE_START));
}
/**********************************************************************************************************************
...
/**********************************************************************************************************************
 * Function Name: lpm_check_input
 * Description  : Checking input into console
 * Return Value : Received character
 *********************************************************************************************************************/
static int8_t lpm_check_input(void)
{
    int8_t c = -1;
    while ((0 != c) || (LPM_SWITCH_MODE == (g_s_lp_bits & 1)))
    {
        /* Check key press*/
        if (key_pressed () || (LPM_SWITCH_MODE == (g_s_lp_bits & 1)))
        {
...
/**********************************************************************************************************************
 End of function lpm_check_input
 *********************************************************************************************************************/
