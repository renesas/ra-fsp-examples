...
static char_t rtc_alarm_check_am_pm (rtc_time_t * rtc_input, char_t * rtc_user_input_am_pm);
static char_t rtc_alarm_check_weekdays (rtc_time_t * rtc_input, char_t * rtc_user_input_weekdays_result, char_t * type);
 ...
 * Description  : Set the time by getting user inputs and setting the RTC.
 ...
 * Description  : Set the time by getting user inputs and setting the alarm process.
 * Return Value : ZERO - Reserved
 *********************************************************************************************************************/
static test_fn set_alarm_option(void)
{
    char_t c = '0';
    ...
    while ((0 != c))
    {
        if (true == s_return_to_menu)
        {
            if (true != s_alarm_set)
            {
                ...
                g_rtc_alarm.dayofweek_match = false;
                g_rtc_alarm.hour_match      = false;
                g_rtc_alarm.min_match       = false;
            }
            break;
        }
        c = input_from_console ();
 ...
 * Description  : Get the time from the console user input.
 * Arguments    : rtc_input : pointer to structure to populate
 *              : type      : string to print representing type
 * Return Value : ZERO - Reserved
 *********************************************************************************************************************/
static test_fn get_time_input(rtc_time_t *rtc_input, char_t *type)
{
    ...
    while ((0 != c))
    {
        ...
        rtc_alarm_check_am_pm(rtc_input, am_pm_read);
        strcpy(rtc_user_input[2], am_pm_read);
        ...
        rtc_alarm_check_weekdays(rtc_input, rtc_user_input[3], type);
        if (true == s_return_to_menu)
        {
            break;
        }
 ...
 * Description  : Return a string formatted with the current set alarm.
 * Argument     : alarm_str : string pointer to populate
 * Return Value : ZERO - Reserved
 *********************************************************************************************************************/
 ...
 * Description  : Convert ASCII character string to number and verify if the number is a valid Hour or Minute setting.
 * Arguments    : p_data
 *              : hours_or_minutes
 * Return Value : valid value, or (-1, -2, -3) if invalid
 *********************************************************************************************************************/
 ...
 * Description  : Read the console input and detect escape characters.
 * Arguments    : p_out     : pointer to string returned
 *              : out_length: maximum length of string to return
 *              : rtc_output: pointer to string to print
 * Return Value : NONE.
 *********************************************************************************************************************/
static void string_from_console(char_t * p_out, size_t out_length, char_t * rtc_output)
{
...
/**********************************************************************************************************************
 * Function Name: rtc_alarm_check_am_pm
 * Description  : Accept input from console check for escape characters and validate a for am and p for pm.
 * Arguments    : rtc_input                     : pointer to rtc structure to update
 *              : rtc_user_input_am_pm_result   : pointer to am or pm string
 * Return Value : ZERO - Reserved
 *********************************************************************************************************************/
static char_t rtc_alarm_check_am_pm(rtc_time_t * rtc_input, char_t * rtc_user_input_am_pm_result)
{
    ...
    while ('0' == c)
    {
        ...
        {
            strcpy (rtc_user_input_am_pm_result, "am");

            if (12 == rtc_input->tm_hour)
            {
                rtc_input->tm_hour = 0;
            }
        }
        ...
        {
            strcpy (rtc_user_input_am_pm_result, "pm");
            if (12 != rtc_input->tm_hour)
            {
...
/**********************************************************************************************************************
 End of function rtc_alarm_check_am_pm
 *********************************************************************************************************************/

/**********************************************************************************************************************
 * Function Name: rtc_alarm_check_weekdays
 * Description  : Accept input from console check for escape characters and validate weekdays.
 * Arguments    : rtc_input                     : pointer to rtc structure to update
 *              : rtc_user_input_weekdays_result: pointer to weekday string
 *              : type                          : select for weekday or day of month
 * Return Value : ZERO _ Reserved
 *********************************************************************************************************************/
static char_t rtc_alarm_check_weekdays(rtc_time_t * rtc_input, char_t * rtc_user_input_weekdays_result, char_t * type)
{
    int8_t val;
    ...
    while ('0' == c)
    {
        ...
        if ((val > 0) && (val < 8))
        {
            strcpy (rtc_user_input_weekdays_result, p_day_options[val - 1]);
...
/**********************************************************************************************************************
 End of function rtc_alarm_check_weekdays
 *********************************************************************************************************************/
