 ...
 * Select whether this project will
...
fsp_err_t common_init(void)
{
    ...
    /* Initialize the control structure to known value of zero */
    memset (&g_mins_wake, 0, sizeof(g_mins_wake));
    ...
    /* Set AGT1 Operating Mode to Timer mode */
    R_AGTW1->AGT16.CTRL.AGTMR1_b.TMOD    = 0;
 ...
 * Description  : Update the Duty Cycle of the LED being controlled.
 * Return Value : NONE.
 *********************************************************************************************************************/
 ...
 *    where the board is tracked to support this.
 * b. The GPT compare to raise an interrupt. Then the LED is toggled in SW within the interrupt handler
 *    (gpt_blue_callback). For where the LED is not directly connected to the GPT compare output pin.
 */
#ifdef USE_SW_TIMER
 ...
 * Description  : Handle the Blue LED Flashing by Timer Control.
 * Argument     : p_args
 * Return Value : NONE.
 *********************************************************************************************************************/
void gpt_blinker_callback(timer_callback_args_t *p_args)
 ...
 * Description  : Initialise the Real Time clock and set a default date / time
 * Return Value : NONE.
 *********************************************************************************************************************/
