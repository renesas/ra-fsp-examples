...
#define MODULE_NAME     "\r\n%d. QUAD-SPI SPEED TEST\r\n"

#define SUB_OPTIONS     "\r\nCompare the write and read times to and from external Quad-SPI" \
                        " flash memory\r\n" \
...
static fsp_err_t qspi_write_test (void);
static uint32_t  qspi_write_block(uint32_t block_size);
static uint32_t  qspi_read_test(uint32_t block_size);
static fsp_err_t qspi_erase (uint32_t block_size);
static void      result_print (uint32_t qspi_write_time_us, uint32_t qspi_read_time_us);
static void      print_header (void);
static void      qspi_read_write_print (uint32_t block_size_actual);
static void      handle_error (fsp_err_t fsp_err);
 ...
 * Description  : Handle the interrupt on completion of the DMA transfer.
 * Argument     : p_args
 * Return Value : NONE.
 *********************************************************************************************************************/
...
/**********************************************************************************************************************
 * Function Name: qspi_write_block
 * Description  : Configures a timer and measures execution time for writing a block of qspi memory.
 * Argument     : block_size
 * Return Value : The time taken in (TIMER COUNTS).
 *********************************************************************************************************************/
static uint32_t qspi_write_block(uint32_t block_size)
{
    fsp_err_t            fsp_err;
    ...
    /* Handle error */
    handle_error (fsp_err);
    qspi_write_time = status.counter;

    return (qspi_write_time);
}
/**********************************************************************************************************************
 End of function qspi_write_block
 *********************************************************************************************************************/
 ...
 * Description  : Handle the user input collecting the requested block size in acsii characters.
 * Return Value : ZERO - Reserved for future use.
 *********************************************************************************************************************/
test_fn ext_display_menu(void)
{
    volatile bool_t   valid_block_size  = false;
    ...
    while (false == valid_block_size)
    {
        ...
        while ((CONNECTION_ABORT_CRTL != c))
        {
            ...
            if ((block_sz_ndx < block_sz_limit) && (BACK_SPACE != c) &&
                (CARRAGE_RETURN != c) && (MENU_ENTER_RESPONSE_CRTL != c))
            {
                /* Cast to req type */
                s_block_sz_str[block_sz_ndx] = (char_t)c;
                block_sz_ndx++;
            }
            else if ((BACK_SPACE == c) && (block_sz_ndx > 0))
            {
                s_block_sz_str[block_sz_ndx - 1] = '\0';
                block_sz_ndx--;
                sprintf (s_print_buffer , "\b \b");
    ...
    if ((MENU_ENTER_RESPONSE_CRTL == c) && (0 != value))
    {
        qspi_read_write_print(value);
    }
 ...
 * Description  : wait for QSPI flash device status register to get idle till operation is in progress
 * Return Value : FSP_SUCCESS or any other possible error codes
 *********************************************************************************************************************/
static fsp_err_t get_flash_status(void)
{
 ...
 * Description  : set QPI Mode in flash device and MCU
 * Return Value : FSP_SUCCESS or any other possible error codes
 *********************************************************************************************************************/
static fsp_err_t qpi_mode_set(void)
{
 ...
 * Description  : Initialise the MCU peripheral mode and the confirm the device.
 * Return Value : FSP_SUCCESS or any other possible error codes
 *********************************************************************************************************************/
static fsp_err_t qpi_init(void)
{
...
/**********************************************************************************************************************
 * Function Name: result_print
 * Description  : Format the results into a printed stream.
 * Arguments    : qspi_write_time_us
 *              : qspi_read_time_us
 * Return Value : NONE.
 *********************************************************************************************************************/
static void result_print(uint32_t qspi_write_time_us, uint32_t qspi_read_time_us)
{
    ...
    /* ignoring -Wpointer-sign is OK for a constant string */
    print_to_console("\r\n-------------------------------------------------");
    sprintf(s_print_buffer, "\r\nWrite                %ld", qspi_write_time_us);

    /* ignoring -Wpointer-sign is OK when treating signed char_t array as as unsigned */
    print_to_console(s_print_buffer);
    sprintf(s_print_buffer, "\r\nRead                 %ld", qspi_read_time_us);
...
/**********************************************************************************************************************
 End of function result_print
 *********************************************************************************************************************/

/**********************************************************************************************************************
 * Function Name: handle_error
 * Description  : Check the err
 * Argument     : fsp_err
 * Return Value : NONE.
 *********************************************************************************************************************/
static void handle_error(fsp_err_t fsp_err)
...
/**********************************************************************************************************************
 * Function Name: qspi_read_write_print
 * Description  : Perform the Read / Write test and format the input confirmation and process into a printed stream..
 * Arguments    : qspi_read_write_print
 *              : block_size_actual
 * Return Value : NONE.
 *********************************************************************************************************************/
static void qspi_read_write_print(uint32_t block_size_actual)
{
    ...
    uint32_t       qspi_read_time_us  = 0;
    uint32_t       qspi_write_time_us = 0;
    ...
    qspi_write_time_us  = ((100000000 / timer_frequency) *qspi_write_block(block_size_actual)) / 100;
    ...
    qspi_read_time_us  = ((100000000 / timer_frequency) *qspi_read_test(block_size_actual)) / 100;
    ...
    /* Handle error */
    handle_error (fsp_err);
    result_print (qspi_write_time_us, qspi_read_time_us);

}
/**********************************************************************************************************************
 End of function qspi_read_write_print
 *********************************************************************************************************************/

/**********************************************************************************************************************
 * Function Name: print_header
 * Description  : Print the defined header text for the demonstration.
 * Return Value : NONE.
 *********************************************************************************************************************/
static void print_header(void)
{
 ...
 * Description  : Perform a simple write / read test to ensure device is accessible.
 * Return Value : FSP_SUCCESS or any other possible error codes
 *********************************************************************************************************************/
static fsp_err_t qspi_write_test(void)
{
...
/**********************************************************************************************************************
 * Function Name: qspi_erase
 * Description  : Erase a block of memory starting at the OSPI_DEVICE_START_ADDRESS
 * Argument     : block_size
 * Return Value : FSP_SUCCESS or any other possible error codes
 *********************************************************************************************************************/
static fsp_err_t qspi_erase(uint32_t block_size)
 ...
 * Description  : Configures a timer and measures execution time for reading a block of qspi memory.
 * Argument     : block_size in kilobytes (1024)
 * Return Value : The time taken in (TIMER COUNTS).
 *********************************************************************************************************************/
static uint32_t qspi_read_test(uint32_t block_size)
{
    ...
    qspi_read_time = status.counter;

    vPortFree(p_dma_read_buffer);
    return (qspi_read_time);
}
/**********************************************************************************************************************
