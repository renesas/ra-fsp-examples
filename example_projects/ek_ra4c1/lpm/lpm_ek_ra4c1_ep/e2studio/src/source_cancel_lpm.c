...
/*******************************************************************************************************************//**
 *  @brief      This function will initialize modules used to cancel LPM mode.
 *  @param      None
...
#if (BSP_PERIPHERAL_ULPT_PRESENT)
    err = R_ULPT_Open(&g_timer_cancel_lpm_ctrl, &g_timer_cancel_lpm_cfg);
    TERMINAL_ERR_RETURN(err, "R_ULPT_Open API Failed\r\n");
#elif (BSP_PERIPHERAL_TML_PRESENT)
    err = R_TML_Open(&g_timer_cancel_lpm_ctrl, &g_timer_cancel_lpm_cfg);
    TERMINAL_ERR_RETURN(err, "R_TML_Open API Failed\r\n");
#else
...
/*******************************************************************************************************************//**
 *  @brief      This function will de-initialize modules used to cancel LPM mode.
 *  @param      None
 *  @retval     FSP_SUCCESS    Upon successful operation.
 *  @retval     Any other error code apart from FSP_SUCCESS.
 **********************************************************************************************************************/
...
{
    /* Close Timer module used to cancel LPM if it is already opened */
    if(g_timer_cancel_lpm_ctrl.open)
    {
#if (BSP_PERIPHERAL_ULPT_PRESENT)
        R_ULPT_Close(&g_timer_cancel_lpm_ctrl);
#elif (BSP_PERIPHERAL_TML_PRESENT)
        R_TML_Close(&g_timer_cancel_lpm_ctrl);
#else
        R_AGT_Close(&g_timer_cancel_lpm_ctrl);
#endif
    }

    /* Close ICU External IRQ module used to cancel LPM if it is already opened */
...
#if(BSP_FEATURE_LPM_HAS_SNOOZE)
    /* Close Timer module used to automatically trigger DTC transfers in case of Snooze mode if it is already opened */
#if (BSP_PERIPHERAL_TML_PRESENT)
    if(g_timer_cancel_lpm_ctrl.open)
    {
        R_TML_Close(&g_timer_cancel_lpm_ctrl);
...
#endif
    /* Close DTC module use to cancel Snooze mode if it is already opened */
...
#elif (BSP_PERIPHERAL_TML_PRESENT)
    err = R_TML_Start(&g_timer_cancel_lpm_ctrl);
    TERMINAL_ERR_RETURN(err, "R_TML_Start API Failed\r\n");
#else
...
fsp_err_t source_cancel_lpm_disable(void)
{
    ...
    TERMINAL_ERR_RETURN(err, "R_ICU_ExternalIrqDisable API Failed\r\n");
...
#elif (BSP_PERIPHERAL_TML_PRESENT)
    /* Stop Timer  */
    err = R_TML_Stop(&g_timer_cancel_lpm_ctrl);
    TERMINAL_ERR_RETURN(err, "R_TML_Stop API Failed\r\n");

    /* Reset Timer counter */
    err = R_TML_Reset(&g_timer_cancel_lpm_ctrl);
    TERMINAL_ERR_RETURN(err, "R_TML_Reset API Failed\r\n");
#else
    /* Stop Timer */
...
/*******************************************************************************************************************//**
 *  @brief      This function will clear all source detection flags.
 *  @param      None
 *  @retval     None
 **********************************************************************************************************************/
