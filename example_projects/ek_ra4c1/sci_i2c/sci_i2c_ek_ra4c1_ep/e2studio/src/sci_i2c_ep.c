...
/*******************************************************************************************************************//**
 * @brief     Initializes SCI_I2C_MASTER and IIC_SLAVE drivers.
 * @param[IN] None
...
fsp_err_t init_i2c_driver(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
    	/* Display failure message in J-link RTT Viewer */
    ...
    {
        /* Display failure message in J-link RTT Viewer */
        APP_ERR_PRINT ("R_IIC"SLAVE_TYPE"_Open API FAILED\r\n");

        /* Slave open unsuccessful, closing the master module */
#if  (BSP_PERIPHERAL_SCI_B_PRESENT)
        ...
        {
            /* Display failure message in J-link RTT Viewer */
            APP_ERR_PRINT ("R_"SCI_TYPE"_Close API FAILED\r\n");
        }
    }
    return err;
}
...
/*******************************************************************************************************************//**
 *  @brief      Performs master write and read operations and toggles the LED on a successful operation;
 *              otherwise, turns the LED on to indicate failure.
 *              If the SCI_I2C master transaction fails, the application halts and turns the LED on
 *              as a sign of failure. Failure messages are also displayed in RTT.
 *  @param[IN]  None
 *  @retval     FSP_SUCCESS On successfully SCI_I2C_Master and Slave Write/Read operation.
 *  @retval     err         Any other error code apart from FSP_SUCCESS indicating an unsuccessful write/read operation.
 **********************************************************************************************************************/
fsp_err_t process_master_WriteRead(void)
{
    ...
    switch(g_master_RW)
    {
        case MASTER_WRITE:
        {
            ...
            {
                /* Print J-link RTT Viewer message */
                APP_ERR_PRINT ("** "SCI_TYPE"_I2C Master Write operation failed ! **\r\n");
            }
            else
            {
                ...
                /* Print J-link RTT Viewer message */
                APP_PRINT ("** "SCI_TYPE"_I2C Master Write operation is successful **\r\n");
            }
        ...
        case MASTER_READ:
        {
            ...
            {
                /* Print J-link RTT Viewer message */
                APP_ERR_PRINT ("** "SCI_TYPE"_I2C Master Read operation failed ! **\r\n");
            }
            else
            {
                /* Toggle LED as sign of success */
                ...
                /* Print J-link RTT Viewer message */
                APP_PRINT ("** "SCI_TYPE"_I2C Master Read operation is successful **\r\n");
            }
    ...
    while( (I2C_MASTER_EVENT_RX_COMPLETE != g_master_event) || (I2C_SLAVE_EVENT_TX_COMPLETE != g_slave_event) )
    {
        ...
        else
        {
            ...
            if (RESET_VALUE == read_time_out)
            {
                ...
                APP_ERR_PRINT ("** No event received during Master read and Slave write operation **\r\n");
...
static fsp_err_t sci_i2c_master_write(void)
{
    ...
    while ( (I2C_MASTER_EVENT_TX_COMPLETE != g_master_event) || (I2C_SLAVE_EVENT_RX_COMPLETE != g_slave_event) )
    {
        ...
        else
        {
            ...
            {
                /* We have reached to a scenario where i2c event not occurred */
                APP_ERR_PRINT ("** No event received during Master Write and Slave read operation **\r\n");
...
#if (BSP_PERIPHERAL_IICA_PRESENT)
                err = R_IICA_SLAVE_Read(&g_i2c_slave_ctrl, g_slave_rx_buf, BUF_LEN);
#elif defined (BOARD_RA6T2_MCK)
...
#if (BSP_PERIPHERAL_IICA_PRESENT)
                err = R_IICA_SLAVE_Write(&g_i2c_slave_ctrl, g_slave_rx_buf, BUF_LEN);
#elif defined (BOARD_RA6T2_MCK)
...
/*******************************************************************************************************************//**
 * @brief     Toggle on-board LEDs that are connected and supported by the BSP.
 * @param[IN] None
 * @retval    None
 **********************************************************************************************************************/
static void toggle_led(void)
...
/*******************************************************************************************************************//**
 * @brief     Closes the SCI_I2C master and slave modules using HAL-level APIs.
...
void deinit_i2c_driver(void)
{
...
#if (BSP_PERIPHERAL_IICA_PRESENT)
    err = R_IICA_SLAVE_Close(&g_i2c_slave_ctrl, g_slave_rx_buf, BUF_LEN);
#elif defined (BOARD_RA6T2_MCK)
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** R_IIC"SLAVE_TYPE"_Close API FAILED **\r\n");
    }
}
