...
void filex_thread_entry(void)
{
    ...
    while (true)
    {
        /* Perform file operations using block media QSPI with LevelX */
...
/*******************************************************************************************************************//**
 * @brief       Perform file operations using block media QSPI with LevelX.
 * @param[IN]   None
...
static UINT filex_qspi_levelx_operation(void)
{
    ...
    switch(read_data)
    {
            ...
            /* Create a text file and write pre-defined data in it */
        {
            ...
            /* Set date and time */
            status = fx_file_date_time_set(&g_fx_media0, file_name1, g_set_time.year, \
                                           g_set_time.month, g_set_time.date,\
             ...
             * Compare read data with write data and print result */
        {
            ...
            /* Set date and time */
            status = fx_file_date_time_set(&g_fx_media0, file_name2, g_set_time.year, \
                                           g_set_time.month, g_set_time.date,\
                                           g_set_time.hour, g_set_time.min, g_set_time.sec);
            if (FX_SUCCESS != status)
            {
...
/*******************************************************************************************************************//**
 * @brief       De-initializes opened media.
 * @param[IN]   None
 * @retval      None
 **********************************************************************************************************************/
static void deinit_media(void)
