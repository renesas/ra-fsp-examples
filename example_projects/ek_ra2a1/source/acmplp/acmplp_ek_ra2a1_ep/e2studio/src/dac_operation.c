...
/*****************************************************************************************************************
 * This function is called to Stop the D/A conversion using its HAL level API.
 * @brief    Stop the D/A conversion. Handle the Error internally with Proper Message.
 * @param[in] None
 * @retval    FSP_SUCCESS         R_DAC conversion stopped successfully.
 * @retval    error               Any Other Error code apart from FSP_SUCCES Unsuccessful stop.
 ****************************************************************************************************************/
fsp_err_t stop_dac(void)
{
    ...
    /* Stopping Conversion */
    error = R_DAC_Stop(&g_dac_ctrl);
    if (FSP_SUCCESS != error)
    {
        APP_ERR_PRINT ("\r\n ** R_DAC_STOP API FAILED ** \r\n");
    }
    return error;
}
