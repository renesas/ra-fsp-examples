...
fsp_err_t adc_reading(uint16_t *p_adc_data)
{
    ...
    fsp_err = R_ADC_ScanStart (g_adc.p_ctrl);
    if (FSP_SUCCESS != fsp_err)
    {
        return fsp_err;
    }
    ...
    do
    {
        fsp_err = R_ADC_StatusGet (g_adc.p_ctrl, &p_status);
        if (FSP_SUCCESS != fsp_err)
        {
            return fsp_err;
        }
    ...
    fsp_err = R_ADC_Read (g_adc.p_ctrl, ADC_CHANNEL_TEMPERATURE, p_adc_data);
    if (FSP_SUCCESS != fsp_err)
    {
        return fsp_err;
    }

    fsp_err = R_ADC_ScanStop (g_adc.p_ctrl);
    if (FSP_SUCCESS != fsp_err)
    {
        return fsp_err;
    }
