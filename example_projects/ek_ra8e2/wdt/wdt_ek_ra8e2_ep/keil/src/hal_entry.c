...
void hal_entry(void)
{
    ...
    APP_PRINT ("\r\nThe user input initializes the WDT and starts GPT timer");
...
#elif defined (BOARD_RA4W1_EK) || defined (BOARD_RA8E2_EK)
    APP_PRINT ("\r\nOn pressing the Push button SW1, WDT counter stops refreshing");
#elif defined (BOARD_RA8T1_MCK)
    APP_PRINT ("\r\nOn pressing the Push button on PMOD BTN, WDT counter stops from refreshing");
#else
    APP_PRINT ("\r\nOn pressing the Push button S1, WDT counter stops refreshing");
#endif
    ...
    if(FSP_SUCCESS != err)
    {
        /* External IRQ initialize failed, cleanup the timer initialization */
 ...
 *              Any Other Error code apart from FSP_SUCCES  Unsuccessful to perform operation
 **********************************************************************************************************************/
