...
void rtt_thread_entry(void)
{
    ...
    while (1)
    {
        ...
        if (APP_CHECK_DATA)
        {
            /* To observe the key printing on notepad for PHID project */
            APP_PRINT("\r\nPlease open Notepad within 3 seconds\r\n");

            /* Delay 3 seconds for the user to open notepad */
             tx_thread_sleep(DELAY_3000_MS);
            ...
            if (TX_SUCCESS != err)
            {
                APP_PRINT("memory_allocate_rtt has failed, consider increasing byte pool memory and"\
                          " confirm de-allocation of the memory.\r\n");
            }
            else
            {
...
UINT app_rtt_print_data(event_id_t id, uint32_t size, void * const p_data)
{
    ...
    if (TX_SUCCESS != err)
    {
        APP_PRINT("\r\nmemory_allocate_rtt has failed, consider increasing byte pool memory and"\
                  " confirm de-allocation of the memory.\r\n");
...
/*******************************************************************************************************************//**
 * @brief      The user input read function. To be called by user thread for receiving RTT input.
