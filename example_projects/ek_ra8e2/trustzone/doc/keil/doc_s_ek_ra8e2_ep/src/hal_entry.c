...
#if BSP_TZ_SECURE_BUILD
/* Note:
 * The compiler uses Registers R0 to R3 to pass parameters and return values. Registers R4 to R12 are
 * used during function execution.The called function restores registers R4 to R12.
 * Hence, if a NSC API is being used for a Secure function with more than 4 arguments, the Guard function
 * should define a function with a different prototype that will be a funnel to handle all the arguments */
...
{
	if (BSP_IO_PORT_02_PIN_05 == pin) /* Only allowing access to LED1 (P205) */
	{
