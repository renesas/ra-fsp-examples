...
rtc_callback_args_t rtc_callback_args;
...
void hal_entry(void)
{
    ...
    err = g_rtc_callback_set_guard(NULL, rtc_callback, NULL, &rtc_callback_args);
    /* Handle error */
    if(FSP_SUCCESS != err)
    {
        rtc_deinit();
        APP_ERR_PRINT("\r\nrtc_callback_set_guard API failed.Closing all drivers. Restart the Application\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    while(true)
    {
        ...
        if (SET_FLAG == g_alarm_irq_flag)
        {
            ...
            err = g_rtc_calendar_time_get_guard(NULL, &g_current_time);
            if (FSP_SUCCESS != err)
            {
                rtc_deinit();
            ...
            APP_PRINT("\r\n RTC Alarm occurred at  Date : %d/%d/%d \n Time  : %d : %d : %d \r\n\n",
                    g_current_time.tm_mday, g_current_time.tm_mon,
                    g_current_time.tm_year, g_current_time.tm_hour,
