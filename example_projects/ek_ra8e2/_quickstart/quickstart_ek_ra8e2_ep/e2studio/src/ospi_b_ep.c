...
static fsp_err_t ospi_b_write_enable ()
{
    ...
    err = R_OSPI_B_DirectTransfer(&g_ospi_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
//    APP_ERR_RETURN(err, "R_OSPI_B_DirectTransfer API FAILED \r\n");
    ...
    err = R_OSPI_B_DirectTransfer(&g_ospi_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
//    APP_ERR_RETURN(err, "R_OSPI_B_DirectTransfer API FAILED \r\n");
...
fsp_err_t ospi_b_init ()
{
    ...
    if(g_ospi_ctrl.open == 0)
    {
        err = R_OSPI_B_Open(&g_ospi_ctrl, &g_ospi_cfg);
        if(err != FSP_SUCCESS) print_to_console("R_OSPI_B_Open API FAILED \r\n");
    }

    err = R_OSPI_B_SpiProtocolSet(&g_ospi_ctrl, SPI_FLASH_PROTOCOL_EXTENDED_SPI);
...
fsp_err_t ospi_b_set_protocol_to_opi ()
{
    ...
    err = R_OSPI_B_SpiProtocolSet(&g_ospi_ctrl, SPI_FLASH_PROTOCOL_8D_8D_8D);
