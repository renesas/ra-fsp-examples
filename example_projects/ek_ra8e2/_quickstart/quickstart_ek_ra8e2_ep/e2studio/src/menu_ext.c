...
#define GPT_PCLKD_CLOCK_MHZ                     (120)
...
#define TEST_ADDRESS                            (OSPI_START_ADDRESS + (63 * 1024 * 1024))
...
//#define OSPI_TEST_DATA_LENGTH  (0x30U)
...
#define OSPI_RESET_DELAY                        (200U)

#define MAX_BLOCK_SIZE                          (64 * 1024)

/* Limit test to 4K as UNHYSA bit is set so device in hybrid mode where the first few sectors are 4k blocks. */
...
const char_t test_data[64] = "Mary had a little lamb, and some other waffle that takes line 2";
...
static uint8_t *const gp_ospi_cs1 = (uint8_t *) TEST_ADDRESS;
...
st_rgb565_text_block_page_t g_ext_page =
{
    ...
    " OCTO-SPI SPEED DEMONSTRATION", /* Title */
    {
       ...
       {{PAGE_LEFT_EDGE, LINE(2)  + BLOCK(0)},"Compares the write and read times to and from external"},
       {{PAGE_LEFT_EDGE, LINE(3)  + BLOCK(0)},"Octo-SPI flash memory."},
...
static clk_settings_t clk_120MHz =
{
    .pll =
    {
        .source_clock = CGC_CLOCK_MAIN_OSC,
        .divider      = BSP_CLOCKS_PLL_DIV_2,
        .multiplier   = BSP_CLOCKS_PLL_MUL(80U, 0U),
        .out_div_p    = CGC_PLL_OUT_DIV_2,
        .out_div_q    = CGC_PLL_OUT_DIV_6, // Not used, set to div / 6 (default)
        .out_div_r    = CGC_PLL_OUT_DIV_6, // Not used, set to div / 6 (default)
    },
    .div = (bsp_clocks_octaclk_div_t) BSP_CLOCKS_OCTA_CLOCK_DIV_4,
};

/* Block in RAM for performance measurement */
uint8_t read_block[MAX_BLOCK_SIZE];
 ...
 * Description  : Set the OSPI flash clock frequency
 * @param[in] clk_settings_t const * const clock_settings pointer to clock settings
 * @retval None
 *********************************************************************************************************************/
        ...
        uint32_t *ospi_performance_read_result)
{
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        {
            err = g_ospi.p_api->erase(g_ospi.p_ctrl, (uint8_t*) (gp_ospi_cs1 + (erase_size * nb)), erase_size);
    ...
    /* increase the clock frequency to 120MHz */
    oclk_change(&clk_120MHz);
...
test_fn ext_display_menu(void)
{
    ...
    while (false == valid_block_size)
    {
        ...
        switch (value)
        {
            ...
            {
                print_to_console("\r\n> Invalid character in entry, enter the text block size specifying a 2K boundary"
                         " (e.g. 24) and press tab: ");
                break;
            }
    ...
    if ((MENU_ENTER_RESPONSE_CRTL == c) && (0 != block_size_actual))
    {
        ...
        ospi_write_result =  ospi_performance_write_result / GPT_PCLKD_CLOCK_MHZ;
        ospi_read_result  =  ospi_performance_read_result / GPT_PCLKD_CLOCK_MHZ;
...
static void wait_for_write(void)
{
    ...
    do
    {
        assert(FSP_SUCCESS == g_ospi.p_api->statusGet (g_ospi.p_ctrl, &status));
    }
...
static fsp_err_t write_test_opi(uint32_t num_blocks)
{
    ...
    for (s_test_current_block = 0; s_test_current_block < num_blocks; s_test_current_block++)
    {
        ...
        err = g_ospi.p_api->write (g_ospi.p_ctrl, (g_test_data),
