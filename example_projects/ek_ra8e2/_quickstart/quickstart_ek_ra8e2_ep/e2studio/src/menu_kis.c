 ...
 * Version      : 1.0
 * Description  : Kit Information Screen. Manages demo interfacce to console and lcd.
 **********************************************************************************************************************/
/**********************************************************************************************************************
...
st_rgb565_text_block_page_t g_kis_page = {
    ...
    " KIT Information", /* Title */
    {
       ...
       {{PAGE_LEFT_EDGE, LINE(7)},"RA MCU Die temperature (F/C)"},
       {{PAGE_1ST_TAB,   LINE(7)},""},

       {{PAGE_LEFT_EDGE, LINE(8)},"Blue LED blinking frequency (Hz)"},
       {{PAGE_1ST_TAB,   LINE(8)},""},
       {{PAGE_LEFT_EDGE, LINE(9)},"Blue LED blinking intensity (%)"},
       {{PAGE_1ST_TAB,   LINE(9)},""},
    }
};
...
test_fn kis_display_menu(void)
{
    ...
    sprintf(mod_uuid, "%08x-%08x", (int_t) p_uid->unique_id_words[0], (int_t) p_uid->unique_id_words[1]);
    toupperstr((char_t *) &mod_uuid, (uint32_t) strlen(mod_uuid));
    sprintf(g_kis_page.text_block[7].line,"%s",mod_uuid);

    sprintf(mod_uuid, "%08x-%08x",(int_t) p_uid->unique_id_words[2], (int_t) p_uid->unique_id_words[3]);
    toupperstr((char_t *) &mod_uuid, (uint32_t) strlen(mod_uuid));
    sprintf(g_kis_page.text_block[8].line,"%s",mod_uuid);

    sprintf(g_kis_page.text_block[10].line, "%d.%02d\'F/%d.%02d\'C",wn_mcu_temp_f, fr_mcu_temp_f, wn_mcu_temp_c, fr_mcu_temp_c);

    dsp_set_background(LCD_FULL_BG_KIT_INFORMATION);
    
    /* Read the console termial to get any characters already pressed. */
    start_key_check();
