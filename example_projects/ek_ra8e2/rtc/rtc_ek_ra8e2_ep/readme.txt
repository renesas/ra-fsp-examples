...
1. Project Overview:
     	...
     	The user can enable periodic interrupt and can also view the current RTC calendar time.
     	On enabling periodic interrupt, on-board LED toggles every 1 second and on occurrence of Calendar alarm,
     	LED is turned ON.
	
Note:
1) The user is expected to use J-Link RTT Viewer with input will be sent when pressing Enter: Input -> Sending... -> Send on Enter

2) The user is expected to Set RTC Calendar Date and Time (option 1) before other runnings. 

3) The user is expected to enter Time values to set Calendar time and Calendar alarm in 24-hours format.
	Sample Input: 23:10:2019 16:14:55 (DD:MM:YYYY HH:MM:SS)

4) The user is expected to enter data not exceeding 15 bytes in size.
