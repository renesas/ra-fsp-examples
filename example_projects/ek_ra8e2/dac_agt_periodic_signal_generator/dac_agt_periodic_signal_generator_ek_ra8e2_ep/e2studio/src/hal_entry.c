...
void hal_entry(void)
{
    ...
    while(1)
    {
        /* Prompt the user to input desired signal's samples per period and check that it is within the valid range */
        APP_PRINT("\r\nEnter desired samples per period for the output sine wave."\
        ...
        determined by the relation: dac_max_samp_rate = freq_max_val/input_spp */
        freq_max_val = (uint32_t)(dac_max_samp_rate/ input_spp);

        /* Prompt the user to input desired signal's frequency and check that it is within the valid range */
        APP_PRINT("\r\nEnter desired frequency for output sine wave, in Hz."\
                  " Maximum input frequency: %d Hz\r\n", freq_max_val);
        is_not_valid = 1;
        ...
        /* Get PCLKB frequency */
        volatile uint32_t timer_freq_hz = R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_PCLKB) >> g_timer0_cfg.source_div;
