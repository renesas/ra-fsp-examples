...
void sender_task_entry(void *pvParameters)
{
    ...
    /* Start TAU timer to start sending message to Message Queue periodically at 1sec from TAU callback */
            ...
            if ( pdPASS == xQueueReset (g_queue) )
            {
                ...
                if ( FSP_SUCCESS != fsp_err )
                {
                    if ( FSP_SUCCESS != R_TAU_Close (&g_periodic_timer_msgq_ctrl))
                    {
                        ...
                        APP_ERR_PRINT ("\r\nSender_Task : TAU Timer Close API failed\r\n");
                    }
                    /* Print out in case of error */
                    APP_ERR_PRINT ("\r\nSender_Task : TAU Timer stop API failed\r\n");
                    APP_ERR_TRAP (fsp_err);
                }
...
/*******************************************************************************************************************//**
 * @brief      User defined TAU irq callback for Message Queue.
 *
