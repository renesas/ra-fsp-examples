...
void wifi_thread_entry(void *pvParameters)
{
        ...
        switch(index_menu_option)
        {
            case SCAN_AP:
                ...
                else
                {
                    ...
                    if(FSP_SUCCESS != err)
                    {
                        APP_ERR_PRINT("\r\n** rm_wifi_onchip_silex_connect API "\
                                      "for rm_wifi_onchip_silex failed ** \r\n");
    ...
    APP_PRINT("\r\nNote: Please give free port number, otherwise TCP connect would fail and application would "\
              "also fail.\r\n");
    user_input(input_buff);
