 ...
 * Description  : Contains macros data structures and functions used in common_utils.c
 **********************************************************************************************************************/
/***********************************************************************************************************************
...
UINT rtt_frameowrk_init(VOID)
{
    ...
    err = tx_queue_create(&rtt_ip_data_queue, (CHAR *)"rtt_input_queue", 1, &queue_memory_ip_rtt_obj, sizeof(queue_memory_ip_rtt_obj));
    if (TX_SUCCESS !=err)
    {
        tx_byte_pool_delete(&byte_pool);
 ...
 *  as RTT Viewer input comes along with \n bytes
 */
void remove_newline(char *dest_str,char *input_str)
{
