...
static int cgiGetTime (PSESS pSess, PEOFILE pEoFile)
{
    ...
    /*  This division text must match the content in the website file it is linked to the 
        section (div) <div id="realTimeClock"> the entire block must be replicated here. */

#ifdef NEWWEBSITE1
    wi_printf(pSess,
            "<div>" \
            "<div class=\"mb-2 font-semibold\">Device ID</div>" \
            "<div class=\"ml-1\">%08x - %08x</div>" \
            "<div class=\"ml-1\">%08x - %08x</div>" \
            "</div>" \
            "<div>" \
            "<div class=\"mb-2 font-semibold\">MCU Temperature (F): %d.%d</div>" \
            "<div class=\"mb-2 font-semibold\">MCU Temperature (C): %d.%d</div>" \
            "<div class=\"mb-2 font-semibold\">Blue LED Attributes</div>" \
            "<div class=\"ml-1\">Frequency (Hz): %d</div>" \
            "<div class=\"ml-1\">Intensity (%%): %d</div>" \
            "</div>",
