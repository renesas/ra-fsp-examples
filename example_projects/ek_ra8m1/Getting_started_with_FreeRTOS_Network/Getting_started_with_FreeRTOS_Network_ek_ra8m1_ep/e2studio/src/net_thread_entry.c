...
uint8_t *gp_domain_name       = (uint8_t *)USR_TEST_DOMAIN_NAME;
    ...
    */
    uint8_t g_mac_address[MAC_ADDR_SIZE]         = {0x74, 0x90, 0x50, 0x65, 0x81, 0x6D};
...
* @retval     return PING_SUCCESS if ping to the IP address is successful, otherwise returns PING_TIMEOUT.
***********************************************************************************************************************/
...
static void print_ipconfig(void)
{
    ...
    APP_PRINT("\tDNS Servers . . . . . . . . . . . : %d.%d.%d.%d\r\n",g_dns_server_address[0],g_dns_server_address[1],g_dns_server_address[2],g_dns_server_address[3]);
}
