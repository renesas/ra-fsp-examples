...
void hal_entry(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        /* close KINT and IO port modules */
        cleanup();
    ...
    while (true)
    {
    	...
    	{
    		/* enter critical section to ensure capture of g_kint_signal value*/
    		...
    		/* restore initial Row pins level */
    		ROW_INIT_LEVEL_SET();
...
static void process_row_identification(uint8_t kint_pin)
{
	...
	VALIDATE_IO_PORT_API(R_IOPORT_PortRead(&g_ioport_ctrl, KEYPAD_PORT, &port_data))
	...
	/* Check KINT pin state when ROW_B P102 is set as low */
	ROW_A_HIGH();
	ROW_B_LOW();
