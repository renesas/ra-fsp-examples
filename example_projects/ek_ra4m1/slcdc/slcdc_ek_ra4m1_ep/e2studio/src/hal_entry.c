...
static fsp_err_t slcd_init(void)
{
     ...
     * R_SLCDC_Start to allow the boost circuit to charge. See RA4M1 User's Manual (R01UH0887EJ0100) section 45.2.2 Notes. */
...
static fsp_err_t set_segments_digits( uint32_t value)
{
    ...
    if (HUNDREDS_DIGIT_MAX_VAL < value)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT ("\r\n ** R_SLCDC_Modify API FAILED AT THOUSANDS PLACE ** \r\n");
            set_err_segments();
            return err;
        }
    ...
    if (TENS_DIGIT_MAX_VAL < value)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT ("\r\n ** R_SLCDC_Modify API FAILED AT HUNDREDS PLACE ** \r\n");
            set_err_segments();
            return err;
        }
    ...
    if (UNIT_DIGIT_MAX_VAL < value)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT ("\r\n ** R_SLCDC_Modify API FAILED AT TENS PLACE ** \r\n");
            set_err_segments();
            return err;
        }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT ("\r\n ** R_SLCDC_Modify API FAILED AT UNITS PLACE **");
        set_err_segments();
        return err;
    }
