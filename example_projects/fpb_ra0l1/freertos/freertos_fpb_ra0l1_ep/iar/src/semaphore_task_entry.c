...
void semaphore_task_entry(void *pvParameters)
{
    ...
    /* Start TAU timer to 'Give' Semaphore periodically at 1 second for semaphore_task */
        ...
        else
        {
            ...
            if (FSP_SUCCESS != fsp_err)
            {
                if (FSP_SUCCESS != R_TAU_Close(&g_periodic_timer_sem_ctrl))
                {
                    ...
                    APP_ERR_PRINT("\r\nSemaphore Task: R_TAU_Close API failed\r\n");
                }
                /* Print out in case of error */
                APP_ERR_PRINT("\r\nSemaphore Task: R_TAU_Stop API failed\r\n");
                APP_ERR_TRAP(fsp_err);
            }
...
/*******************************************************************************************************************//**
 * @brief      User-defined TAU IRQ callback for semaphore handling.
...
void periodic_timer_semphr_cb(timer_callback_args_t *p_args)
{
    ...
    else
    {
        ...
        /* Set the flag to suspend the semaphore task */
        semphr_counter++;
    }
