 ...
 * @param[IN]  	flash_type			    Macro indicating data flash or code flash operations.
...
uint8_t block_num_input_and_validation(uint8_t flash_type)
{
    ...
    if (DATA_FLASH_OPERATIONS == flash_type)
    {
        ...
        APP_PRINT("\r\nEnter a valid block number for Data Flash operations:\r\n");
        ...
        while ((FLASH_DF_BLOCK0 >= FLASH_DF_BLOCK(block_num)) || (FLASH_DF_BLOCK_END < FLASH_DF_BLOCK(block_num)))
        {
            ...
            {
                APP_PRINT("\r\nThe entered Data Flash block number is not valid\r\n");
                APP_PRINT("Please enter a valid block number for Data Flash operations\r\n");
    ...
    else if (CODE_FLASH_OPERATIONS == flash_type)
    {
        ...
        APP_PRINT("\r\nEnter a valid block number for Code Flash operations\r\n");
        ...
        while ((FLASH_CF_BLOCK10 > FLASH_CF_BLOCK(block_num)) || (FLASH_CF_BLOCK_END < FLASH_CF_BLOCK(block_num)))
        {
            ...
            if(('0' <= rByte[ARRAY_INDEX]) && ('9' >= rByte[ARRAY_INDEX]))
            {
                ...
                else
                {
                    ...
                    APP_PRINT("Please enter a valid block number for Code Flash operations\r\n");
                    num_bytes = RESET_VALUE;
                    continue;
                }
            }
