...
#elif (BSP_PERIPHERAL_UARTA_PRESENT == 1U)
...
{
    /* De-initialize SCI UART if it is opened */
    if (g_uart_ctrl.open)
    {
#if (BSP_FEATURE_SCI_VERSION == 1U)
...
fsp_err_t uart_ep_info_print(void)
{
    ...
    /* Print EP version and FSP version to terminal on the host PC via SCI UART */
    ...
    /* Print EP information to terminal on the host PC via SCI UART */
...
fsp_err_t uart_error_info_print(uint8_t * p_func, uint8_t * p_info)
{
    ...
    /* Print to terminal on the host PC via SCI UART */
    err = uart_print(uart_buffer);
    return err;
}
