 ...
 * @param[IN]   g_time_cancel_lpm   The period time from the user input.
 * @retval      FSP_SUCCESS         Set the period time for counter in LPM operation successfully.
...
static fsp_err_t lpm_enter_procedure(app_lpm_states_t lpm_mode)
{
    ...
    {
        APP_PRINT("\r\nPlease enter the period time for counter in Sleep mode in seconds");
    }
    else if (APP_LPM_SW_STANDBY_STATE == lpm_mode)
    {
        APP_PRINT("\r\nPlease enter the period time for counter in SW Standby mode in seconds");
    }
    else
    {
        /* Do nothing */
