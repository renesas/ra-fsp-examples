...
uint8_t *gp_remote_ip_address = (uint8_t *) USR_TEST_PING_IP;
    ...
    static uint8_t ucMACAddress[6]       = {0x74, 0x90, 0x50, 0x65, 0x81, 0x70};
    ...
    while (true)
    {
        ...
        if (SUCCESS == isNetworkUp())
        {
            ...
            if (!(PRINT_NWK_USR_MSG_DISABLE & usr_print_ability))
            {
                ...
                dnsQueryFunc((char *)gp_domain_name);
            }
...
/*******************************************************************************************************************//**
 * @brief      Creates and prints the IP configuration to display on the RTT console.
 * @param[in]  None.
 * @retval     None.
 **********************************************************************************************************************/
