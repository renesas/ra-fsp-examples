 ...
 **********************************************************************************************************************/
static uint8_t g_ip0_stack_memory[G_IP0_TASK_STACK_SIZE] BSP_PLACE_IN_SECTION(".stack.g_ip0")\
...
uint8_t g_packet_pool0_pool_memory[(G_PACKET_POOL0_PACKET_NUM * (G_PACKET_POOL0_PACKET_SIZE +\
                                   sizeof(NX_PACKET)))] BSP_ALIGN_VARIABLE(4);;
#else
uint8_t g_packet_pool0_pool_memory[(G_PACKET_POOL0_PACKET_NUM * (G_PACKET_POOL0_PACKET_SIZE +\
                                   sizeof(NX_PACKET)))] BSP_ALIGN_VARIABLE(4) ETHER_BUFFER_PLACE_IN_SECTION;;
#endif
...
/*******************************************************************************************************************//**
 * @brief     This function is called when the link status changes. It sets the event flag to notify the system
 *            of the link status change.
...
/*******************************************************************************************************************//**
 * @brief     This function manages the network link status and handles events related to link changes.
 *            t also starts the network application thread and manages link-up and link-down events.
 * @param[in] None.
 * @retval    None.
 **********************************************************************************************************************/
void net_link_thread_entry(void)
{
    ...
    while (true)
    {
        ...
        /* Handle the physical link-up event */
        ...
        /* Handle the physical link-down event */
...
/*******************************************************************************************************************//**
 * @brief     This function performs the following tasks:
...
/*******************************************************************************************************************//**
 * @brief     This function checks if the DHCP client has resolved the IP address.
 *            If not, it enables the network link, stops the DHCP client service, reinitializes the DHCP client,
 *            and starts the service again. Finally, it retrieves the current network configuration details.
 * @param[in] None.
 * @retval    NX_SUCCESS on successful operation, other error codes otherwise.
 **********************************************************************************************************************/
static UINT net_system_link_up_process(void)
...
/*******************************************************************************************************************//**
 * @brief      This function gets the MAC address, IPv4 address, subnet mask, default gateway, and DHCP server
 *             address for the specified network.
 * @param[out] p_info   Pointer to a net_info_t structure where the retrieved network information will be stored.
 * @retval     NX_SUCCESS on successful operation, other error codes otherwise.
 **********************************************************************************************************************/
static UINT net_system_get_info(net_info_t * p_info)
{
    ...
    TERM_ERR_TRAP(NX_SUCCESS != status, status,\
                  "nx_dhcp_user_option_retrieve for retrieving the default gateway's IP address failed");
    ...
    TERM_ERR_TRAP(NX_SUCCESS != status, status,\
                  "nx_dhcp_user_option_retrieve for retrieving the DHCP server's IP address failed");
...
/*******************************************************************************************************************//**
 * @brief     This function allocates a packet for the ping command, sends a ping request, and releases the
 *            allocated packet afterward.
