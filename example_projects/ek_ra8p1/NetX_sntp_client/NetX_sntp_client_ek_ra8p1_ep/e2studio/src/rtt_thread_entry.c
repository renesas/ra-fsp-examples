        ...
        if (err == TX_SUCCESS)
        {
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                case RTT_OUTPUT_MESSAGE_APP_GET_SERVER_STR:
                {
                    ...
                    APP_PRINT("\r\nINFO: Retrieving the server IP address from the URL: %s...", server_url);
                }
                break;
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_SERVER_STR:
                {
                    UCHAR server_url[SERVER_URL_LEN] = {'\0'};
                    memcpy(server_url, (UCHAR*)rtt_op_data->p_msg, sizeof(server_url));
                    APP_PRINT("\r\nINFO: Successfully acquired the server IP address from the SNTP URL: %s",
                              server_url);
                }
                break;
                ...
                {
                    rtc_time_t print_time = {0};
                    memcpy(&print_time, (rtc_time_t *)rtt_op_data->p_msg, sizeof(rtc_time_t));
                    /* Print date and time in user-readable format */
                    APP_PRINT("Date (DD/MM/YYYY): %02d/%02d/%d\r\nTime (HH:MM:SS)  : %02d:%02d:%02d\r\n",
                              print_time.tm_mday, print_time.tm_mon, print_time.tm_year, print_time.tm_hour,
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_IP:
                {
                    ...
                    sprintf(ipv4_address, "%d.%d.%d.%d", client_ip.IP_t.ip_a, client_ip.IP_t.ip_b, client_ip.IP_t.ip_c,
                            client_ip.IP_t.ip_d);
                    APP_PRINT("\r\nINFO: The SNTP client IP address is %s", ipv4_address);
                }
                break;
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_SERVER_IP:
                {
                    union IP server_ip;
                    char ipv4_address[64] = {'\0'};
                    /* Assign IPv4 address to the local buffer */
                    ULONG server_addr = *(ULONG*)rtt_op_data->p_msg;
                    server_ip.ip_address = server_addr;
                    sprintf(ipv4_address, "%d.%d.%d.%d", server_ip.IP_t.ip_a, server_ip.IP_t.ip_b, server_ip.IP_t.ip_c,
                            server_ip.IP_t.ip_d);
                    APP_PRINT("\r\nINFO: The SNTP server IP address is %s", ipv4_address);
                }
                break;
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_DNS_SERVER_IP:
                {
                    ...
                    APP_PRINT("\r\nINFO: Successfully pinged the DNS Server: %s", ipv4_address);
                }
                break;
