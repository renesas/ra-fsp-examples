...
void rtt_thread_entry(void)
{
    ...
    if (TX_SUCCESS != err)
    {
        APP_PRINT("\r\nERROR: RTT framework initialization failed\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    while (1)
    {
        ...
        process_rtt_op_msg();
#if (BSP_CFG_RTOS == 1)
        /* 1-tick sleep is needed to signal to higher priority threads */
        tx_thread_sleep (1);
#endif
    }
}
    ...
    do
    {
        ...
        if(err == TX_SUCCESS)
        {
            ...
            switch(rtt_op_data->msg_id)
            {
                ...
                break;
                case RTT_OUTPUT_MESSAGE_APP_GENERAL_STR:
                {
                    APP_PRINT("%s\r\n", rtt_op_data->p_msg);
                }
                break;
                ...
                {
                    /* Print NetX WiFi menu options to connect */
                    APP_PRINT(AZURE_WIFI_SCAN_MENU);
                }
                break;
                case RTT_OUTPUT_MESSAGE_MCU_IP_ADDRESS:
                {
                    uint32_t mcu_ip[IP_BUFF_LEN] = {RESET_VALUE};
                    memcpy(mcu_ip, (uint32_t*)rtt_op_data->p_msg, BUFF_LEN);
                    APP_PRINT("\r\nMCU IP address: %d.%d.%d.%d\r\n", mcu_ip[INDEX_ZERO],\
                              mcu_ip[INDEX_ONE], mcu_ip[INDEX_TWO], mcu_ip[INDEX_THREE]);
                }
                break;
                case RTT_OUTPUT_TCP_RECV_LED_STATUS:
                {
                    uint32_t serv_recv_msg[BUFF_LEN] = {RESET_VALUE};
                    memcpy(serv_recv_msg, (uint32_t*)rtt_op_data->p_msg, BUFF_LEN);
                    APP_PRINT("Received: %s, User LED %s\r\n", (char*)serv_recv_msg, (char*)serv_recv_msg);
                }
                break;
                case RTT_OUTPUT_TCP_RECV_INVALID:
                {
                    uint32_t serv_recv_msg[BUFF_LEN] = {RESET_VALUE};
                    memcpy(serv_recv_msg, (uint32_t*)rtt_op_data->p_msg, BUFF_LEN);
                    APP_PRINT("Received: %s, Invalid message. Send 'on' or 'off'\r\n", (char*)serv_recv_msg);
                }
                break;
                case RTT_OUTPUT_MESSAGE_SECURITY:
                {
                    WIFIScanResult_t scan_data =
                    {
                        .ucSSID       = {RESET_VALUE},
                    ...
                    scan_data = *(WIFIScanResult_t*)rtt_op_data->p_msg;
                    APP_PRINT("%s: Signal strength: %d, Channel: %d",\
                              scan_data.ucSSID, scan_data.cRSSI, scan_data.ucChannel);
                }
                break;
                case RTT_OUTPUT_PRINT_INDEX:
                {
                    APP_PRINT("Index:(%d)", *(uint8_t*)rtt_op_data->p_msg);
                }
                break;
                case RTT_OUTPUT_PRINT_SECURITY_MENU:
                {
                    APP_PRINT(AZURE_WIFI_SECURITY_MENU);
                }
                break;
                default:
                {
 ...
 *            This is called by each the user thread wherever the user wishes to
 ...
 * @param[IN] p_buf       Pointer to the user display data.
