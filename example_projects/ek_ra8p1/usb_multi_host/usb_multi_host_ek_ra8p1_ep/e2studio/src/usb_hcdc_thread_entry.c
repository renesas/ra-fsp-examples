        ...
        if (pdTRUE == queue_item_status)
        {
            switch (p_usb_cdc_event_info->event)
            {
                case USB_STATUS_CONFIGURED:
                {
                    ...
                    /* Wait for the USB MSC (Mass Storage Class) command completion event
                     * if USB MSC device is connected */
                ...
                case USB_STATUS_REQUEST_COMPLETE:
                {
                    ...
                    else if (USB_CDC_SET_CONTROL_LINE_STATE == (p_usb_cdc_event_info->
                                                                setup.request_type & USB_BREQUEST))
                    {
                        /* CDC Class request "SetLineCoding" */
...
static void usb_cdc_operation(void)
{
    ...
    else if ((uxBits & IS_CDC_DEVICE_CONNECTED))
    {
        ...
        {
            char input_char = (char) g_new_api_key_str[INDEX_ZERO];
