...
1. Project Overview:
	...
	perform file operations on the SD Card. Error and info messages will be printed on J-Link RTT Viewer.

2. Hardware Requirements:
	Supported RA boards: EK-RA6M1, EK-RA6M2, EK-RA6M3, EK-RA6M3G, EK-RA6M4, EK-RA6M5, FPB-RA6E1, EK-RA8M1,
			     EK-RA8D1, MCK-RA8T1, EK-RA8P1.
	...
	1 x Pmod SD (Except for MCK-RA8T1. e.g., https://digilent.com/shop/pmod-sd-full-sized-sd-card-slot/).
...
2) Regarding the power supply pin: By default, the user can use Pmod SD GND (J1:5) and Pmod SD VCC (J1:6) as shown in
   the hardware connection, or alternatively, Pmod SD GND (J1:11) and Pmod SD VCC (J1:12).

3) Format the SD Card before performing any operation.

4) Data written can also be manually verified in file ra_sdhi.txt.

5) The user is expected to execute "Safely Remove the SD Card" option before removing SD Card else further file
   operations may fail and SD Card data may get corrupted.

6) The user must remove the SD Card after executing "Safely Remove the SD Card" option, and re-insert it after
   executing "Initialize FreeRTOS+FAT" option.
