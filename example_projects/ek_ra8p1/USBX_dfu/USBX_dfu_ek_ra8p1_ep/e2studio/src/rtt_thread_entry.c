...
UINT app_rtt_print_data(event_id_t id, uint32_t size, void * const p_data)
{
    ...
    err = memory_allocate_rtt(&byte_pool, &p_display_data, sizeof(rtt_msg_t) + size);
    if (TX_SUCCESS != err)
    {
 ...
 *             message once received through queue (be it RTT sender or receiving queue) is released.
