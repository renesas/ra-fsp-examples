...
fsp_err_t ulpt_periodic_operation(void)
{
    ...
    while (TIMES_MAX > count_value)
    {
        if (true == g_periodic_timer_flag)
        {
            ...
            {
                R_IOPORT_PinRead(&g_ioport_ctrl, g_bsp_leds.p_leds[LED_USE], (bsp_io_level_t *)&led_state);
    ...
    APP_PRINT("\r\nULPT1 in periodic mode stopped\r\n");

    return err;
}

/*******************************************************************************************************************//**
 * @brief       This function demonstrates a One-Shot timer operation in low power mode.
 * @param[in]   None.
 * @retval      FSP_SUCCESS Upon successful operation.
 * @retval      Any other error code apart from FSP_SUCCESS.
 **********************************************************************************************************************/
