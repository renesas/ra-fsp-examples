        ...
        if(RxIsData == true)
        {
            if(smc_control_status == SMC_DEVICE_ATR)
            {
                ...
                if(smc_atr_table_pos == 1)
                {
                    ...
                    atr_length += (uint8_t)((have_next_td == 1) ? (uint8_t)(atr_historical_length + atr_interface_length + 2 + 1) :
...
void recveExpireCallback(timer_callback_args_t *p_args)
{
...
void smar_card_detection_cb(external_irq_callback_args_t *p_args)
{
...
fsp_err_t Smart_Card_Interface_Card_Detection(void)
{
    ...
    do{
        current_state = (bsp_io_level_t) R_PFS->PORT[SMCI_DETECT >> 8].PIN[SMCI_DETECT & BSP_IO_PRV_8BIT_MASK].PmnPFS_b.PIDR;
...
/**
  * @brief  Set the smart card baud rate function of F and D parameters and apply
  *         PPS procedure.
...
fsp_err_t Smart_Card_Interface_F_D_confirmation(smc_atr_message_t * sc_returned_atr)
{
    ...
    else
    {
        ...
        if(sc_returned_atr->T0 & 0x10)
        {
            ...
            {
                APP_PRINT("TA1 presents and the card supported asynchronous parameters are different to the interface used\r\n");
                ...
                if(pps1 != 0)
                {
                    APP_PRINT("Used updated asynchronous parameters to update the baud rate setting\r\n");
