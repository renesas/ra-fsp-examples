 ...
 **********************************************************************************************************************/
/* LEDs status */
volatile bsp_io_level_t g_led_1_status = BSP_IO_LEVEL_LOW;
volatile bsp_io_level_t g_led_2_status = BSP_IO_LEVEL_LOW;
...
void led_control_thread_entry(void)
{
    ...
    TERM_ERROR_TRAP(TX_SUCCESS != status, status, "R_ICU_ExternalIrqOpen for the SW1 failed\r\n");
    ...
    TERM_ERROR_TRAP(TX_SUCCESS != status, status, "R_ICU_ExternalIrqOpen for the SW2 failed\r\n");

    /* Create an event flag group for LED control. */
    status = tx_event_flags_create(&g_led_event_flags, "LED Control Events");
    TERM_ERROR_TRAP(TX_SUCCESS != status, status, "tx_event_flags_create for the LED control failed\r\n");

    /* Enable external pins to generate interrupts for SW1. */
    status = (UINT)R_ICU_ExternalIrqEnable(&g_external_irq_sw1_ctrl);
    TERM_ERROR_TRAP(TX_SUCCESS != status, status, "R_ICU_ExternalIrqEnable for the SW1 failed\r\n");

    /* Enable external pins to generate interrupts for SW2. */
    status = (UINT)R_ICU_ExternalIrqEnable(&g_external_irq_sw2_ctrl);
    TERM_ERROR_TRAP(TX_SUCCESS != status, status, "R_ICU_ExternalIrqEnable for the SW2 failed\r\n");
    ...
    while (true)
    {
        ...
        TERM_ERROR_TRAP(TX_SUCCESS != status, status, "tx_event_flags_get for the LED control event failed\r\n");
