...
static bool verify_cryptographic_operation_with_psa_api(sce_aes_wrapped_key_t *wrapped_key_128);
static void key_inject_check(void);
static void deinit_littlefs(void);
static bool init_lfs(void);

/* The set of NIST vectors are used in this example project */
    ...
    *output_len = 0;
    while (bytes_written != c_input_size)
    {
        bytes_to_write = (c_input_size - bytes_written > c_part_size ?
                          c_part_size : c_input_size - bytes_written);

        psa_status = psa_cipher_update(operation, c_input + bytes_written, bytes_to_write,
    ...
    if (FSP_SUCCESS == psa_status)
    {
        ...
        if (PSA_SUCCESS == psa_status)
        {
            ...
            if (PSA_SUCCESS == psa_status)
            {
                ...
                if (PSA_SUCCESS == psa_status)
                {
                    cipher_operation(&operation_aes, enc_plain, BLOCK_SIZE, 10, cipher, 48, &output_length);
                }
