...
static void mqtt_subscription_cb(void *param1, IotMqttCallbackParam_t * const pPublish)
{
    ...
    (void ) param1;
    led_msg_t led_data =    { '\0' };
    led_data.id = ID_LED_ACT;
    strncpy ((char *) &led_data.led_data.led_act_topic_msg, pPayload, pPublish->u.message.info.payloadLength);

    xQueueGenericSendFromISR (g_topic_queue, &led_data, &xHigherPriorityTaskWokenByPost, queueSEND_TO_BACK);
