...
#ifdef USE_SG
#include "ob1203_bio/SAVGOL/SAVGOL.h"
#endif
 ...
 * Return:     none
 ************************************************************************************/
void SPO2_init(struct oxim *_ox, struct ob1203 *_ob,struct spo2 *_sp)
{/*class initializer*/
	_sp->data_ptr = 0;
	_sp->period2use1f = 0;
	_sp->b2b1f = 0;
    ...
    _sp->no_disp_spo2_cnt = 0;
    _sp->spo2_err = 0;
    _sp->ppg2 = _ox->ppg2; /*conform algorithm setting to oximeter HR vs. SpO2 setting*/
}
 ...
 * Return:     none
 ************************************************************************************/
void do_algorithm_part2(struct oxim *_ox, struct ob1203 *_ob, struct spo2 *_sp)
{
	...
	hpf_window_peak_detect(_ox, _sp);
#ifdef USE_WINDOW_RMS
	get_rms_from_sum_abs(_sp);
#endif
	get_dc_from_baseline(_ob,_sp);
	get_pressure(_sp);
	...
	{ //original algorithm part 2 (RMS-based)
		...
		if (_sp->hr_zc > 0)
		{
			_sp->est_hr_period_from_zc1f = ((_sp->hpf_array_len << FIXED_BITS) / _sp->hr_zc)
					* 2;
			...
			else if (_sp->est_hr_period_from_zc1f
					< (int16_t) MIN_HR_PERIOD << FIXED_BITS)
			{
		...
		do_noise_correction(_sp,_ob);
#endif
		calc_R(_sp); //calc PI and R ratio of ratios if ppg2 mode
		if(_sp->ppg2)
		{
			calc_spo2(_sp);
		}
...
#else //USE_WINDOW_P2P
	if( (_sp->ppg2 == 0 || _sp->beat_amp[RED] > 0) && _sp->beat_amp[IR]>0)
	{
...
void calc_spo2(struct spo2 *_sp)
{
	...
	if (_sp->R > 0)
	{
		...
		_sp->current_spo21f =
				((_sp->current_spo21f >> FIXED_BITS) > 100) ?
						100 << FIXED_BITS : _sp->current_spo21f;
		_sp->current_spo21f =
				((_sp->current_spo21f >> FIXED_BITS) < MIN_SPO2) ?
				MIN_SPO2 << FIXED_BITS : _sp->current_spo21f;
#endif
	}
}
	...
	if (_sp->period2use1f > 0)
	{
		_sp->current_hr1f =
				((uint32_t) ((uint32_t) SAMPLE_RATE_PER_MIN << FIXED_BITS)
						<< FIXED_BITS) / (uint32_t) _sp->period2use1f;
	}
	else
	{
		_sp->current_hr1f = 0;
...
{
	/*Called by get_sensor_data in oximeter.c to load new samples into the algorithm's buffer.
	...
	const uint8_t mask = (uint8_t) NUM2AVG - 1;
	/*avoid rollover of 16 bit integer conversion -- constrain values at limits*/
	temp_ir = ((int32_t) ir_data - (int32_t) (_ob->target_counts[IR]));
	if (temp_ir < -32768)
	{
		temp_ir = -32768;
	}
	...
	if(_sp->ppg2)
	{
		temp_r = ((int32_t) r_data - (int32_t) (_ob->target_counts[RED]));
		if (temp_r < -32768)
		{
			temp_r = -32768;
		}
		...
		if (_sp->systole_found != 0)
		{
			...
			if (_sp->do_rr_alg)
			{
				...
				for (int m = 0; m < _sp->max_rr_filter; m++)
				{
					...
					{
						_sp->detrended[n] = _sp->lin_buffer[n + filter_len / 2]
								- _sp->rr_filter[n];
					}
...
//		badness[center_filter] /= CENTER_BIAS;
		...
		if (_sp->max_rr_filter == NUM_RR_FILTERS)
		{
			_sp->max_filter =
					(_K_F[rr_filt].kalman_avg
							> ALLOW_LONGEST_RR_FILTER_THRESHOLD ?
							NUM_RR_FILTERS - 1 : NUM_RR_FILTERS - 2);
		}
		else
		{
			_sp->max_filter =
					(max_rr_filter == NUM_RR_FILTERS - 1) ?
					NUM_RR_FILTERS - 1 :
															NUM_RR_FILTERS - 2;
		}
#else
		...
		else if (_sp->filter_tracker
				> (_sp->max_fom * FILTER_DAMPING + (FILTER_DAMPING >> 1)))
		{
		...
		{ //skipping zero values
			_sp->rr_current_monitor1f = (((uint32_t) MAX_BREATH_FILTER_SPAN * 2)
					<< FIXED_BITS) / _sp->zero_crossings[_sp->current_filter]; //RR period measured in algorithm runs per breath with fixed precision. Includes factor of 2 for 2 crossing per breath
			run_kalman(rr_filt, _sp->rr_current_monitor1f, NO_RATIO);
		}

#ifdef USE_RR_DISPLAY_AVG
		if ((_K_F[rr_filt].kalman_avg >= RR_MIN_PERIOD_1F)
				&& (_K_F[rr_filt].kalman_avg <= RR_MAX_PERIOD_1F))
		{
		...
		int16_t *_iir_rms, int16_t *_iir_rms_smooth)
{
	...
	_iir_rms[arr_num] = (int16_t) uint_sqrt(
			((((uint32_t) new_val - _iir_avg[arr_num])
					* ((uint32_t) new_val - _iir_avg[arr_num])) >> kinv)
					+ (((uint32_t) _iir_rms_smooth[arr_num]
							* (uint32_t) _iir_rms_smooth[arr_num]) >> kinv)); // sqrt(k*(new_val-avg)^2 + (1-k)*(smooth)^2)
		...
		uint8_t polarity)
{
	...
	if (_K_F[hr_filt].kalman_avg > 0)
	{
		if (_K_F[hr_filt].kalman_avg < ((uint16_t)MAX_HR_TO_USE_DEBOUNCING)<<(uint8_t)FIXED_BITS)
		{
			...
			kalman_hr_period = (uint16_t) (((uint32_t) SAMPLE_RATE_PER_MIN)
					<< FIXED_BITS) / (uint32_t) _K_F[hr_filt].kalman_avg;
		}
	}
	...
	int16_t threshold = (int32_t) max_val * ((int32_t) HPF_THRESHOLD_NUM)
			/ ((int32_t) HPF_THRESHOLD_DEN); //putting threshold above zero to reduce detection of dicrotic notches
...
{ //simple mean. Resets when passed a zero.
	...
	{ //>0 valid, do average
		...
		if (num_PIs > PI_AVGS) num_PIs = PI_AVGS; //truncate number of averages at max value
		...
		for (uint8_t n = 0; n < num_PIs; n++)
		{
			rms += (uint32_t) ((int32_t) PI_int_buffer[new_ind] - int_avg)
					* (uint32_t) ((int32_t) PI_int_buffer[new_ind] - int_avg);
			new_ind--;
			if (new_ind < 0)
				new_ind += PI_AVGS;
		}
...
uint16_t compare_hr(struct spo2 *_sp, uint16_t zc_est, uint16_t b2b_est)
{
	...
	if (_K_F[b2b_filt].kalman_avg > 0)
		use_b2b = 1;
#ifdef _50sps
	if(_K_F[zc_filt].kalman_avg < ((uint16_t)MIN_50_SPS_PERIOD_2_USE_B2B)<<FIXED_BITS) use_b2b = 0;
#endif
	...
	{
		_sp->b2b_rms_norm1f = ((_K_F[b2b_filt].data_std << FIXED_BITS) << 2)
				/ _K_F[b2b_filt].kalman_avg;
		...
		if (use_b2b)
		{
			if (kalman_filters[b2b_filt].max_outlier_cnt
					< max_outlier_cnt_init[b2b_filt])
...
//	int32_t res_sum = 0;
	...
	*new_array_len = (uint16_t) ARRAY_LENGTH + 1 - baseline_len;
	for (uint8_t channel = 0; channel < _sp->ppg2+1; channel++)
	{
...
{
	for (uint8_t ch = 0; ch<_sp->ppg2+1; ch++)
	{
		_sp->flat_data[ch] = 0;
 ...
 * Return:		none
 ************************************************************************************/
void hpf_window_peak_detect(struct oxim * _ox, struct spo2 *_sp)
{
	uint16_t first_pt;
	uint16_t mid_pt;
	uint16_t last_pt;
	...
	for (uint8_t n = 0; n < (uint8_t) INTERVAL; n++)
	{
		...
		if (_sp->baseline_cnt == baseline_length)
		{
			_sp->data_cnt++;
			ppg_cnt++;
			for (uint8_t ch = 0; ch<_sp->ppg2+1; ch++) //for IR and optionally red channels
			{
 ...
 * Return:		none
 ******************************************************/
void get_dc_from_baseline(struct ob1203 *_ob, struct spo2 *_sp)
{
	if (_sp->baseline_cnt == ((uint16_t)1) << HPF_BITS) //if we have enough points
	{
		for(uint8_t ch = 0; ch < _sp->ppg2+1; ch++){
			_sp->mean[ch] = (_sp->baseline_sum[ch]>>HPF_BITS) + (uint32_t)_ob->target_counts[ch];
		}
	}
}
...
void get_pvi(struct spo2 *_sp, uint16_t hpf_len, uint16_t period_1f)
{
	...
	{
		_sp->pvi_avg += (_sp->pvi - _sp->pvi_avg) * (int32_t) PVI_FILTER_NUM
				/ (int32_t) PVI_FILTER_DEN;
	}
}
#endif
...
void set_sg_interval(struct spo2 *_sp)
{
	...
	else if ((_sp->sg_interval == LONG_SG)
			&& (_sp->period2use1f < (int16_t) SHORT_PER1F_THRESH_DOWN))
	{
	...
	else if ((_sp->sg_interval == SHORT_SG)
			&& (_sp->period2use1f > (int16_t) SHORT_PER1F_THRESH_UP))
	{
		_sp->persist_up++;
		_sp->persist_down = 0;
...
void do_noise_correction(struct spo2 *_sp, struct ob1203*_ob)
{
	...
	#endif
		for (uint8_t m =0;m<_sp->ppg2+1;m++) { //linear ramp down noise correction as signal goes to zero starting at 2x noise
...
void check_pi(struct oxim *_ox, struct spo2 *_sp)
{
	...
	if (_sp->ppg_rms1f[IR] != 0 && _sp->perfusion_index[IR] > 0)
	{
		...
		if (((_sp->ppg_rms1f[IR] >> (uint8_t)FIXED_BITS) < (uint32_t)MIN_RMS) || (_ox->latest_ir_data < (uint32_t)8000) || (_sp->perfusion_index[IR] < (float)MIN_PI))
		{
