...
***********************************************************************************************/
/* NIST vector plain text message used for both directly installed AES key and updated AES key*/
...
/* NIST vector AES-256 key directly installed */
#if 0
static uint8_t aes_256_key_installed_plaintext[32] = {
...
/* NIST cipher created externally using aes_256_key_installed_directly and plain_text*/
...
{
    memcpy(encrypted_new_key, g_encrypted_user_key_data.encrypted_user_key, ENCRYPTED_NEW_AES_256_KEY_SIZE);
}

/* access the initialization vector used for encrypting the new AES key */
void init_iv_encrypt_new_aes_key(void)
{
    memcpy(iv_encrypt_new_key, g_encrypted_user_key_data.initial_vector, AES_CBC_IV_SIZE);
}

/*****************************************************************************************
 *  @brief      cipher_operation_with_directly_installed_aes_key: this function reads the directly installed
 *              AES key from the data flash and perform cryptographic operations with the AES key
 *  @retval     bool
 *  **************************************************************************************/
bool cipher_operation_with_directly_installed_aes_key(void)
{
    R_SCE_Open(&sce_ctrl, &sce_cfg);

    memcpy(installed_key.value, (uint32_t *)DIRECT_AES_KEY_ADDRESS, HW_SCE_AES256_KEY_INDEX_WORD_SIZE*4);

    installed_key.type = SCE_KEY_INDEX_TYPE_AES256;

    error = R_SCE_AES256CBC_EncryptInit(&handle, &installed_key, iv);
...
bool update_aes_key(void)
{
    ...
    /*generate a new wrapped AES key using a previously installed Key-Update Key */
