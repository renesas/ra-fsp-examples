...
extern sce_aes_wrapped_key_t wrapped_new_user_key;
...
fsp_err_t flash_hp_data_flash_operations(void)
{
    ...
    memcpy(write_buffer, (uint8_t*)(wrapped_new_user_key.value), HW_SCE_AES256_KEY_INDEX_WORD_SIZE*4);

    /* Erase Block */
    error = R_FLASH_HP_Erase(&g_flash0_ctrl, FLASH_HP_DF_BLOCK_1, BLOCK_NUM);
    /* Error Handle */
    ...
    error = R_FLASH_HP_BlankCheck(&g_flash0_ctrl, FLASH_HP_DF_BLOCK_1,FLASH_HP_DF_BLOCK_SIZE, &blank_check_result);
    /* Error Handle */
    if (FSP_SUCCESS != error)
    {
        return error;
    ...
    error = R_FLASH_HP_Write(&g_flash0_ctrl, (uint32_t) write_buffer,FLASH_HP_DF_BLOCK_1, BLOCK_SIZE);
    /* Error Handle */
    if (FSP_SUCCESS != error)
    {
        return error;
    ...
    memset((uint8_t*)&wrapped_new_user_key, 0, 4*(HW_SCE_AES256_KEY_INDEX_WORD_SIZE+1));
    return error;
}
...
fsp_err_t store_new_user_key_to_data_flash(void)
{
    ...
    error = flash_hp_data_flash_operations();
    return error;
}
