...
static sce_ecc_public_wrapped_key_t ecc_public_key_installed;

static uint8_t encrypted_new_ecc_key[ENCRYPTED_ECC_PUB_BYTE_SIZE] = {0};
...
***********************************************************************************************/
/* NIST vector: plaintext message used for signature creation for the directly installed ECC public key */
...
/* NIST vector: externally generated NIST signature using the ECC private key which matches the installed ECC public key*/
...
{
    memcpy(encrypted_new_ecc_key, g_encrypted_ecc_key_data.encrypted_user_key, ENCRYPTED_ECC_PUB_BYTE_SIZE);
}

/* access the initialization vector used for encrypting the new ECC public key */
void init_iv_encrypt_new_ecc_key(void)
{
    memcpy(iv_encrypt, g_encrypted_ecc_key_data.initial_vector, IV_SIZE);
}

/*****************************************************************************************
 *  @brief      cryptograhpic_operation_with_directly_installed_ecc_pub_key: this function reads the directly installed
 *              ECC public key from the data flash and perform cryptographic operations with the ECC public key
 *  @retval     bool
 *  **************************************************************************************/
bool cryptograhpic_operation_with_directly_installed_ecc_pub_key(void)
{
    bool result = false;
    ...
    wrapped_ecc_public_key_size = sizeof(ecc_public_key_installed.value);
    memcpy((uint8_t *)(&(ecc_public_key_installed.value)), (uint8_t *)DIRECT_ECC_PUB_KEY_ADDRESS, wrapped_ecc_public_key_size);
    ...
    /* using the installed ECC public key to verify the NIST signature */
    error = R_SCE_ECDSA_secp256r1_SignatureVerify(&local_sig, &local_hash, &ecc_public_key_installed);
...
bool update_ecc_pub_key(void)
{
   ...
   /*generate a new wrapped ECC public key using a previously installed Key-Update Key */
...
bool cryptographic_operation_with_updated_ecc_pub_key(void)
{
    ...
    /* using the installed ECC public key to verify the matching NIST signature */
