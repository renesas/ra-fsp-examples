 ...
 * Regenerate this template if any MCUboot properties change in the bootloader project.
 Make the following updates to convert to binary, then sign image 1 (primary slot: 20000, secondary/upgrade slot: 10000):
 ...
 arm-none-eabi-objcopy -O binary ${ProjName}.elf ${ProjName}.bin & python ${workspace_loc:ra_mcuboot_ra6m4}/ra/mcu-tools/MCUboot/scripts/imgtool.py sign -k ${workspace_loc:ra_mcuboot_ra6m4}/ra/mcu-tools/MCUboot/root-ec-p256.pem --version 1.0.0+0 --header-size 0x80 --align 128 --max-align 128 --slot-size 0x10000 --max-sectors 4 --overwrite-only  --confirm --pad-header ${ProjName}.bin ${ProjName}_signed.bin

 Make the following updates to convert to binary, then sign image 2 (primary slot: 30000, secondary/upgrade slot: 50000):
 1. Update ${ProjName} to reflect the location of the image 2 .elf file
 2. Update ${workspace_loc:ra_mcuboot_ra6m4} to reflect the location of the root of the bootloader project
 arm-none-eabi-objcopy -O binary ${ProjName}.elf ${ProjName}.bin & python ${workspace_loc:ra_mcuboot_ra6m4}/ra/mcu-tools/MCUboot/scripts/imgtool.py sign -k ${workspace_loc:ra_mcuboot_ra6m4}/ra/mcu-tools/MCUboot/root-ec-p256.pem --version 1.0.0+0 --header-size 0x80 --align 128 --max-align 128 --slot-size 0x20000 --max-sectors 4 --overwrite-only  --confirm --pad-header ${ProjName}.bin ${ProjName}_signed.bin
 */
