...
namespace app {
	...
	// Constructor for the DetectorPreProcess class
    DetectorPreProcess::DetectorPreProcess(TfLiteTensor* inputTensor, bool rgb2Gray, bool convertToInt8)
    :   m_inputTensor{inputTensor},	// Initialize member variable with the input tensor
        m_rgb2Gray{rgb2Gray},	// Initialize member variable for RGB to Grayscale conversion flag
        m_convertToInt8{convertToInt8}	// Initialize member variable for conversion to int8 flag
    {}
    ...
    bool DetectorPreProcess::DoPreProcess(const void* data, size_t inputSize) {
        ...
        // convert RGB to Gray Scale if needed
        if (this->m_rgb2Gray) {
