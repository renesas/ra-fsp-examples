                           ...
                           tflite::MicroAllocator* allocator)
{
    ...
    if (!this->m_pInterpreter) {
        error("Failed to allocate interpreter\n");
        return false;
    }
...
void arm::app::Model::LogTensorInfo(TfLiteTensor* tensor)
{
    ...
    info("\ttensor occupies %d bytes with dimensions\n",
         tensor->bytes);
...
void arm::app::Model::LogInterpreterInfo()
{
    ...
    info("Activation buffer (a.k.a tensor arena) size used: %d\n",
...
{
    if (!this->IsInited()) {
        error("Model is not initialised! Terminating processing.\n");
        return false;
    }
