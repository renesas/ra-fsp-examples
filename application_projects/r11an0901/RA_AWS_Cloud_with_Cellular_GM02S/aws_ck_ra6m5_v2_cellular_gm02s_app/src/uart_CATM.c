...
fsp_err_t CATM_Ping(void)
{
    ...
    status = CATM_query("AT+PINGCFG=0", "OK", 'O', NULL);
...
fsp_err_t CATM_initialize(void)
{
    ...
    ONE_DELAYMSEC(1000);
    getResponse(tempBuffer);
    APP_DBG_PRINT("\r\n-> AT+CGSN=1  "); // Read the IMEI of the CATM module
    status = CATM_uart_print_user_msg((uint8_t *)"AT+CGSN=1\r\n");
    if (FSP_SUCCESS != status)
    {
        APP_DBG_PRINT ("\r\n ** UART TX FAILED ** \r\n");
        return(status);
        ...
        while (true)
        {
            ...
            if ((rcvdLength = getResponse(tempBuffer)) > 0)
            {
                ...
                if (isInStr(tempBuffer, "OK"))
                {
                    ...
                    if (parseString((uint8_t *)g_strIMEI, (uint8_t *)tempBuffer, '"', 1) == true)
                    {
                        sprintf (g_print_buf, "\r\n<- IMEI=%s\r\n", g_strIMEI);
                        APP_DBG_PRINT(g_print_buf);
                        break;
