...
static uint8_t model_input[IMAGE_DATA_SIZE];

/* Use the SRAM for TensorArena area for improvement performance. */
__attribute__((aligned(16), section(".sdram"))) static uint8_t compute_buffer[kBufferSize_sub_0000_net1 ];

extern uint8_t output_buffer[1000];
...
{
	// Copy input data to another buffer so the inference engine has stable input
    copy_data_to_mera((int8_t*)model_input, (uint8_t*)model_buffer_int8, (uint32_t)mera_input_size());
   
    /* Run inference over this image. */
    volatile uint32_t old_counter =  TimeCounter_CurrentCountGet();
    compute_sub_0000_net1(compute_buffer, model_input, output_buffer);
    ...
    create_int8_tensor(&outputTensor, (int8_t*)output_buffer, 0.00390625, -128);
