...
static void print_inf_time(void)
{
    ...
    char time_str[9] = {'0', '0', '0', '0','0', ' ', 'm', 's', '\0'};
    time_str[0] += (char)(time / 10000);
    time_str[1] += (char)((time / 1000) %10);
    time_str[2] += (char)((time / 100) % 10);
    time_str[3] += (char)((time / 10) % 10);
    time_str[4] += (char)(time % 10);
...
static void display_camera_image(void)
{
    ...
    d2_blitcopy(d2_handle,
                         CAM_QVGA_WIDTH, CAM_QVGA_HEIGHT,  // Source width/height
                                        (d2_blitpos) 0, 0, // Source position
                                        (d2_width) ((CAM_VGA_WIDTH) << 4), (d2_width) ((CAM_VGA_HEIGHT) << 4), // Destination size width/height
                                        (d2_width) (((0)/2) << 4), (d2_width) (((0)/2) << 4), // Destination offset position
                                        d2_tm_filter);
}
        ...
        xEventGroupWaitBits(g_ai_app_event, GLCDC_VSYNC, pdFALSE, pdTRUE, 1);
        {
            ...
            if (!overlay_drawn)
            {
               ...
               //
                ...
                print_bg_font_18(d2_handle, 20,  95, DISPLAY_FONT_SCALING, (char*)"Model: EfficientNet Lite0");

                print_bg_font_18(d2_handle, 20,  135, DISPLAY_FONT_SCALING, (char*)"Dataset: ImageNet");

                /*show inference time in ms*/
                print_bg_font_18(d2_handle, 20, 205, DISPLAY_FONT_SCALING, (char*)"CPU Inference Time:");
