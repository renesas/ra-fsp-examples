...
#define FPS_TARGET ((float)55)
/* PCLK is sys_clk_hz / 2 */
#define PIXEL_X_Y_RATIO (((float)sensor_max_x/sensor_max_y))
#define VTS sqrt((((float)sys_clk_hz / 2 / FPS_TARGET) / PIXEL_X_Y_RATIO))
...
*/
  ...
  /* The GLCDC Layer 1 framebuffer is updated in this callback */
...
/* Debug functions can be inserted in this callback */
