...
UCHAR sample_device_cert_ptr[2048];
...
void application_thread_entry(void)
{
    ...
    result = cellular_init();
    if (EWF_RESULT_OK != result)
    {
        IotLogErr("cellular Initialization  failed: %u\r\n", result);
        IotLogErrTrap(result);
    }

    status = usr_netx_init();
    if (NX_SUCCESS != status)
    {
        IotLogErr("usr_netx_init  failed: %u\r\n", status);
        IotLogErrTrap(status);
    }
