...
uint8_t getResponse(char *p2Result)     /* Return the length received (0 if none) */
{
    ...
    while (true)
    {
        ...
        {
            R_BSP_SoftwareDelay(10, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(10);  /* Wait a few more to catch the whole string */
            ...
            else
            {
                ...
                R_BSP_SoftwareDelay(10, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(10);  /* Wait a few more to see if there's more data */
            }
        }
        R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(1);
...
fsp_err_t CATM_Ping(void)
{
    ...
    while (true)
    {
        ...
        {
            printf_colour ("\r\n ** PING FAILED ** \r\n");
            R_BSP_SoftwareDelay(3000, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(1000);
        }
        else if (isGetExpectedResponsePing(&bPingTimeout) == true)
        {
            ...
            {
                /* Server did not respond:  RYZ014A responded with "+PING: 1,<ip addr>,-1,-1" */
                /* where Time = -1, TTL = -1 */
...
fsp_err_t CATM_initialize(void)
{
     ...
     * ATSQNAUTOCONNECT=1  OK              So that CATM module will boot with he radio on
     * ATCFUN=1            OK              To turn the radio on for the first time
     * ATCGSN              111111111111111            Read the IMEI of the CATM module
     * ATSQNCCID           SQNCCID: "1111111111111111111",""
    ...
    for (int ii=0; ii<10; ii++)
    {
        ...
        R_BSP_SoftwareDelay(100, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(100);
    }
    if (FSP_SUCCESS != status)
    {
        R_BSP_SoftwareDelay(10000, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(10000);
        return (status);
    }
    ...
    while (true)
    {
        ...
        R_BSP_SoftwareDelay(3000, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(3000);    /* 1 sec is too little, 2 sec's works, but 3 to be safe */
        IotLogDebug("\r\nConfirm Re-Start\r\n-> AT  ");
        status = CATM_uart_print_user_msg((uint8_t *)"AT\r\n");
        if (FSP_SUCCESS != status)
        {
    ...
    R_BSP_SoftwareDelay(1000, BSP_DELAY_UNITS_MILLISECONDS);// ONE_DelayMsec(1000);
    IotLogDebug("\r\n-> AT+SQNAUTOCONNECT=1  ");
    /* So that CATM module will boot with the radio on */
    ...
    R_BSP_SoftwareDelay(1000, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(1000);
    APP_DBG_PRINT("\r\n-> AT+CFUN=1  ");
    /* To turn the radio on for the first time */
    ...
    R_BSP_SoftwareDelay(1000, BSP_DELAY_UNITS_MILLISECONDS);//ONE_DelayMsec(1000);
    IotLogDebug("\r\n-> AT+SQNCCID  "); //Read ICCID of  the SIM
