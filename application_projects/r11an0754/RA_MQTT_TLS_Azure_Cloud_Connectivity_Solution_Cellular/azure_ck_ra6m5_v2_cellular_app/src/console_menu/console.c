...
void printf_colour(const char *format, ...)
{
     ...
     */
    status = tx_mutex_get(&consolprint_mutex, 10);
    ...
    if(TX_SUCCESS ==status)
    {
        ...
        status = tx_mutex_put(&consolprint_mutex);
 ...
 * @retval
 *********************************************************************************************************************/
void g_console_uart_callback(uart_callback_args_t *p_args)
{
        ...
        case UART_EVENT_RX_CHAR:
        {
            ...
            if (sizeof(g_out_of_band_received) > g_out_of_band_index)
            {
                ...
                if (UART_DATA_BITS_8 >= g_console_uart_cfg.data_bits)
                {
                    ...
                    {
                        /* Fill received data to write buffer when AZURE credentials are being saved */
 ...
 * Clear buffer when AZURE credentials are stored in flash
 * @param[in]
 * @param[in]
 * @retval None
 ******************************************************************************/
