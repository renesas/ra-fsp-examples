...
uint16_t  packet_create(uint8_t send_mode, uint16_t* raw_light_data_buff, uint16_t* raw_temp_data_buff, uint16_t* analog_value_ch19, uint16_t* analog_value_ch21, uint8_t* send_buff);
...
void data_output(uint8_t send_mode)
{
    ...
    /* Open the SCI module */
    err = R_SCI_UART_Open(&g_sci_uart9_ctrl, &g_sci_uart9_cfg);
    if(err != FSP_SUCCESS)
    {
    ...
    err = R_SCI_UART_Close(&g_sci_uart9_ctrl);
    if(err != FSP_SUCCESS)
    {
        pin_state_change(ERROR_LED, PIN_HIGH__LED_ON);
        __BKPT(0);
...
void data_send(uint8_t send_mode)
{
    ...
    /* Local variables */
    uint16_t current_analog_value_ch19;
    uint16_t current_analog_value_ch21;

    /* Get current ADC read value when SEND_CURRENT_ANA mode */
    if(send_mode == SEND_LEVEL_JUDGEMENT_RESULT)
    {
        current_analog_value_ch19 = ADC0_CH19_DATA_REGISTER;
        current_analog_value_ch21 = ADC0_CH21_DATA_REGISTER;
    }

    /* Create a packet */
    packet_size = packet_create(send_mode, &g_raw_light_data_buff[0], &g_raw_temp_data_buff[0],
                                &current_analog_value_ch19, &current_analog_value_ch21, &uart_send_buff[0]);

    /* Send the packet */
    err = R_SCI_UART_Write(&g_sci_uart9_ctrl, &uart_send_buff[0], packet_size);
    if(err != FSP_SUCCESS)
    {
        pin_state_change(ERROR_LED, PIN_HIGH__LED_ON);
        __BKPT(0);
...
uint16_t  packet_create(uint8_t send_mode, uint16_t* raw_light_data_buff, uint16_t* raw_temp_data_buff, uint16_t* analog_value_ch19, uint16_t* analog_value_ch21, uint8_t* send_buff)
{
    ...
    switch(send_mode)
    {
        ...
        case SEND_LEVEL_JUDGEMENT_RESULT:
            ...
            // Data
            ASCII_data_count = (uint16_t)sprintf(ASCII_data, "%04d/%04d",
                                                 (int)(*analog_value_ch19),
                                                 (int)(*analog_value_ch21));
