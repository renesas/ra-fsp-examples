    ...
    switch (rx_str[0])
    {
        ...
        /* Download a new image in the secondary slot using the XModem protocol */
           ...
           comms_send(tx_str, strlen((char *)tx_str));
            ...
            p_src = (uint8_t *) PRIMARY_IMAGE_START_ADDRESS;
            do {
                if (*p_src != 0xFF)
                {
                    is_blank = false;
                ...
                p_src++;
            } while ((is_blank == true) && (p_src < (uint8_t *)PRIMARY_IMAGE_END_ADDRESS));


            if (true == is_blank)
            {
                sprintf((char *)tx_str, "The primary slot blank\r\n");
                comms_send(tx_str, strlen((char *)tx_str));
            }
            else
            {
                sprintf((char *)tx_str, "Erasing the primary slot...\r\n");
                comms_send(tx_str, strlen((char *)tx_str));
                threads_and_interrupts(DISABLE);
                err = R_FLASH_LP_Erase(&g_flash0_ctrl, (uint32_t) PRIMARY_IMAGE_START_ADDRESS, PRIMARY_IMAGE_NUM_BLOCKS);
                if (FSP_SUCCESS == err)
                {
                    err = R_FLASH_LP_BlankCheck(&g_flash0_ctrl, PRIMARY_IMAGE_START_ADDRESS, PRIMARY_IMAGE_END_ADDRESS - PRIMARY_IMAGE_START_ADDRESS + 1, &blank_check_result);
                    if (FSP_SUCCESS == err)
                    {
                ...
                if (FSP_SUCCESS == err)
                {
                    sprintf((char *)tx_str, "Primary slot erased\r\n");
                    comms_send(tx_str, strlen((char *)tx_str));
                }
                else
                {
                    sprintf((char *)tx_str, "ERROR: Erasing the primary slot\r\n");
                    comms_send(tx_str, strlen((char *)tx_str));
            ...
            comms_send(tx_str, strlen((char *)tx_str));
            unsigned char xm_err = xmodem_download_and_program_flash(PRIMARY_IMAGE_START_ADDRESS);
            if (XM_OK != xm_err)
            {
                switch (xm_err)
                {
