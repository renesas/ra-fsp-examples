	...
	while (1)
	{
		...
		while ((retry_counter > 0) && (xm_ret == XM_TOUT))
		{
		    if (true == start_condition)
			{
				...
				err = comms_read((uint8_t *)&rx_byte_buffer[0], &rx_len, XMODEM_TIMEOUT_S);
				if (FSP_SUCCESS == err)
				{
				    xm_ret = OK;
				}
			...
			else
			{
                ...
                rx_len = 1;
                memset((void *)&rx_byte_buffer[0], 0, 132);
                err = comms_read((uint8_t *)&rx_byte_buffer[0], &rx_len, 1);
                if (FSP_SUCCESS == err)
                {
                    if (SOH == rx_byte_buffer[0])
                    {
                        rx_len = 131;
                        err = comms_read((uint8_t *)&rx_byte_buffer[1], &rx_len, XMODEM_TIMEOUT_S);
                        if (FSP_SUCCESS == err)
                        {
                            xm_ret = OK;
                        }
		...
		else			
		{
		    ...
		    // Secondary Image File Xmodem Transfer has completed so return ACK to sender
			...
			{				
                ...
                else
                {
                    ...
                    if (rx_byte_buffer[1] == expected_blk_num)
                    {
                        ...
                        status = R_FLASH_LP_Write(&g_flash0_ctrl, (uint32_t)&rx_byte_buffer[3], (uint32_t)address, 128);
