    ...
    while(1)
    {
        ...
        while ( (RetryCounter > 0) && (xm_ret == XM_TOUT) )
        {
            if(true == StartCondition)
            {
                ...
                err = comms_read((uint8_t *)&RxByteBuffer[0], &rx_len, 6000);   // Now 6 sec * 10 retries = 60 secs total timeout, was 10000 msecs for total 100 secs for timeout on wait for XMODEM start
            }
            else
            {
            ...
            {
                // any other error
                // xm_ret = XM_ERROR;  // include retries exceeded waiting on start
                // for FSP_ERR_USB_BUSY exits before all retries exhausted
                xm_ret = XM_TOUT;
            }
            RetryCounter--;
        }
        ...
        if ( xm_ret == XM_ERROR )
        {
            return ( XM_TIMEOUT );    // XMODEM file transfer did not start or other error.
        }
        ...
        else
        {
            ...
            if ( RxByteBuffer[0] == EOT )
            {
                ...
                sprintf((char *)tx_str, "\r\nImage Transferred to MCU\r\n");
                comms_send(tx_str, strlen((char *)tx_str));
                sprintf((char *)tx_str, "\r\nHit Enter Key in Tera Term after Reset\r\n");
                comms_send(tx_str, strlen((char *)tx_str));
                sprintf((char *)tx_str, "\r\nChoose Option 2 to Write Image\r\n");
                comms_send(tx_str, strlen((char *)tx_str));
                NVIC_SystemReset();
