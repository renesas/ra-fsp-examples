...
void icp_get(void)
{
    ...
    g_icp_data_s.temperature_C = g_temperature;
    g_icp_data_s.pressure_Pa = g_pressure*100; //Convert from mBar to Pa

    /* Send ICP sensor data to the ICP queue */
    status = send_data_to_queue(&g_icp_queue, &g_icp_data_s);
