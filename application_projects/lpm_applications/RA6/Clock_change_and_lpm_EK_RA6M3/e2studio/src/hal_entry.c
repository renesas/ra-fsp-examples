...
/* Flag to check the RESET status of the MCU */
static bool is_reset_from_deep_sw_sby = false;
...
void hal_entry(void)
{
    ...
    /*
     * Check whether the Reset path is via Power on Reset or from the Deep Software Standby mode reset.
     * If the Reset is from the Power on Reset, configure the RTC Calendar Time. There is no Battery backup on the board,
     * so we need to configure the RTC time every time.
     */
    if(true == is_reset_from_deep_sw_sby)
    {
        APP_PRINT("\r\nMCU Wake Up : from Deep SW standby Mode \r\n");
...
void R_BSP_WarmStart(bsp_warm_start_event_t event)
{
	if (BSP_WARM_START_POST_C == event)
	{
		...
		if(1 == R_SYSTEM->RSTSR0_b.DPSRSTF)
		{
			...
			is_reset_from_deep_sw_sby = true;
		}
		/* Configure pins. */
