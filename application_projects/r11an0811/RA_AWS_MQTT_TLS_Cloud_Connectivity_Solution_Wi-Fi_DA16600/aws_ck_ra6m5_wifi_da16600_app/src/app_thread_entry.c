...
void app_thread_entry(void *pvParameters)
{
    ...
    /* Wifi connection IP init */
    while (g_mqtt_status.mqtt_connect_status != true)
    {
        ...
        {
            FAILURE_INDICATION;
            APP_ERR_PRINT("** Wi-Fi Connectivity Failed **\r\n");
            APP_ERR_TRAP(err);
        }

        err = dns_query (USER_MQTT_ENDPOINT, (uint8_t *) &g_wifi_nwk.server_ip);
        if (FSP_SUCCESS == err)
        {
            if (g_wifi_nwk.server_ip != 0)
            {
                /* Convert the IP address to a string. */
                FREERTOS_INET_NTOA(g_wifi_nwk.server_ip, (char * ) cBuffer);
                APP_PRINT("\r\n MQTT End point %s with IP address = %s\r\n",USER_MQTT_ENDPOINT,cBuffer);
                NETWORK_CONNECT_INDICATION;
            }
            else
            {
                FAILURE_INDICATION;
                APP_ERR_PRINT("** DNS Query Failed for the MQTT End point **\r\n");
                APP_ERR_TRAP(err);
            }
        }
        else
        {
            FAILURE_INDICATION;
            APP_ERR_PRINT("** dns_query API Failed **\r\n");
            APP_ERR_TRAP(err);
        }
