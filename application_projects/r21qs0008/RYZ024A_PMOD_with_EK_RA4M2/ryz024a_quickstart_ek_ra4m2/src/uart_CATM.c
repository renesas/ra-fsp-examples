...
static bool isGetExpectedResponse(char *p2ExpectedStr)
{
    ...
    if (result == false)
    {
         // ONE_DelayMsec(5000);  // wait more time before sending next attempt
         ONE_DelayMsec(500);  // wait more time before sending next attempt    // 12/10/22
    }
    return (result);
}
...
fsp_err_t comms_initialize(void)
{
    ...
    fsp_err = R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_06_PIN_08, BSP_IO_LEVEL_LOW);
    ONE_DelayMsec(5);
    /* PMOD2 RESET : de-assert */
    fsp_err = R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_06_PIN_08, BSP_IO_LEVEL_HIGH);
...
/********************************************************************************************************************
* @brief      Validate SIM Card operation by Server Ping
* @param[in]  None
