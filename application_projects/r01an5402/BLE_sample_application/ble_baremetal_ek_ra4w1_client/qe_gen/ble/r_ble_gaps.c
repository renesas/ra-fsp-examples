...
{
    if (BLE_GAPS_DEV_NAME_LEN < p_gatt_value->value_len)
    {
        return BLE_ERR_INVALID_DATA;
    }

    memset(p_app_value->name, 0x00, BLE_GAPS_DEV_NAME_LEN);

    memcpy(p_app_value->name, (char *)p_gatt_value->p_value, p_gatt_value->value_len);
...
static ble_status_t encode_st_ble_gaps_dev_name_t(const st_ble_gaps_dev_name_t *p_app_value, st_ble_gatt_value_t *p_gatt_value)
{
    ...
    memset(p_gatt_value->p_value, 0x00, BLE_GAPS_DEV_NAME_LEN);

    memcpy((char *)p_gatt_value->p_value, p_app_value->name, p_app_value->length);
    p_gatt_value->value_len = (uint16_t)p_app_value->length;
