...
bool hwCreatekey(uint8_t * pubkey_buffer, psa_key_handle_t *key_handle )
{
    ...
    if(FSP_SUCCESS == status)
    {
        ...
        if(PSA_SUCCESS == status)
        {
           ...
           psa_set_key_type(&persistent_attributes, PSA_KEY_TYPE_ECC_KEY_PAIR(PSA_ECC_CURVE_SECP_R1));
           ...
           #else
                  psa_set_key_lifetime(&persistent_attributes, PSA_KEY_LIFETIME_PERSISTENT_WRAPPED);
           #endif
               ...
               if(FSP_SUCCESS == status)
               {
                   ...
                   if (FSP_SUCCESS == status)
                   {
                       ...
                       if(PSA_SUCCESS == psa_status)
                       {
                           ...
                           {
                               #ifdef SWO_PRINTF
                                   printf("ECC key generation successful, public key export failed:%x \n\r", (unsigned int)status);
                               #endif
                                   ret = false;
                           }
                       }
                       ...
                       {
                           printf("ECC SECP256R1 key generation failed: %x \n\r", (unsigned int)status);
                           ret = false;
                       }
                   }
                   else
                   {
                       printf("Little file system mounting failed: %x \n\r", (unsigned int)status);
                       ret = false;
                   }
               }
               else
               {
                   printf("Little file system formatting failed: %x \n\r", (unsigned int)status);
                   ret = false;
               }
           }
           else
           {
               printf("Little file system open failed: %x \n\r", (unsigned int)status);
               ret = false;
           }
        }
        else
        {
            printf("PSA Crypto init failed:%x \n\r", (unsigned int)status);
            ret = false;
        }
    }
    else
    {
        printf("platform set up failed:%x \n\r", (unsigned int)status);
        APP_ERR_TRAP(status);
    }
