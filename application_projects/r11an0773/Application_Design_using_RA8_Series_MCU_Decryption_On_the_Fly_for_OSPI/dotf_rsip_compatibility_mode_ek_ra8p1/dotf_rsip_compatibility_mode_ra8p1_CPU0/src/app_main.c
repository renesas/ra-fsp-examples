...
void app_main(void)
{
    ...
    /* Write the Plaintext Data to OSPI area */
    err = write_plaintext_data_to_ospi();
    if (FSP_SUCCESS != err)
    {
        app_handle_error(err, APP_WRITE_PLAINTEXT_DATA);
    }
    ...
    sprintf (sprintf_buffer, "\r\nDOTF RSIP Compatibility Mode evaluation finished successfully on CPU0. \r\n");
    jlink_print_to_console(sprintf_buffer);
