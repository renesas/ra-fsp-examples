...
#define CPU_CLOCK_IN_GHZ                        (R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_CPUCLK)/1000000000)
...
static const uint8_t *p_encryption_key = (uint8_t*)(BSP_PARTITION_DOTF_KEY_CPU0_S_START);
...
void app_main(void)
{
    ...
    for (uint32_t i = 0; i < NUM_ITERACTIONS_TO_EVALUATE; i++)
    {
        ...
        exec_time_ns = cycle_count  / CPU_CLOCK_IN_GHZ;
    ...
    for (uint8_t index = 0; index < LED_BLINK_COUNT; index++)
    {
        LED1_ON;
        R_BSP_SoftwareDelay(500, BSP_DELAY_UNITS_MILLISECONDS);
        LED1_OFF;
