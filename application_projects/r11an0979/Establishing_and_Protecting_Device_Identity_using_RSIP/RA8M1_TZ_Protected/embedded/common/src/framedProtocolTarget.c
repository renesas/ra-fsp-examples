...
/*****************************************************************************************************************
 *  @brief      share_with_ns: response to command from the PC
 *  @param[in]  pBuffer: buffer located in non-secure region
 *  @param[in]  numBytes: number of bytes for the data
 *  @retval     none
 *  ****************************************************************************************************************/
void share_with_ns(uint8_t *pBuffer, uint16_t *numBytes)
{
    uint8_t *const p_args_checked = cmse_check_pointed_object(pBuffer, CMSE_AU_NONSECURE);
       FSP_ASSERT (p_args_checked == pBuffer );

    memcpy (pBuffer, fpTxBuffer, len );
    *numBytes = len;
}
                                 ...
                                 const uint32_t numBytes __attribute__((unused)))
{
   ...
   ret = hwCreatekey(devPubKeyPtr);
    ...
    else
    {
        ...
        sendResponse(WRAPPER_KEY_REQUEST, devPubKeyPtr, (uint16_t) RSA_PUBLIC_KEY_SIZE);
    }
}
                                       ...
                                       const uint32_t numBytes)
{
   ...
   ret = hwSignData(hash, sig);
    ...
    else
    {
        ...
        sendResponse(WRAPPED_KEY_CHALLENGE_RESP, sig, (uint16_t)sizeof(RSASignature));
    }
    ...
    memset (sig,  0, sizeof (RSASignature));
}
