...
void main_thread_entry(void *pvParameters)
{
    ...
    while(true)
    {
        ...
        if(data_ready == true)
        {
            ...
            status = R_USB_Write (&g_basic0_ctrl, fpTxBuffer, len, g_usb_class_type);
            /* receive the next command */
            status = R_USB_Read(&g_basic0_ctrl, g_usb_buf, READ_BUFFER_SIZE, USB_CLASS_PCDC);
            if (FSP_SUCCESS != status)
            {
              /* Fatal error */
                APP_ERR_TRAP(status);
...
static void g_usb_class_req_handler(usb_setup_t * p_req)
{
    switch (p_req->request_type & USB_BREQUEST)
    {
        ...
        {
            R_USB_PeriControlDataGet(&g_basic0_ctrl, (void *) &g_usb_cls,
                    sizeof(g_usb_cls));
