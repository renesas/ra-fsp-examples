...
static bool sendCommandAndWaitAck(const HANDLE handle, const uint8_t *pCommand, const uint32_t commandLength, const uint32_t timeoutmS = 50000)
{
	bool success = false;
	FpContext *pContext = getContext(handle);
	if (pContext)
...
*/
static uint8_t *sendCommandAndWaitAckWithPayload(const HANDLE handle, const uint8_t *pCommand, const uint32_t commandLength, uint32_t *pPayloadLength, const uint32_t timeoutmS = 55000)
{
