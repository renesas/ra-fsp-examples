...
int CertAuth::CreateCert(RSA *rootKeyPair, RSA *deviceKey, char *outputFile, char *returnCert, int returnCertMaxLength, uint16_t *pemLength)
{
    X509 *cert;
    ...
    *pemLength = 0;
    ...
    // Convert the root key to EVP_PKEY
    EVP_PKEY *keyPairEVPRoot = EVP_PKEY_new();
    EVP_PKEY_assign_RSA(keyPairEVPRoot, rootKeyPair);  // Assign RSA root key to EVP_PKEY structure
    ...
    else
    {
        ...
        EVP_PKEY_assign_RSA(keyPairEVPDevice, deviceKey);  // Assign RSA device key to EVP_PKEY structure
    ...
    if (returnCert)
    {
        ...
        {
            printf("Certificate creation failed, not enough space allocated for cert. Cert size = %d\n", size);
            return -1;
        }
