...
extern "C" int RSA_verify(int type, const unsigned char *m, unsigned int m_length, const unsigned char *sigbuf, unsigned int siglen, RSA *rsa);
...
static RSA *pkey_get_rsa(EVP_PKEY *key, RSA **rsaKey)
{
    RSA *dtmp;
    
    if (!key) return NULL;
    
    dtmp = EVP_PKEY_get1_RSA(key);
    EVP_PKEY_free(key);
...
static bool ChallengeDeviceIdentity(DeviceContext *pContext)
{
	...
	else if (!(rsaKey = pkey_get_rsa(pkey, NULL)))
    {
        fprintf(stderr, "Failed to extract RSA public key from OEM Certificate\n");
    }
    else
    {

        verified = RSA_verify(NID_sha256, challengeHashed, SHA256_SIZE, pSignature, 512, rsaKey);
    }
...
static bool genAndProgramDevCert(DeviceContext *pContext)
{
    ...
    uint8_t *deviceKey = (uint8_t *)malloc(devicekeyLength);
    if (deviceKey == NULL)
    {
    ...
    memcpy(deviceKey, dk, devicekeyLength); // Copy the key (including header and modulus+exponent)

    // Create BIGNUM for the RSA modulus (n) and exponent (e)
    BIGNUM *bn_n = BN_bin2bn(deviceKey, 512, NULL); // The 512 bytes for n (modulus)
    BIGNUM *bn_e = BN_bin2bn(&deviceKey[512], 4, NULL); // The 4 bytes for e (exponent)

	if (bn_n == NULL || bn_e == NULL)
    {
        printf("Failed to convert key components to BIGNUM\n");
        free(deviceKey);
        return false;
    }

    // Create the RSA key using the modulus and exponent
    RSA *devKey  = RSA_new();
    if (devKey == NULL)
    {
	...
	devKey->n = bn_n;  // Set the modulus
	devKey->e = bn_e;  // Set the public exponent

    // Now, devKey contains the RSA public key
    printf("Successfully set the RSA public key in devKey\n");
