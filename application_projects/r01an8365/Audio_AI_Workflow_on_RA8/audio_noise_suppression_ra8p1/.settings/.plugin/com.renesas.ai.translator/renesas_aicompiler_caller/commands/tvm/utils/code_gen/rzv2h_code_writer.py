...
BYTE_ALIGN = 64
OFFSET_TO_SUPPORT_ADR_CONV = 0x1000000


@register_code_generator
class V2HSampleAppCppWriterForV210(Writer):
    ...
    def __init__(self,
                 ...
                 drpai_tvm_version: str = "v2.1.0"):
        super().__init__(filename)
        default_template_file = f"{mod_path}/template_apps/drpai_tvm_v210/rzv2h/sample_app.cpp"
    ...
    def _parse_preprocess_obj_size(self, preprocess_obj_dirs: List[str]):
        ...
        for preprocess_obj_dir in preprocess_obj_dirs:
            ...
            file = os.path.join(abs_preprocess_obj_dir, "addr_map.txt")
    ...
    def _calc_preprocess_obj_offset(self, preprocess_obj_sizes: List[int]):
        ...
        for preprocess_obj_size in preprocess_obj_sizes[:-1]:
            ...
            # To mask the lower 24 bits to 00_0000h.
            offset = offset_addr % OFFSET_TO_SUPPORT_ADR_CONV
            offset_addr = (offset_addr - offset) + OFFSET_TO_SUPPORT_ADR_CONV
    ...
    def _calc_drpai_tvm_model_obj_offset(self, preprocess_obj_sizes: List[int]):
        ...
        # To mask the lower 24 bits to 00_0000h.
        # ioctl(DRPAI_SET_ADRCONV), that is DRP-AI Driver API,
        # is used in MeraDrpRuntimeWrapper.cpp and PreRuntimeV2H.cpp.
        # According this specification, the lower 24 bits must match the address
        # before conversion and the address after conversion.
        # In MeraDrpRuntimeWrapper.cpp and PreRuntimeV2H.cpp,
        # the lower 24 bits of the address before conversion is set to 00_0000h.
        # Thus, this function sets the lower 24 bits to 00_0000h.
        offset = offset_addr % OFFSET_TO_SUPPORT_ADR_CONV
        offset_addr = (offset_addr - offset) + OFFSET_TO_SUPPORT_ADR_CONV
...
@register_code_generator
class V2HSampleAppCppWriterForV221(Writer):
    """Writer to output sample code depending on DRP-AI TVM Model Object.
    ...
    def __init__(self,
                 ...
                 drpai_tvm_version: str = "v2.2.1"):
        # DRP-AI TVM V2.1.0 and DRP-AI TVM V2.2.1 use the same template sample code.
        # Therefore, this line specifies the template sample code for DRP-AI TVM V2.1.0.
        default_template_file = f"{mod_path}/template_apps/drpai_tvm_v210/rzv2h/sample_app.cpp"
        template_file = default_template_file if template_file is None else template_file
        self._writer = V2HSampleAppCppWriterForV210(filename,
...
@register_code_generator
class V2HSampleAppCppWriterForV230(Writer):
    """Writer to output sample code depending on DRP-AI TVM Model Object.
    ...
    def __init__(self,
        ...
        # DRP-AI TVM V2.1.0 and DRP-AI TVM V2.3.0 use the same template sample code.
        # Therefore, this line specifies the template sample code for DRP-AI TVM V2.1.0.
        default_template_file = f"{mod_path}/template_apps/drpai_tvm_v210/rzv2h/sample_app.cpp"
        template_file = default_template_file if template_file is None else template_file
        self._writer = V2HSampleAppCppWriterForV210(filename,
