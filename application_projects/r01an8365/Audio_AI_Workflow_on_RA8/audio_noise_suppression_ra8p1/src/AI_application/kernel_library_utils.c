...
void NdArrayDescsForElementwiseBroadcast1(const int32_t* input0_data,
  int32_t input0_dims, const int32_t* input1_data, int32_t input1_dims,
  const int32_t* input2_data, int32_t input2_dims, NdArrayDesc* desc0_out,
  NdArrayDesc* desc1_out, NdArrayDesc* desc2_out, uint32_t num_dims) {

  int32_t extended_input0_shape[num_dims];
  int32_t extended_input1_shape[num_dims];
  int32_t extended_input2_shape[num_dims];
  int32_t N = num_dims;

  ExtendedShape(input0_data, input0_dims, extended_input0_shape, N);
  ExtendedShape(input1_data, input1_dims, extended_input1_shape, N);
  ExtendedShape(input2_data, input2_dims, extended_input2_shape, N);

  CopyDimsToDesc(extended_input0_shape, desc0_out, num_dims);
  CopyDimsToDesc(extended_input1_shape, desc1_out, num_dims);
  CopyDimsToDesc(extended_input2_shape, desc2_out, num_dims);

  for (int i = 0; i < N; ++i) {
    const int extent0 = extended_input0_shape[i];
    const int extent1 = extended_input1_shape[i];
    const int extent2 = extended_input2_shape[i];
