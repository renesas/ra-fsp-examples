...
void Div(const int8_t* input1, const int32_t* input1_shape, int32_t input1_dims,
  const int8_t* input2, const int32_t* input2_shape, int32_t input2_dims, int8_t* output,
  const int32_t* output_shape, int32_t output_dims, ArithmeticParams quant_params) {
...
int8_t SubOp(int8_t x, int8_t y, const ArithmeticParams quant_params) {
  const int32_t input1_val = quant_params.input1_offset + x;
  const int32_t input2_val = quant_params.input2_offset + y;
  const int32_t shifted_input1_val = input1_val * (1 << quant_params.left_shift);
  const int32_t shifted_input2_val = input2_val * (1 << quant_params.left_shift);
...
extern inline void BroadcastSubRecursiveDimensions(
  ...
  if (dimension > 0) {
    ...
    if (input1_is_broadcast) {
      // input1 is broadcast.
      SubBroadcast1(input1_data_ptr, input2_data_ptr, output_data_ptr,
                        compressed_output_shape[dimension], quant_params);
      *input2_offset_p += compressed_output_shape[dimension];
    } else if (input2_is_broadcast) {
      // input2 is broadcast.
      SubBroadcast2(input1_data_ptr, input2_data_ptr, output_data_ptr,
                        compressed_output_shape[dimension], quant_params);
      *input1_offset_p += compressed_output_shape[dimension];
    } else {
      // Sub element-wise.
...
// Supporting function for Sub op
extern inline void SubBroadcast1(const int8_t* input1_data, const int8_t* input2_data,
                         int8_t* output_data, int32_t size, ArithmeticParams quant_params) {

  for (int32_t c = 0; c < size; ++c) {
    output_data[c] = SubOp(input1_data[0], input2_data[c], quant_params);
  }
}

// Supporting function for Sub op
extern inline void SubBroadcast2(const int8_t* input1_data, const int8_t* input2_data,
                         int8_t* output_data, int32_t size, ArithmeticParams quant_params) {

  for (int32_t c = 0; c < size; ++c) {
    output_data[c] = SubOp(input1_data[c], input2_data[0], quant_params);
  }
}
...
// Ported from TF
void BroadcastComparison4DSlowWithScaling(const int8_t* input1,
  const int32_t* input1_shape, int32_t input1_dims, const int8_t* input2,
  const int32_t* input2_shape, int32_t input2_dims, int8_t* output,
  const int32_t* output_shape, int32_t output_dims, ArithmeticParams op_params) {
      ...
      for (int x = 0; x < extended_output_shape[2]; ++x) {
        for (int c = 0; c < extended_output_shape[3]; ++c) {
          const int32_t input1_val =
              input1_offset + input1[SubscriptToIndex(desc1, b, y, x, c)];
          const int32_t input2_val =
              input2_offset + input2[SubscriptToIndex(desc2, b, y, x, c)];
          ...
          output[Offset(extended_output_shape, b, y, x, c)] = (scaled_input1_val > scaled_input2_val);
        }
      }
    }
  }
...
// Ported from TF
void Greater(const int8_t* input1, const int32_t* input1_shape,
  int32_t input1_dims, const int8_t* input2, const int32_t* input2_shape,
  int32_t input2_dims, int8_t* output, const int32_t* output_shape,
  int32_t output_dims, ArithmeticParams op_params) {

  if (!HaveSameShapes(input1_shape, input1_dims, input2_shape, input2_dims)) {
    BroadcastComparison4DSlowWithScaling(input1, input1_shape, input1_dims,
      input2, input2_shape, input2_dims, output, output_shape, output_dims, op_params);
  } else {
...
void BroadcastSelect5DSlowQauntize(const int8_t* input1, const int32_t* input1_shape,
  int32_t input1_dims, const int8_t* input2, const int32_t* input2_shape,
  int32_t input2_dims, const int8_t* condition, const int32_t* condition_shape,
  int32_t condition_dims, int8_t* output, const int32_t* output_shape,
  int32_t output_dims) {
...
void SelectV2(const int8_t* input1, const int32_t* input1_shape, int32_t input1_dims,
  const int8_t* input2, const int32_t* input2_shape, int32_t input2_dims,
  const int8_t* condition, const int32_t* condition_shape, int32_t condition_dims,
  int8_t* output, const int32_t* output_shape, int32_t output_dims) {
  ...
  if (!same_shape) {
    BroadcastSelect5DSlowQauntize(input1, input1_shape, input1_dims,
...
// Ported from TF
void Gather(const int8_t* input, const int32_t* input_shape,
  int32_t input_dims, const int32_t* coords, const int32_t* coords_shape,
  int32_t coords_dims, int32_t axis, int32_t batch_dims, int8_t *output) {
    ...
    for (int outer = 0; outer < outer_size; ++outer) {
      for (int coord = 0; coord < coord_size; ++coord) {
        memcpy(output + (((batch * outer_size) + outer) * coord_size + coord) * inner_size,
               ...
               inner_size, sizeof(int8_t) * inner_size);
      }
    }
  }
}
...
// Ported from TF
void Maximum(const int8_t* input_1, const int8_t* input_2,  int8_t* output,
  const int32_t* input1_shape, int32_t input1_dims, const int32_t* input2_shape,
  int32_t input2_dims, const int32_t* output_shape, int32_t output_dims) {
  bool simple_mode = false;
