...
static uint8_t write_buffer[FLASH_WRITE_LENGTH];    /* buffer for flash writing */

static void setup_the_write_buffer(void)
{
    volatile int i = 0;

    for (i = 0; i < FLASH_WRITE_LENGTH; i++)
    {
        write_buffer[i] = FLASH_WRITE_TEST_DATA;
    }
}
...
void blinky_thread_entry (void * pvParameters)
{
    ...
    while (1)
    {
         ...
         */
        status_flash = flash_write_ns(FLASH_WRITE_TEST_BLOCK, (uint8_t *)write_buffer);
