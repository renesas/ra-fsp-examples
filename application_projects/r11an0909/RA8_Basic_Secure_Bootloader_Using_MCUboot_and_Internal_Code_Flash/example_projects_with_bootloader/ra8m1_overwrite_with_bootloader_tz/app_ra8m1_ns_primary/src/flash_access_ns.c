...
static uint8_t read_buffer[FLASH_WRITE_LENGTH];
...
bool flash_write_ns(uint32_t flash_address, uint8_t *write_buffer)
{
		...
		if (FSP_SUCCESS == err)
		{
			err = g_flash0_blank_check_guard(NULL, flash_address, FLASH_SECTOR_SIZE_256_BYTES, &blank_check_result);
			{
				if (FSP_SUCCESS == err)
				{
					...
					{
						err = g_flash0_write_guard(NULL, (uint32_t)write_buffer, flash_address, FLASH_WRITE_LENGTH);
						{
							if (FSP_SUCCESS == err)
							{
								 ...
								 */
								memcpy(read_buffer, (uint8_t *) flash_address, FLASH_WRITE_LENGTH);
								/*below line will cause secure fault if trying to read from secure flash region */
								if (!memcmp(write_buffer, read_buffer, FLASH_WRITE_LENGTH))
								{
									err = FSP_SUCCESS;
								}
							}
