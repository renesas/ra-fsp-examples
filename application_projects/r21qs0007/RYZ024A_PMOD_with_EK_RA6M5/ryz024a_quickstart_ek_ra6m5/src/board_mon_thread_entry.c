...
static void test_temperature_change()
{
    ...
    if (0 == g_adc_info_rtn.calibration_data)
    {
        /* read of TSN cal value may have failed.  Try read again now */
    	fsp_err = R_ADC_InfoGet (&g_adc_ctrl, &g_adc_info_rtn);
    	if ((fsp_err != FSP_SUCCESS) || (g_adc_info_rtn.calibration_data == 0))
    	{
    		// read of cal data failed or not in range
    		// use nominal cal value to keep temperature in range
    	    g_adc_info_rtn.calibration_data = 2016;
    	}
    }
